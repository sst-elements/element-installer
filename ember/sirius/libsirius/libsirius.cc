...
extern "C" int MPI_Irecv(void *buffer, int count, MPI_Datatype datatype, int src,
    ...
    if (sirius_output) {
        if ((nullptr == request) || ((*request) == MPI_REQUEST_nullptr)) {
            printUINT64((uint64_t) SIRIUS_MPI_REQUEST_nullptr);
        } else {
...
extern "C" int MPI_Wait(MPI_Request *request, MPI_Status *status) {
    ...
    if (sirius_output) {
        ...
        if ((nullptr == request) || MPI_REQUEST_nullptr == (*request)) {
            printUINT64((uint64_t) SIRIUS_MPI_REQUEST_nullptr);
        } else {
            printUINT64((uint64_t)(*request));
        }
...
extern "C" int MPI_Waitall(int count, MPI_Request array_of_requests[],
    ...
    if (sirius_output) {
        ...
        for (int i = 0; i < count; i++) {
            if (MPI_REQUEST_nullptr == (array_of_requests[i])) {
                printUINT64((uint64_t) SIRIUS_MPI_REQUEST_nullptr);
            } else {
