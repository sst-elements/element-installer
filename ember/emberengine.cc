...
EmberEngine::EmberEngine(SST::ComponentId_t id, SST::Params &params) :
    ...
    selfEventLink = configureSelfLink(
        "self", "1ps",
...
EmberEngine::ApiMap EmberEngine::createApiMap(OS *os,
    ...
    while (1) {
        ...
        Hermes::Interface *api = dynamic_cast<Interface *>(
            owner->loadSubComponent(moduleName, owner, modParams));
...
EmberGenerator *EmberEngine::initMotif(
    SST::Params params, const ApiMap &apiMap, int jobId, int motifNum,
    NodePerf *nodePerf) {
    EmberGenerator *gen = nullptr;
