...
TLBhierarchy::TLBhierarchy(ComponentId_t id, int tlb_id, int Levels, Params &params)
    ...
    if (page_migration) {
        ...
        if (!found)
            output->fatal(CALL_INFO, -1, "%s, Param not specified: memory_size\n",
                          getName().c_str());
...
bool TLBhierarchy::tick(SST::Cycle_t x) {
    ...
    if (hold ==
        1) // If the page table walker is suggesting to hold, the TLBhierarchy will stop processing any events
    {
        PTW->tick(x);
        return false;
    }
    ...
    while (!mem_reqs.empty() && !shootdown && !hold) {
        ...
        if (emulate_faults) {
            ...
            if ((*PTE).find(vaddr / 4096) == (*PTE).end())
                std::cout << "Error: That page has never been mapped:  " << vaddr / 4096
                          << std::endl;
