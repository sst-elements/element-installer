...
TLB::TLB(ComponentId_t id, int Page_size, int Assoc, TLB *Next_level, int Size)
...
TLB::TLB(ComponentId_t id, int tlb_id, TLB *Next_level, int Level, SST::Params &params)
    : ComponentExtension(id) {
    ...
    for (int i = 0; i < sizes; i++) {
        ...
        size[i] = ((uint32_t) params.find<uint32_t>("size" + std::to_string(i + 1) + "_L" + LEVEL,
                                                    1));


        assoc[i] = ((uint32_t) params.find<uint32_t>("assoc" + std::to_string(i + 1) + "_L" + LEVEL,
                                                     1));


        page_size[i] = 1024 * ((uint32_t) params.find<uint32_t>(
            "page_size" + std::to_string(i + 1) + "_L" + LEVEL, 4));
        ...
        } else // We know the translation doesn't exist, so we need to service a miss
        {
            ...
            if ((int) pending_misses.size() < (int) max_outstanding) {
                ...
                if ((level == 1) && (PENDING_MISS.find(addr / 4096) != PENDING_MISS.end())) {
                    ...
                    SAME_MISS[addr /
                              4096][ev] = 1; // Just adding it to the 2D map, so we later hand it back once the master miss is complete
        ...
        if (st->second <= x) {
            ...
            if (SIZE_LOOKUP.find(ready_by_size[st->first]) != SIZE_LOOKUP.end()) {
                ...
                if (!check_hit(addr, SIZE_LOOKUP[ready_by_size[st->first]])) {
                    insert_way(addr, find_victim_way(addr, SIZE_LOOKUP[ready_by_size[st->first]]),
                               SIZE_LOOKUP[ready_by_size[st->first]]);
