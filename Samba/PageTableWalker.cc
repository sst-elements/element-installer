...
PageTableWalker::PageTableWalker(ComponentId_t id, int Page_size, int Assoc,
                                 PageTableWalker *Next_level, int Size) : ComponentExtension(id) {


}


PageTableWalker::PageTableWalker(ComponentId_t id, int tlb_id, PageTableWalker *Next_level,
                                 int level, SST::Params &params) : ComponentExtension(id) {
...
bool PageTableWalker::tick(SST::Cycle_t x) {
    ...
    if (stall && emulate_faults) {
        ...
        if ((*PENDING_PAGE_FAULTS).find(stall_addr / page_size[0]) ==
            (*PENDING_PAGE_FAULTS).end()) {
            stall = false;
            *hold = 0;
        }
    ...
    for (; st_1 != not_serviced.end() && !(*shootdown); st_1++) {
        ...
        if (emulate_faults == 1) {
            ...
            if (fault) {
                stall_addr = addr;
                if ((*PENDING_PAGE_FAULTS).find(addr / page_size[0]) ==
                    (*PENDING_PAGE_FAULTS).end()) {
            ...
            if ((int) pending_misses.size() < (int) max_outstanding) {
                ...
                if (to_mem != nullptr) {
                    ...
                    ready_by[ev] = x + latency + 2 * upper_link_latency +
                                   page_walk_latency;  // the upper link latency is substituted for sending the miss request and reciving it, Note this is hard coded for the last-level as memory access walk latency, this ****definitely**** needs to change
        ...
        if (st->second <= x) {
            ...
            if (emulate_faults)
                if ((*PTE).find(addr / 4096) == (*PTE).end()) {
                    ...
                    std::cout << "The address is " << hex << addr << " (" << addr / 4096 << ")"
                              << std::endl;
                }
...
void PageTableWalker::initaitePageMigration(Address_t vaddress, Address_t paddress) {
    ...
    stall_addr = vaddress;
    if ((*PENDING_SHOOTDOWN_EVENTS).find(vaddress / page_size[0]) ==
        (*PENDING_SHOOTDOWN_EVENTS).end()) {
        (*PENDING_SHOOTDOWN_EVENTS)[vaddress / page_size[0]] = 0;
        (*PENDING_PAGE_FAULTS)[vaddress /
                               page_size[0]] = 0;        //add to pending page faults list
