...
Samba::Samba(SST::ComponentId_t id, SST::Params &params) : Component(id) {
    ...
    for (uint32_t i = 0; i < core_count; ++i) {
        sprintf(link_buffer, "cpu_to_mmu%"
        PRIu32, i);

        sprintf(link_buffer2, "mmu_to_cache%"
        PRIu32, i);
        ...
        SST::Link *link2 = configureLink(link_buffer, "0ps",
                                         new Event::Handler<TLBhierarchy>(TLB[i],
                                                                          &TLBhierarchy::handleEvent_CPU));
        cpu_to_mmu[i] = link2;


        SST::Link *link = configureLink(link_buffer2, "0ps",
                                        new Event::Handler<TLBhierarchy>(TLB[i],
                                                                         &TLBhierarchy::handleEvent_CACHE));
        mmu_to_cache[i] = link;


        sprintf(link_buffer3, "ptw_to_mem%"
        PRIu32, i);
        SST::Link *link3;

        if (self == 0)
            link3 = configureLink(link_buffer3,
                                  new Event::Handler<PageTableWalker>(TLB[i]->getPTW(),
                                                                      &PageTableWalker::recvResp));
        else
            link3 = configureSelfLink(link_buffer3, std::to_string(page_walk_latency) + "ns",
                                      new Event::Handler<PageTableWalker>(TLB[i]->getPTW(),
                                                                          &PageTableWalker::recvResp));
        ...
        sprintf(link_buffer4, "ptw_to_opal%"
        PRIu32, i);
        SST::Link *link4;


        if (emulate_faults == 1) {
            link4 = configureLink(link_buffer4,
                                  new Event::Handler<PageTableWalker>(TLB[i]->getPTW(),
                                                                      &PageTableWalker::recvOpal));
            ptw_to_opal[i] = link4;
            ...
            event_link = configureSelfLink(link_buffer, "1ns",
                                           new Event::Handler<PageTableWalker>(TLB[i]->getPTW(),
                                                                               &PageTableWalker::handleEvent));

            TLB[i]->getPTW()->setEventChannel(event_link);
            TLB[i]->setPageTablePointers(&CR3, &PGD, &PUD, &PMD, &PTE, &MAPPED_PAGE_SIZE1GB,
                                         &MAPPED_PAGE_SIZE2MB, &MAPPED_PAGE_SIZE4KB,
                                         &PENDING_PAGE_FAULTS, &PENDING_SHOOTDOWN_EVENTS);

        }

        TLB[i]->setPTWLink(
            link3); // We make a connection from the TLB hierarchy to the memory hierarchy, for page table walking purposes
...
void Samba::init(unsigned int phase) {
    ...
    for (uint32_t i = 0; i < core_count; i++) {
        ...
        while ((ev = mmu_to_cache[i]->recvInitData())) {
            ...
            if (mEv &&
                mEv->getInitCmd() == SST::MemHierarchy::MemEventInit::InitCommand::Coherence) {
