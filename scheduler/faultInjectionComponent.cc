...
faultInjectionComponent::faultInjectionComponent(ComponentId_t id, Params &params) : Component(id) {
    ...
    selfLink = configureSelfLink("linkToSelf", new Event::Handler<faultInjectionComponent>(this,
                                                                                           &faultInjectionComponent::handleSelfLink));
    ...
    for (int nodeCount = 0; !retrievedLastNode; nodeCount++) {
        ...
        SST::Link *nodeLink = configureLink(nodeName, SCHEDULER_TIME_BASE,
                                            new Event::Handler<faultInjectionComponent, int>(this,
                                                                                             &faultInjectionComponent::handleNodeLink,
                                                                                             nodeCount));
        if (nodeLink) {
    ...
    if (!params.find<std::string>("faultInjectionFilename").empty())
        ...
        schedout.fatal(CALL_INFO, 1,
                       "faultInjectionComponent: faultInjectionFilename must be provided\n");
    ...
    if (!params.find<std::string>("injectionFrequency").empty())
        ...
        schedout.fatal(CALL_INFO, 1,
                       "faultInjectionComponent: injectionFrequency must be specified\n");
...
void faultInjectionComponent::setup() {
    for (std::vector<SST::Link *>::iterator nodeIter = nodeLinks.begin();
         nodeIter != nodeLinks.end(); nodeIter++) {
...
std::map <std::string, std::string> *faultInjectionComponent::readFailFile() {
    ...
    while (!fileContainsToken) {
        if (Utils::file_exists(failFilename) &&
...
void faultInjectionComponent::handleSelfLink(SST::Event *event) {
    if (nullptr != dynamic_cast<sendFailsEvent *>( event )) {
        std::map <std::string, std::string> *failNodes = readFailFile();

        for (std::map<std::string, std::string>::iterator nodeIter = failNodes->begin();
             nodeIter != failNodes->end(); nodeIter++) {
            if (std::find(nodeIDs.begin(), nodeIDs.end(), nodeIter->first) != nodeIDs.end()) {
                SST::Event *failEvent = new CommunicationEvent(FAIL_JOBS,
                                                               new std::string(nodeIter->second));
                nodeLinks[std::distance(nodeIDs.begin(), std::find(nodeIDs.begin(), nodeIDs.end(),
                                                                   nodeIter->first))]->send(
                    failEvent);
            } else {
                schedout.output(
                    "failInjectionComponent: connection to node %s does not exist, ignoring\n",
