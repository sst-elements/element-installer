...
void FST::jobArrives(Job *inj, Scheduler *insched, Machine *inmach) {
    ...
    Machine *mach = new SimpleMachine(inmach->numNodes, true, inmach->coresPerNode, nullptr);
    ...
    for (unsigned int x = 0; x < running->size(); x++) {
        ...
        if (nullptr == ai) {
            schedout.fatal(CALL_INFO, 1,
                           "in FST could not allocate running job\nMachine had %d processors for %s",
                           mach->getNumFreeNodes(), running->at(x)->toString().c_str());
        }
    ...
    Statistics *stats = new Statistics(mach, sched, alloc, taskMap, nullstr.c_str(), nullcstr, true,
                                       this);

    //have to keep track of each job's end time
    bool (*compeventsptr)(Job *, Job *) = compevents;
    std::multimap < Job * , unsigned long, bool(*)
    (Job * , Job *) > *endtimes =
        new std::multimap < Job *, unsigned long, bool(*)
    (Job * , Job *) > (compeventsptr);

    for (unsigned int x = 0; x < running->size(); x++) {
        endtimes->insert(pair<Job *, unsigned long>(running->at(x), running->at(x)->getStartTime() +
                                                                    running->at(
                                                                        x)->getActualTime()));
    }
    ...
    while (!success && !endtimes->empty()) {
        ...
        if (!found)
            schedout.fatal(CALL_INFO, 1, "Could not find %s in running or toRun",
                           endtimes->begin()->first->toString().c_str());

        if (jobToAi->find(endtimes->begin()->first) == jobToAi->end())
            schedout.fatal(CALL_INFO, 1, "couldn't find %s in jobToAi",
                           endtimes->begin()->first->toString().c_str());
    ...
    if (!success)
        schedout.fatal(CALL_INFO, 1, "Could not find time for %s in FST\n", j->toString().c_str());
        ...
        if (ai != nullptr) {
            ...
            if (ai->job == j) {
                ...
                schedout.debug(CALL_INFO, 7, 0, "%lu: FST starting %s\n", time,
                               ai->job->toString().c_str());
                endtimes->insert(
                    pair<Job *, unsigned long>(ai->job, time + ai->job->getActualTime()));
...
void FST::jobCompletes(Job *j) {
    ...
    schedout.fatal(CALL_INFO, 1,
                   "FST could not find completing job in its currently-running list\n");
}
...
unsigned long FST::getFST(int num) {
    if (num < 0 || num >= numjobs)
        schedout.fatal(CALL_INFO, 1, "trying to get FST value out of range: %d", num);
    return jobFST[num];
}
