        ...
        switch (schedulername(schedparams->at(0))) {
            ...
            case EASY:
                ...
                if (schedparams->size() == 1) {
                    ...
                    EASYScheduler::JobComparator *comp = EASYScheduler::JobComparator::Make(
                        schedparams->at(1));
                    if (comp == nullptr) {
                        schedout.fatal(CALL_INFO, 1,
                                       "Argument to Easy Scheduler parameter not found:%s",
                                       schedparams->at(1).c_str());
                    }
                    return new EASYScheduler(comp);
                } else {
                    schedout.fatal(CALL_INFO, 1,
                                   "EASY Scheduler requires 1 or 0 parameters (determines type of queue or defaults to FIFO");
                }
                break;
            ...
            case CONS:
                ...
                if (schedparams->size() == 1) {
                    return new StatefulScheduler(numNodes,
                                                 StatefulScheduler::JobComparator::Make("fifo"),
                                                 true, mach);
                } else {
                    return new StatefulScheduler(numNodes, StatefulScheduler::JobComparator::Make(
                        schedparams->at(1)), true, mach);
                }
                break;
            ...
            case PRIORITIZE:
                ...
                if (schedparams->size() == 1) {
                    schedout.fatal(CALL_INFO, 1,
                                   "PrioritizeCompression scheduler requires number of backfill times as an argument");
                }
                filltimes = strtol(schedparams->at(1).c_str(), nullptr, 0);
                if (2 == schedparams->size()) {
                    ...
                    return new StatefulScheduler(numNodes, StatefulScheduler::JobComparator::Make(
                        schedparams->at(2)), filltimes, mach);
                }
                break;
            ...
            case ELC:
                ...
                if (schedparams->size() == 1) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Even Less Conservative scheduler requires number of backfill times as an argument");
                }
                filltimes = strtol(schedparams->at(1).c_str(), nullptr, 0);
                if (schedparams->size() == 2) {
                    ...
                    return new StatefulScheduler(numNodes, StatefulScheduler::JobComparator::Make(
                        schedparams->at(2)), filltimes, true, mach);
                }
                break;
        ...
        switch (machinename(schedparams->at(0))) {
            ...
            case MESH: {
                ...
                if (schedparams->size() != 3 && schedparams->size() != 4) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Wrong number of arguments for Mesh 3D Machine:\nNeed 3 (x, y, and z dimensions) or 2 (z defaults to 1)");
                }

                std::vector<int> dims(3);
                dims[0] = strtol(schedparams->at(1).c_str(), nullptr, 0);
                dims[1] = strtol(schedparams->at(2).c_str(), nullptr, 0);
                if (schedparams->size() == 4) {
                    dims[2] = strtol(schedparams->at(3).c_str(), nullptr, 0);
                } else {
                    dims[2] = 1;
                }
                if (dims[0] * dims[1] * dims[2] != numNodes) {
                    schedout.fatal(CALL_INFO, 1,
                                   "The dimensions of the mesh do not correspond to the number of nodes");
                }
            ...
            case TORUS: {
                if (schedparams->size() != 3 && schedparams->size() != 4) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Wrong number of arguments for Torus 3D Machine:\nNeed 3 (x, y, and z dimensions) or 2 (z defaults to 1)");
                }
            ...
            case DRAGONFLY: {
                if (schedparams->size() < 7) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Wrong number of arguments for Dragonfly Machine: Usage:"
                                   "dragonfly[routersPerGroup, portsPerRouter, opticalsPerRouter, nodesPerRouter, localTopology, globalTopology]");
                }
                int routersPerGroup = strtol(schedparams->at(1).c_str(), nullptr, 0);
                int portsPerRouter = strtol(schedparams->at(2).c_str(), nullptr, 0);
                int opticalsPerRouter = strtol(schedparams->at(3).c_str(), nullptr, 0);
                int nodesPerRouter = strtol(schedparams->at(4).c_str(), nullptr, 0);
                ...
                retMachine = new DragonflyMachine(routersPerGroup, portsPerRouter,
                                                  opticalsPerRouter,
...
Allocator *Factory::getAllocator(SST::Params &params, Machine *m, schedComponent *sc) {
    if (params.find<std::string>("allocator").empty()) {
        ...
        vector <string> *nearestparams = nullptr;
        switch (allocatorname(schedparams->at(0))) {
            ...
            case CONSTRAINT: {
                ...
                if (depsFile.empty()) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Constraint Allocator requires ConstraintAllocatorDependencies scheduler parameter");
                }
                if (constFile.empty()) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Constraint Allocator requires ConstraintAllocatorConstraints scheduler parameter");
                }
            ...
            case NEARESTAMAP:
                if (!params.find<std::string>("taskMapper").empty()
                    && taskmappername(parseparams(params.find<std::string>("taskMapper"))->at(0)) ==
                       NEARESTAMT) {
            ...
            case SIMPLESPREAD: {
                ...
                if (dMachine == nullptr) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Simple Spread allocator requires dragonfly machine\n");
                } else {
            ...
            case DFLYHYBRID: {
                ...
                if (dMachine == nullptr) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Dragonfly Hybrid allocator requires dragonfly machine\n");
                } else {
            ...
            case DFLYJOKANOVIC: {
                ...
                if (dMachine == nullptr) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Dragonfly Jokanovic allocator requires dragonfly machine\n");
                } else {
            ...
            case DFLYRDR: {
                ...
                if (dMachine == nullptr) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Dragonfly Random Router allocator requires dragonfly machine\n");
                } else {
            ...
            case DFLYRDG: {
                ...
                if (dMachine == nullptr) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Dragonfly Random Group allocator requires dragonfly machine\n");
                } else {
            ...
            case DFLYRRN: {
                ...
                if (dMachine == nullptr) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Dragonfly Round Robin Nodes allocator requires dragonfly machine\n");
                } else {
            ...
            case DFLYRRR: {
                ...
                if (dMachine == nullptr) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Dragonfly Round Robin Routers allocator requires dragonfly machine\n");
                } else {
            ...
            case DFLYSLURM: {
                ...
                if (dMachine == nullptr) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Dragonfly Slurm allocator requires dragonfly machine\n");
                } else {
...
TaskMapper *Factory::getTaskMapper(SST::Params &params, Machine *mach) {
    ...
    if (params.find<std::string>("taskMapper").empty()) {
        ...
        switch (taskmappername(taskmapparams->at(0))) {
            ...
            case NEARESTAMT:
                if (!params.find<std::string>("allocator").empty()
                    && allocatorname(parseparams(params.find<std::string>("allocator"))->at(0)) ==
                       NEARESTAMAP) {
...
int Factory::getFST(SST::Params &params) {
    if (params.find<std::string>("FST").empty()) {
        ...
        switch (FSTname(FSTparams->at(0))) {
            ...
            default:
                schedout.fatal(CALL_INFO, 1,
                               "Could not parse FST type; should be none, strict, or relaxed");
        }
    }
