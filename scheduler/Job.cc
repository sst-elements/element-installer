...
Job::Job(unsigned long arrivalTime, int procsNeeded, unsigned long actualRunningTime,
         unsigned long estRunningTime, CommInfo inComm, PhaseInfo inPhase) : commInfo(inComm),
                                                                             phaseInfo(inPhase) {
...
Job::Job(long arrivalTime, int procsNeeded, long actualRunningTime,
         long estRunningTime, std::string ID, CommInfo inComm, PhaseInfo inPhase) : commInfo(
    inComm), phaseInfo(inPhase) {
    initialize(arrivalTime, procsNeeded, actualRunningTime, estRunningTime);
    this->ID = ID;
}
...
Job::Job(const Job &job) : commInfo(job.commInfo) {
    ...
    hasRun = job.hasRun;
    started = job.started;
    if (job.taskCommInfo == nullptr) {
        taskCommInfo = nullptr;
    } else {
