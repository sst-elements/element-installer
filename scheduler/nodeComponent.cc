...
void readCSVpairsIntoMap(Tokenizer <escaped_list_separator> Tokenizer,
    ...
    for (unsigned int counter = 0; counter < tokens.size(); counter += 2) {
        ...
        Map->insert(std::pair<std::string, float>(tokens.at(counter),
                                                  atof(tokens.at(counter + 1).c_str())));
    }
}


void readDelaysIntoMap(Tokenizer <escaped_list_separator> Tokenizer,
    ...
    for (unsigned int counter = 0; counter + 2 < tokens.size(); counter += 3) {
        ...
        if (upperLatencyBound < lowerLatencyBound) {
            ...
            schedout.fatal(CALL_INFO, 1,
                           "nodeNum %d's fault delay upper bound is lower than its lower bound: %s, %s, %s\nBad delay bounds",
                           nodeNum, tokens.at(counter).c_str(), tokens.at(counter + 1).c_str(),
                           tokens.at(counter + 2).c_str());
        }

        FaultLatencyBounds->insert(std::pair < std::string, std::pair < unsigned int,
                                   unsigned int > >
                                   (faultName, std::pair<unsigned int, unsigned int>(
                                       lowerLatencyBound, upperLatencyBound)));
    }
}
...
nodeComponent::nodeComponent(ComponentId_t id, Params &params) :
    ...
    if (params.find<std::string>("nodeNum").empty()) {
        Simulation::getSimulation()->getSimulationOutput().fatal(CALL_INFO, -1,
                                                                 "couldn't find the nodeNum param for this node\n");
    }
    ...
    Scheduler = configureLink("Scheduler", SCHEDULER_TIME_BASE,
                              new Event::Handler<nodeComponent>(this, &nodeComponent::handleEvent));
    Builder = configureLink("Builder", SCHEDULER_TIME_BASE,
                            new Event::Handler<nodeComponent>(this, &nodeComponent::handleEvent));
    failureInjector = configureLink("faultInjector", SCHEDULER_TIME_BASE,
                                    new Event::Handler<nodeComponent>(this,
                                                                      &nodeComponent::handleEvent));
    SelfLink = configureSelfLink("linkToSelf", SCHEDULER_TIME_BASE,
                                 new Event::Handler<nodeComponent>(this,
                                                                   &nodeComponent::handleSelfEvent));
    FaultLink = configureSelfLink("SelfFaultLink", SCHEDULER_TIME_BASE,
                                  new Event::Handler<nodeComponent>(this,
                                                                    &nodeComponent::handleFaultEvent));

    SST::Link *tmp;
    char port[16];

    for (int counter = 0; counter == 0 || tmp != nullptr; counter++) {
        sprintf(port, "Parent%d", counter);

        tmp = configureLink(port, SCHEDULER_TIME_BASE,
                            new Event::Handler<nodeComponent>(this, &nodeComponent::handleEvent));
        if (tmp) {
    ...
    if (!((((int) ChildFaultLinks.size()) > 0) || Scheduler)) {
        schedout.fatal(CALL_INFO, 1,
                       "Node number %d has neither children nor a link to the scheduler.\nInvalid node",
                       nodeNum);
    ...
    readCSVpairsIntoMap(
        Tokenizer<escaped_list_separator>(params.find<std::string>("faultActivationRate")),
        &Faults);
    readDelaysIntoMap(
        Tokenizer<escaped_list_separator>(params.find<std::string>("errorPropagationDelay")),
        &FaultLatencyBounds, nodeNum);
    readCSVpairsIntoMap(
        Tokenizer<escaped_list_separator>(params.find<std::string>("errorCorrectionProbability")),
        &errorCorrectionProbability);
    readCSVpairsIntoMap(
        Tokenizer<escaped_list_separator>(params.find<std::string>("errorMessageProbability")),
        &errorLogProbability);
    readCSVpairsIntoMap(
        Tokenizer<escaped_list_separator>(params.find<std::string>("jobFailureProbability")),
        &jobKillProbability);
...
unsigned int nodeComponent::genFaultLatency(std::string faultName) {
    ...
    if (FaultLatencyBounds.find(faultName) != FaultLatencyBounds.end()) {
        ...
        if (bounds.first == 0 &&
            bounds.second == 0) {    // bounds of [0, 0] were specified, so use latency of zero
            //      std::cout << "no bounds, using 0" << std::endl;
        } else if (bounds.second ==
                   bounds.first) {    // no upper bound, so just use the lower bound as our latency
            ...
            latency = (((unsigned int) jrand48(yumyumErrorLatencyRand48State)) %
                       (bounds.second - bounds.first + 1)) + bounds.first;
...
bool nodeComponent::canCorrectError(FaultEvent *error) {
    return errorCorrectionProbability.find(error->faultType) != errorCorrectionProbability.end() &&
           erand48(yumyumErrorCorrectionRand48State) <
           (errorCorrectionProbability.find(error->faultType)->second);
}
            ...
            if (Scheduler && jobNum != -1) {
                ...
                if (faultEvent->shouldKillJob == FAULT_EVENT_SHOULDKILL
                    || (jobKillProbability.find(faultEvent->faultType) == jobKillProbability.end()
                        || erand48(yumyumJobKillRand48State) <
                           jobKillProbability.find(faultEvent->faultType)->second)) {

                    //SelfLink->send( getCurrentSimTime(), new JobKillEvent( this->jobNum ) ); 

                    faultEvent->jobNum = jobNum;
                    faultEvent->nodeNumber = nodeNum;
                    Scheduler->send((unsigned int) genFaultLatency(faultEvent->faultType),
                                    faultEvent->copy());
                ...
                if (faultEvent->shouldKillJob == FAULT_EVENT_UNDECIDED &&
                    ...
                    if (erand48(yumyumJobKillRand48State) <
                        jobKillProbability.find(faultEvent->faultType)->second) {
                        faultEvent->shouldKillJob = FAULT_EVENT_SHOULDKILL;
                    }
                }
                for (std::vector<SST::Link *>::iterator it = ChildFaultLinks.begin();
                     it != ChildFaultLinks.end(); ++it) {
                    (*it)->send((unsigned int) genFaultLatency(faultEvent->faultType),
                                faultEvent->copy());
                }
            }
        }
...
void nodeComponent::handleSelfEvent(Event *ev) {
    ...
    if (event) {
        if (killedJobs.erase(event->jobNum) == 1) {
            ...
            schedout.fatal(CALL_INFO, 1,
                           "Error!! We are not running this job we're supposed to finish!\n");
        ...
        logFault(faultEvent);
        sendNextFault(std::string(
            faultEvent->faultType));     // handled this fault, send another fault to the future
        handleFaultEvent(ev);
    } else if (dynamic_cast<JobKillEvent *>(ev)) {
        handleJobKillEvent(dynamic_cast<JobKillEvent *>(ev));
        delete ev;
        ...
        if (commEvent->CommType == START_FAULTING) {
            ...
            for (std::map<std::string, float>::iterator faultIter = Faults.begin();
                 faultIter != Faults.end(); faultIter++) {
...
void nodeComponent::logError(FaultEvent *faultEvent) {
    ...
    if ((this->errorLogProbability.find(faultEvent->faultType) != this->errorLogProbability.end() &&
         erand48(yumyumErrorLogRand48State) <
         this->errorLogProbability.find(faultEvent->faultType)->second) ||
