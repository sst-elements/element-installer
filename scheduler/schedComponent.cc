...
schedComponent::schedComponent(ComponentId_t id, Params &params) :
    ...
    if (runningTimeSeed.compare("none") != 0) {
        rng = new SST::RNG::MersenneRNG(strtol(runningTimeSeed.c_str(), nullptr, 0));
    } else {
        rng = new SST::RNG::MersenneRNG();
    }

    //set up the all-important self-link
    selfLink = configureSelfLink("linkToSelf", new Event::Handler<schedComponent>(this,
    ...
    while (!done) {
        ...
        SST::Link *l = configureLink(name, SCHEDULER_TIME_BASE,
                                     new Event::Handler<schedComponent, int>(this,
                                                                             &schedComponent::handleCompletionEvent,
                                                                             nodeCount));
        if (l) {
            nodes.push_back(l);
            nodeCount++;
    ...
    if (FSTtype > 0) {
        calcFST = new FST(
            FSTtype);  //must call calcFST -> setup() once we know the number of jobs (in other words, in setup())
    } else {
        calcFST = nullptr;
    }
        ...
        if (temp_string.compare("ON") == 0) {
            ...
            if (params.find<std::string>("completedJobsTrace").empty()) {
                schedout.fatal(CALL_INFO, 1,
                               "detailedNetworkSim: You must specify a completedJobsTrace name!\n");
            }
            // look for a file that lists all jobs that are still running on ember
            if (params.find<std::string>("runningJobsTrace").empty()) {
                schedout.fatal(CALL_INFO, 1,
                               "detailedNetworkSim: You must specify a runningJobsTrace name!\n");
            }
        }
    }

    if (nullptr != dynamic_cast<SimpleMachine *>(machine)) {
        char timestring[] = "time";
        stats = new Statistics(machine, scheduler, theAllocator, theTaskMapper, trace, timestring,
                               false, calcFST);
    } else {
        if (doDetailedNetworkSim == false) {
            char timestring[] = "time,alloc";
            stats = new Statistics(machine, scheduler, theAllocator, theTaskMapper, trace,
                                   timestring, false, calcFST);
        } else {
...
void schedComponent::setup() {
    for (vector<SST::Link *>::iterator nodeIter = nodes.begin();
    ...
    if (doDetailedNetworkSim) {
        ...
        for (std::map<int, unsigned long>::iterator it = emberFinishedJobs.begin();
             it != emberFinishedJobs.end(); it++) {
...
void schedComponent::handleCompletionEvent(Event *ev, int node) {
    if (dynamic_cast<CommunicationEvent *>(ev)) {
        ...
        if (nullptr == runningJobs[jobNum].tmi) {
        ...
        if (event) {
            ...
            //if ai is nullptr we already killed this job
...
void schedComponent::handleJobArrivalEvent(Event *ev) {
    ...
    if (nullptr != commEvent) {
        ...
        if (commEvent->CommType == LOG_JOB_START) {
            ...
            while (startingNewJob) {
                ...
                if (nextJob != nullptr) {
                    ...
                    if (doDetailedNetworkSim && !newJobsStarted && !emberRunningJobs.empty() &&
                        getCurrentSimTime() == ignoreUntilTime) {
        ...
        startNextJob();
    } else if (nullptr != sev) { //snapshot event
...
void schedComponent::startJob(Job *job) {
    ...
    if (timePerDistance->at(0) != 0 && nullptr == sMachine && nullptr != ai) {
        ...
        baselineRunningTime = actualRunningTime / (1 + timePerDistance->at(0) *
                                                       pow(baselineHopBytes,
                                                           timePerDistance->at(1) + randomNumber));

        //get new hopBytes
        double hopBytes = tmi->getHopBytes();
        double timeWithComm = baselineRunningTime * (1 + timePerDistance->at(0) * pow(hopBytes,
                                                                                      timePerDistance->at(
                                                                                          1) +
                                                                                      randomNumber));
    ...
    if (actualRunningTime > job->getEstimatedRunningTime()) {
        schedout.fatal(CALL_INFO, 1,
                       "Job %lu has running time %lu, which is longer than estimated running time %lu\n",
...
void schedComponent::logJobStart(ITMI itmi) {
    ...
    if (jobLog.is_open()) {
        ...
        schedout.fatal(CALL_INFO, 1, "Couldn't open %s for writing the job log.",
                       this->jobLogFileName.c_str());
    }
}
...
void schedComponent::logJobFinish(ITMI itmi) {
    ...
    if (jobLog.is_open()) {
        jobLog << *itmi.tmi->job->getID() << "," << itmi.tmi->job->getStartTime() << ","
               << getCurrentSimTime() << ",0,";
        for (int counter = 0; counter < itmi.tmi->job->getProcsNeeded(); counter++) {
            if (counter > 0) {
                jobLog << " ";
    ...
    if (getCurrentSimTime() < itmi.tmi->job->getStartTime()) {
        schedout.fatal(CALL_INFO, 1,
                       "Job begin time larger than end time: jobid=%s, begin=%lu, end=%"
        PRIu64
