...
void Machine::allocate(TaskMapInfo *taskMapInfo) {
    ...
    if (numAvail < nodeCount) {
        schedout.fatal(CALL_INFO, 1, "Attempted to allocate %d nodes when only %d are available",
                       nodeCount, numAvail);
    } else {
        numAvail -= nodeCount;
    }
    ...
    for (int i = 0; i < nodeCount; i++) {
        if (!freeNodes[allocInfo->nodeIndices[i]]) {
            schedout.fatal(CALL_INFO, 0, "Attempted to allocate job %ld to a busy node: ",
                           allocInfo->job->getJobNum());
        }
    ...
    for (std::map<unsigned int, double>::iterator it = jobTraffic.begin();
         it != jobTraffic.end(); it++) {
...
void Machine::deallocate(TaskMapInfo *taskMapInfo) {
    ...
    if (nodeCount > (numNodes - numAvail)) {
        schedout.fatal(CALL_INFO, 1, "Attempted to deallocate %d nodes when only %d are busy",
                       nodeCount, (numNodes - numAvail));
    } else {
        numAvail += nodeCount;
    }
    ...
    for (int i = 0; i < nodeCount; i++) {
        if (freeNodes[allocInfo->nodeIndices[i]]) {
            schedout.fatal(CALL_INFO, 0, "Attempted to deallocate job %ld from an idle node: ",
                           allocInfo->job->getJobNum());
        }
