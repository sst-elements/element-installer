...
ConstraintAllocator::ConstraintAllocator(SimpleMachine *m, std::string DepsFile,
                                         std::string ConstFile, schedComponent *sc)
    : Allocator(*m) {
...
//returns info on the allocation or nullptr if it wasn't possible
AllocInfo *ConstraintAllocator::allocate(Job *job) {
    ...
    if ((unsigned) ceil((double) job->getProcsNeeded() / machine.coresPerNode) <=
        freeNodes->size()) {
        ...
        ConstrainedAllocation *top_allocation = nullptr;

        int i = 1;
        for (std::list < std::vector < std::string > * >
             ::iterator constraint_iter = constraints.begin();
            constraint_iter != constraints.end();
        ++constraint_iter ){
            if (DEBUG)
                std::cout << "Attempting constraint " << i
                          << " for jobid " << *(job->getID()) << std::endl;
            top_allocation = allocate_constrained(job, *constraint_iter);
            if (top_allocation != nullptr) {
                if (DEBUG)
                    std::cout << " SUCCESS in satisfying constraint " << i
                              << " for jobid " << *(job->getID()) << std::endl;
        ...
        if (top_allocation != nullptr) {
            ...
            if (DEBUG)
                std::cout << " FAILED to satisfy any constraint"
                          << " for jobid " << *(job->getID()) << std::endl;
...
AllocInfo *ConstraintAllocator::generate_AllocInfo(ConstrainedAllocation *constrained_alloc) {
    ...
    for (std::set<int>::iterator unconstrained_node_iter = constrained_alloc->unconstrained_nodes.begin();
         unconstrained_node_iter !=
         constrained_alloc->unconstrained_nodes.end(); ++unconstrained_node_iter) {
    ...
    for (std::set<int>::iterator constrained_node_iter = constrained_alloc->constrained_nodes.begin();
         constrained_node_iter !=
         constrained_alloc->constrained_nodes.end(); ++constrained_node_iter) {
...
std::set <std::string> *ConstraintAllocator::get_constrained_leaves(
...
// returns an allocation satisifying the given constraint, or nullptr if it can not be satisifed 
// satisfied means: at least one constrained node used and one constraint node avoided
ConstrainedAllocation *ConstraintAllocator::allocate_constrained(Job *job,
                                                                 std::vector <std::string> *nodes_on_constraint_line) {
    ...
    std::list < std::vector < int > * > *constrained_compute_nodes =
    ...
    for (std::vector<std::string>::iterator constraint_node = nodes_on_constraint_line->begin();
        ...
        std::set <std::string> *dependent_compute_node_IDs = this->get_constrained_leaves(
            *constraint_node);
        ...
        std::vector<int> *new_unconstrained_compute_nodes = new std::vector<int>(
            unconstrained_compute_nodes->size());
        ...
        new_unconstrained_compute_nodes->resize(
            unconstrained_iter - new_unconstrained_compute_nodes->begin());
    ...
    for (std::list < std::vector < int > * >
         ::iterator constraint_node = constrained_compute_nodes->begin();
        constraint_node != constrained_compute_nodes->end();
    ++constraint_node ){
        for (std::vector<int>::reverse_iterator constrained_node = (*constraint_node)->rbegin();
             (constrained_node != (*constraint_node)->rend()) &&
             ((new_allocation->constrained_nodes.size() +
               new_allocation->unconstrained_nodes.size()) < numNodes); ++constrained_node) {
    ...
    for (std::vector<int>::reverse_iterator unconstrained_node = unconstrained_compute_nodes->rbegin();
         unconstrained_node != unconstrained_compute_nodes->rend() &&
         ((new_allocation->constrained_nodes.size() + new_allocation->unconstrained_nodes.size()) <
          numNodes); ++unconstrained_node) {
...
bool ConstraintAllocator::try_to_remove_constraint_set(unsigned int num_constrained_needed,
                                                       std::list<std::vector < int> *

> * constrained_compute_nodes ){
std::vector<int> *all_nodes = new std::vector<int>();

if (constrained_compute_nodes->

size()

== 1 ) { // must use at least one constrained compute node
return false;
}
...
for( std::list<std::vector < int> *>
::iterator constraint_node = constrained_compute_nodes->begin();
constraint_node != constrained_compute_nodes->
...
== 0) {  // remove all zero-size sets
zero_size_sets = true;
if (DEBUG) std::cout << " Removing zero-size set" <<
std::endl;
constraint_node = constrained_compute_nodes->erase(constraint_node);
}
else {
std::vector<int> *tmp_all_nodes = new std::vector<int>(
...
== 0) { // must use at least one constrained compute node
return false;
}
...
constrained_compute_nodes->
erase( constraint_node ); // remove this constraint node
// and its compute nodes from the other sets
for( std::list<std::vector < int> *>
::iterator inner_constraint_node = constrained_compute_nodes->begin();
inner_constraint_node != constrained_compute_nodes->

end();

++ inner_constraint_node ){
...
std::sort( tmp_constraint
->

begin(), tmp_constraint
...
std::list<std::vector < int> *> *
deep_copy_set_list( std::list<std::vector < int>
*> * list ){
std::list<std::vector < int> *> *
new_list = new std::list < std::vector < int > * > ();
for( std::list<std::vector < int> *>
