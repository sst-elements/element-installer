...
vector<MeshLocation *> *IntersectionCenterGen::getCenters(vector<MeshLocation *> *available) {
    ...
    //no duplicate values
    for (vector<MeshLocation *>::iterator loc = available->begin();
         loc != available->end(); ++loc) {
    ...
    for (vector<int>::iterator ind_x = X.begin();
         ind_x != X.end(); ind_x++) { //ind_x is x val index
...
std::pair<long, long> *PairwiseL1DistScorer::valueOf(MeshLocation *center,
...
long Tiebreaker::getTiebreak(MeshLocation *center, vector<MeshLocation *> *avail,
                             StencilMachine *mesh) {
    int num = avail->size();
    long ret = 0;
...
string Tiebreaker::getInfo() {
    stringstream ret;
    ret << "(" << maxshells << "," << availFactor << "," << wallFactor << "," << borderFactor << ","
        << curveFactor << "," << curveWidth << ")";
    return ret.str();
}


pair<long, long> *LInfDistFromCenterScorer::valueOf(MeshLocation *center,
                                                    vector<MeshLocation *> *procs,
                                                    StencilMachine *mach) {
...
pair<long, long> *L1DistFromCenterScorer::valueOf(MeshLocation *center,
                                                  vector<MeshLocation *> *procs,
                                                  StencilMachine *mach) {
