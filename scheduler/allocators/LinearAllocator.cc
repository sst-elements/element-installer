...
int LinearAllocator::MeshLocationOrdering::valtox(int x, int xoff, int y, int yoff, int rotate,
                                                  int mirror) {
    switch (rotate) {
        case 0:
            return x + mirror * xoff;
        case 1:
            return x - mirror * yoff;
        case 2:
            return x - mirror * xoff;
        case 3:
            return x + mirror * yoff;
        ...
        default:
            schedout.fatal(CALL_INFO, 1, "rotate value too large in LinearAllocator.cc: %d",
                           rotate);
            return 0;
    }
}

int LinearAllocator::MeshLocationOrdering::valtoy(int x, int xoff, int y, int yoff, int rotate,
                                                  int mirror) {
    switch (rotate) {
        case 0:
            return y + mirror * yoff;
        case 1:
            return y - mirror * xoff;
        case 2:
            return y - mirror * yoff;
        case 3:
            return y + mirror * xoff;
...
LinearAllocator::MeshLocationOrdering::triple LinearAllocator::MeshLocationOrdering::addwithrotate(
    triple origin, int coordinate, int offset, int mirror, rotation rot) {
    triple dest;
    for (int x = 0; x < 3; x++) {
        if (((mirror & (1 << x)) != 0)) {
...
void LinearAllocator::MeshLocationOrdering::threedmark(triple curpoint, rotation *rot, int *rank,
                                                       int curdim, triple *next, int *mirrors,
                                                       int *inverse, int *rules) {
    ...
    if (1 == biginverse) {
        for (int point = 0; point < 8; point++) {
            ...
            localmirror = (int) (mirror.d[0] != 0) + 2 * (int) (mirror.d[1] != 0) +
                          4 * (int) (mirror.d[2] != 0); //don't want negative values
            ...
            //find next point
            curpoint2 = addwithrotate(curpoint, 0, 2 * curdim / 6 * inverse[tripletoint(curpoint)],
                                      mirrors[tripletoint(curpoint)], rot[tripletoint(curpoint)]);
            curpoint2 = addwithrotate(curpoint2, 2,
                                      -1 * curdim / 6 * inverse[tripletoint(curpoint)],
                                      mirrors[tripletoint(curpoint)], rot[tripletoint(curpoint)]);
            ...
            if (inverse[tripletoint(curpoint)] == 1) {
                curpoint2 = addwithrotate(curpoint2, 2, -1, mirrors[tripletoint(curpoint)],
                                          rot[tripletoint(curpoint)]);
            } else {
                curpoint2 = addwithrotate(curpoint2, 0, -1, mirrors[tripletoint(curpoint)],
                                          rot[tripletoint(curpoint)]);
            }
...
LinearAllocator::MeshLocationOrdering::MeshLocationOrdering(Machine *mach, bool SORT = false,
    ...
    //if (nullptr == m) error("Linear Allocators Require Mesh Machine");
    if (nullptr == m) schedout.fatal(CALL_INFO, 1, "Linear Allocators Require Mesh/torus Machine");
            ...
            while (curxd > 1 && curyd > 1) {
                if ((curxd % 2 != 0 && 1 != curxd) || (curyd % 2 != 0 && 1 != curyd))
                    schedout.fatal(CALL_INFO, 1,
                                   "Hilbert Curve requires dimensions to be powers of two currently");
                ...
                int point2 = valtox(curx, 0, cury, curyd / 2, rotate[point1], mirror[point1]) +
                             xdimh *
                             valtoy(curx, 0, cury, curyd / 2, rotate[point1], mirror[point1]);
                ...
                //point 3
                int point3 =
                    valtox(curx, curxd / 2, cury, curyd / 2, rotate[point1], mirror[point1]) +
                    xdimh *
                    valtoy(curx, curxd / 2, cury, curyd / 2, rotate[point1], mirror[point1]);
                ...
                //point 4
                int point4 =
                    valtox(curx, curxd - 1, cury, curyd / 2 - 1, rotate[point1], mirror[point1]) +
                    xdimh *
                    valtoy(curx, curxd - 1, cury, curyd / 2 - 1, rotate[point1], mirror[point1]);
                ...
                if (0 == rotate[point1] % 2) {
                    if (valtoy(curx, curxd / 2, cury, curyd / 2, rotate[point1], mirror[point1]) ==
                        ydim) {
                        rankh[point4] = 0; //undo our old point 4
                        point4 = valtox(curx, curxd / 2, cury, 0, rotate[point1], mirror[point1]) +
                                 xdimh *
                                 valtoy(curx, curxd / 2, cury, 0, rotate[point1], mirror[point1]);
                        rankh[point4] = rankh[point1] + 3 * size;
                    ...
                    if (valtox(curx, curxd / 2, cury, curyd / 2, rotate[point1], mirror[point1]) ==
                        xdim) {
                        rankh[point4] = 0; //undo our old point 4
                        point4 = valtox(curx, curxd / 2, cury, 0, rotate[point1], mirror[point1]) +
                                 xdimh *
            ...
            for (int x = 0; x < realxdim; x++) {
                for (int y = 0; y < realydim; y++) {
                    for (int z = 0; z < realzdim; z++) {
                        rank[x + y * realxdim + z * realydim * realxdim] = rankh[x + y * xdim +
                                                                                 z * ydim * xdim];
                    }
                }
            }
            ...
            for (int rankin = 0; rankin < xdim * ydim * zdim; rankin++) {
                ...
                for (int index = 0; index < xdim * ydim * zdim; index++) {
                    if (rank[index] == rankin) {
                        ...
                        schedout.debug(CALL_INFO, 4, 0, "(%d,%d,%d),  ", index % xdim,
                                       (index % (xdim * ydim)) / xdim, index / (xdim * ydim));
    ...
    switch (params->size()) {
        ...
        case 1:
            if ("sort" == params->at(0)) {
                ...
                schedout.fatal(CALL_INFO, 1,
                               "Argument to Linear Allocator must be sort, nosort, hilbert, or snake:%s",
                               params->at(0).c_str());
            }
            break;
        case 2:
            if ("sort" == params->at(0)) {
                ...
                schedout.fatal(CALL_INFO, 1,
                               "First argument to Linear Allocator must be sort or nosort:%s",
                               params->at(0).c_str());
            }
            if ("hilbert" == params->at(1)) {
                ...
                schedout.fatal(CALL_INFO, 1,
                               "Second argument to Linear Allocator must be hilbert or snake:%s",
                               params->at(1).c_str());
            }
            break;
    }
...
vector<vector < MeshLocation * >*>*

LinearAllocator::getIntervals() {
    set < MeshLocation * ,
        MeshLocationOrdering > *avail = new set<MeshLocation *, MeshLocationOrdering>(*ordering);
    ...
    for (set<MeshLocation *, MeshLocationOrdering>::iterator ml = avail->begin();
         ml != avail->end(); ml++) {
...
AllocInfo *LinearAllocator::minSpanAllocate(Job *job) {
    ...
    int bestSpan =
        ordering->rankOf(avail->at(num - 1)) - ordering->rankOf(avail->at(0));//best location's span
