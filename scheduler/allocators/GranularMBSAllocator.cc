...
GranularMBSAllocator::GranularMBSAllocator(std::vector <std::string> *params, Machine *mach)
    : MBSAllocator(mach) {
            ...
            while (blocks->size() > 0 && it != blocks->end()) {
                ...
                if (toRemove->count(first) != 0) {
                    ...
                    if (blocks->count(second) != 0) {
                        ...
                        blocks->erase(
                            first); //it's slower to erase by value instead of using the iterator but we can't corrupt it (it's already been incremented anyway for the same reason)
...
Block *GranularMBSAllocator::nextBlock(int d, Block *first) {
    ...
    //error("nextBlock returning nullptr!");
    schedout.fatal(CALL_INFO, 1, "nextBlock returning nullptr!");
    return nullptr;
}
...
Block *GranularMBSAllocator::lookX(Block *b) {
    ...
    return new Block(new MeshLocation(tempVec),
                     b->dimension); //TODO:  do we want to copy the dimensio as well?
}
...
/**
 * Attempts to perform a get operation on a std::set. Returns nullptr if the block is not found
...
Block *GranularMBSAllocator::FBRGet(Block *needle) {
    ...
    std::set < Block * , Block > *haystack = FBR->at(
        distance(ordering->begin(), find(ordering->begin(), ordering->end(), needle->size())));
    ...
    if (it == haystack->end()) {
        ...
        schedout.fatal(CALL_INFO, 1,
                       "no blocks of correct rank in FBR when searching for nextBlock");
        return nullptr;
    }
    retVal = *it;
