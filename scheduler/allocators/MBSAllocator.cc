...
void MBSAllocator::initialize(MeshLocation *dim, MeshLocation *off) {
    schedout.debug(CALL_INFO, 7, 0, "Initializing a %dx%dx%d region at %s\n", dim->dims[0],
                   dim->dims[1], dim->dims[2], off->toString().c_str());
...
int MBSAllocator::createRank(int size) {
    ...
    std::vector < std::set < Block * , Block > * > ::iterator
    FBRit = FBR->begin();
    i = 0;
...
void MBSAllocator::deallocate(AllocInfo *alloc) {
    ...
    if (nullptr == dynamic_cast<MBSMeshAllocInfo *>(alloc)) {
        schedout.fatal(CALL_INFO, 1,
                       "MBS allocator can only deallocate instances of MBSMeshAllocInfo");
    } else {
...
void MBSAllocator::unallocate(MBSMeshAllocInfo *info) {
    ...
    for (std::set<Block *, Block>::iterator b = info->blocks->begin();
         b != info->blocks->end(); b++) {
        int rank = distance(ordering->begin(),
                            find(ordering->begin(), ordering->end(), (*b)->size()));
...
void MBSAllocator::mergeBlock(Block *p) {
    ...
    for (std::set<Block *, Block>::iterator child = p->children->begin();
         child != p->children->end(); child++) {
        rank = distance(ordering->begin(),
                        find(ordering->begin(), ordering->end(), (*child)->size()));
