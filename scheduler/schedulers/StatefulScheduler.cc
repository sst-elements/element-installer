...
StatefulScheduler::StatefulScheduler(int numprocs, StatefulScheduler::JobComparator *comp,
                                     bool dummy, const Machine &inmach) : mach(inmach) {
...
StatefulScheduler::StatefulScheduler(int numprocs, StatefulScheduler::JobComparator *comp,
                                     int filltimes, const Machine &inmach) : mach(inmach) {
    schedout.init("", 8, ~0, Output::STDOUT);
    SCComparator *sccomp = new SCComparator();
    numProcs = numprocs;
    freeProcs = numprocs;
...
StatefulScheduler::StatefulScheduler(int numprocs, StatefulScheduler::JobComparator *comp,
                                     const Machine &inmach) : mach(inmach) {
    schedout.init("", 8, ~0, Output::STDOUT);
    SCComparator *sccomp = new SCComparator();
    numProcs = numprocs;
    freeProcs = numprocs;
...
StatefulScheduler::StatefulScheduler(int numprocs, StatefulScheduler::JobComparator *comp,
                                     int filltimes, bool dummy, const Machine &inmach) : mach(
    inmach) {
    schedout.init("", 8, ~0, Output::STDOUT);
    SCComparator *sccomp = new SCComparator();
    numProcs = numprocs;
...
StatefulScheduler::StatefulScheduler(StatefulScheduler *insched,
                                     set<SchedChange *, SCComparator> *inestSched, Manager *inheart,
                                     map<Job *, SchedChange *, StatefulScheduler::JobComparator> *inJobToEvents,
                                     const Machine &inmach) : mach(inmach) {
    schedout.init("", 8, ~0, Output::STDOUT);
    numProcs = insched->numProcs;
    freeProcs = insched->freeProcs;
    //if (nullptr == inestSched) {
    //SCComparator *sccomp = new SCComparator();
    //estSched = new set<SchedChange*, SCComparator>(*sccomp);
    //} else {
    estSched = inestSched;
    //}
    //if (nullptr == inJobToEvents) {
    //jobToEvents = new map<Job*, SchedChange*, StatefulScheduler::JobComparator>(*(inheart -> origcomp));
    //} else {
    jobToEvents = inJobToEvents;
    //}
    //if (nullptr == inheart) {
    //heart = insched -> heart -> copy(nullptr);
    //} else {
    heart = inheart;
...
StatefulScheduler *StatefulScheduler::copy(std::vector<Job *> *inrunning,
                                           std::vector<Job *> *intoRun) {
    //copy heart -> backfill if it exists (as well as anything else with job
    //pointers)

    SCComparator sccomp;
    std::set < SchedChange * ,
        SCComparator > *newestSched = new std::set<SchedChange *, SCComparator>(sccomp);
    map < Job * , SchedChange *, StatefulScheduler::JobComparator >
                                 *newJobToEvents = new map<Job *, SchedChange *, StatefulScheduler::JobComparator>(
        *(origcomp));

    for (set<SchedChange *, SCComparator>::iterator it = estSched->begin();
         it != estSched->end(); it++) {
        bool found = false;
        //jobs in estSched can be either running or can be waiting to be
        //scheduled, so we must check in both inrunning and intoRun
        for (vector<Job *>::iterator it2 = inrunning->begin();
             !found && it2 != inrunning->end(); it2++) {
        ...
        for (vector<Job *>::iterator it2 = intoRun->begin();
             !found && it2 != intoRun->end(); it2++) {
            if ((*it2)->getJobNum() == (*it)->j->getJobNum()) {
                SchedChange *tempsc = new SchedChange(*it, mach);
                tempsc->j = *it2;
                newestSched->insert(tempsc);
    ...
    for (set<SchedChange *, SCComparator>::iterator it = newestSched->begin();
         it != newestSched->end(); it++) {
        if (!(*it)->isEnd) {
            //find the corresponding SchedChange
            bool found = false;
            for (set<SchedChange *, SCComparator>::iterator it2 = it;
                 !found && it2 != newestSched->end(); it2++) {
            ...
            if (!found) {
                ...
                for (set<SchedChange *, SCComparator>::iterator it3 = newestSched->begin();
                     it3 != newestSched->end(); it3++) {
                    (*it3)->print();
                }
                schedout.output("===========================================\n");
                (*it)->print();
    ...
    for (map<Job *, SchedChange *, StatefulScheduler::JobComparator>::iterator it = jobToEvents->begin();
         it != jobToEvents->end(); it++) {
        bool found = false;
        for (vector<Job *>::iterator it2 = inrunning->begin();
             !found && it2 != inrunning->end(); it2++) {
...
SchedChange::SchedChange(unsigned long intime, Job *inj, bool end, const Machine &inmach,
                         SchedChange *inpartner) : mach(inmach) {
    if (nullptr == inpartner && !end)
        schedout.fatal(CALL_INFO, 1, "Schedchange beginning not given partner");
    partner = inpartner;
    time = intime;
    j = inj;
    isEnd = end;
...
unsigned long StatefulScheduler::scheduleJob(Job *j, unsigned long time) {
    ...
    SchedChange *endChange = new SchedChange(starttime + j->getEstimatedRunningTime(), j, true,
                                             mach, nullptr);
...
unsigned long StatefulScheduler::findTime(set<SchedChange *, SCComparator> *sched, Job *j,
                                          unsigned long time) {
    ...
    SchedChange *sc = nullptr;  //first change we haven't looked at yet
        ...
        if (it != sched->end()) {
            ...
            if (scTime <= anchorTime) {
                ...
                while (!done && (currentFree >= getNodesNeeded(*j))) {
                    ...
                    ++it;
                    if ((scTime >= anchorTime + j->getEstimatedRunningTime()) ||
                        it == sched->end()) {
                        --it;
            ...
            if (currentFree != numProcs) {
                printPlan();
                schedout.fatal(CALL_INFO, 1,
                               "Scheduler got to end of estimated schedule with %d processors free out of %d total\n",
...
unsigned long StatefulScheduler::zeroCase(set<SchedChange *, SCComparator> *sched, Job *filler,
                                          unsigned long time) {
    ...
    if (avaProcs >= getNodesNeeded(*filler)) {//check if enough procs
        ...
        if (avaProcs != numProcs) {
            printPlan();
            schedout.fatal(CALL_INFO, 1,
                           "Scheduler got to end of estimated schedule w/o all processors being free");
        }
    }
    return lookAtTime;
}
...
void StatefulScheduler::jobFinishes(Job *j, unsigned long time, const Machine &mach) {
    ...
    while (it != estSched->rbegin() && !success) {
        ...
        if (sc->j->getJobNum() == j->getJobNum()) {
            if (!sc->isEnd)
                schedout.fatal(CALL_INFO, 1, "Job finished before scheduler started it");
            success = true;
...
//returns first job to start, nullptr if none
//(if not nullptr, should call tryToStart again)
...
void StatefulScheduler::startNext(unsigned long time, const Machine &mach) {
    if (nextToStart == nullptr) {
        schedout.fatal(CALL_INFO, 1,
                       "Called startNext() job from scheduler when there is no available Job at time %lu",
                       time);
    } else if (nextToStartTime != time) {
        schedout.fatal(CALL_INFO, 1,
                       "startNext() and tryToStart() are called at different times for Job #%ld",
...
void StatefulScheduler::Manager::compress(unsigned long time) {
    set < SchedChange * ,
        SCComparator > *oldEstSched = new set<SchedChange *, SCComparator>(*(scheduler->estSched));
    SCComparator *sccomp = new SCComparator();
    ...
    for (set<SchedChange *, SCComparator>::iterator it = oldEstSched->begin();
        ...
        if (!sc->isEnd) {
            ...
            if (newStartTime > scTime)
                schedout.fatal(CALL_INFO, 1, "Attempt to delay estimated start of Job");
            delete sc;
        } else if (nullptr == sc->j) {
...
StatefulScheduler::PrioritizeCompressionManager::PrioritizeCompressionManager(
    StatefulScheduler *inscheduler, JobComparator *comp, int infillTimes, const Machine &inmach)
    : Manager(inmach) {
...
StatefulScheduler::PrioritizeCompressionManager::PrioritizeCompressionManager(
    PrioritizeCompressionManager *inmanager, set<Job *, JobComparator> *inbackfill,
    const Machine &inmach) : Manager(inmach) {
...
StatefulScheduler::PrioritizeCompressionManager *StatefulScheduler::PrioritizeCompressionManager::copy(
    std::vector<Job *> *running, std::vector<Job *> *intoRun) {
    ...
    for (set<Job *, JobComparator>::iterator it = backfill->begin(); it != backfill->end(); it++) {
        bool found = false;
        for (std::vector<Job *>::iterator it2 = intoRun->begin();
             !found && it2 != intoRun->end(); it2++) {
    ...
    if (notfound > 1)
        schedout.fatal(CALL_INFO, 1,
                       "Prioritize Compression Manager could not find two jobs for its new backfill in copy()");
...
void StatefulScheduler::PrioritizeCompressionManager::done() {
    for (int i = 0; i < fillTimes; i++) {
        if (numSBF[i] != 0)
            schedout.verbose(CALL_INFO, 4, 0,
                             "backfilled successfully %d times in a row %d times\n", i, numSBF[i]);
    }
}
...
void StatefulScheduler::PrioritizeCompressionManager::earlyFinish(Job *j, unsigned long time) {
    ...
    for (times = 0; times < fillTimes; times++) {
        ...
        for (set<Job *, JobComparator>::iterator it = backfill->begin();
             it != backfill->end(); it++) {
            ...
            scheduler->jobToEvents->erase(
                *it); // otherwise there are multiple pais for the job and the wrong one may be used
            //try to backfill job
            newTime = scheduler->scheduleJob(*it, time);
            if (newTime > oldTime) { //not supposed to happen
                for (set<SchedChange *, SCComparator>::iterator sc = scheduler->estSched->begin();
                     sc != scheduler->estSched->end(); sc++) {
                    (*sc)->print();
                }
                schedout.verbose(CALL_INFO, 4, 0, "old: %ld, new:%ld\nbackfilling: %s\n", oldTime,
                                 newTime, (*it)->toString().c_str());
                oldStartTime->print();
                schedout.fatal(CALL_INFO, 1,
                               "PrioritizeCompression Backfilling gave a new reservation that was later than previous one");
            }
            delete oldStartTime;
...
StatefulScheduler::DelayedCompressionManager::DelayedCompressionManager(
    StatefulScheduler *inscheduler, JobComparator *comp, const Machine &inmach) : Manager(inmach) {
    scheduler = inscheduler;
    backfill = new set<Job *, JobComparator>(*comp);
    results = 0;
    origcomp = comp;
...
StatefulScheduler::DelayedCompressionManager::DelayedCompressionManager(
    DelayedCompressionManager *inmanager, set<Job *, JobComparator> *inbackfill,
    const Machine &inmach) : Manager(inmach) {
    scheduler = inmanager->scheduler;
    backfill = inbackfill;
...
StatefulScheduler::DelayedCompressionManager *StatefulScheduler::DelayedCompressionManager::copy(
    std::vector<Job *> *running, std::vector<Job *> *intoRun) {
    set < Job * , JobComparator > *newbackfill = new set<Job *, JobComparator>(*origcomp);
    int notfound = 0;
    for (set<Job *, JobComparator>::iterator it = backfill->begin(); it != backfill->end(); it++) {
    ...
    if (notfound > 1)
        schedout.fatal(CALL_INFO, 1,
                       "Delayed Compression Manager could not find two jobs for its new backfill in copy()");
...
void StatefulScheduler::DelayedCompressionManager::arrival(Job *j, unsigned long time) {
    ...
    for (set<Job *, JobComparator>::iterator it = backfill->begin();
         it != backfill->end(); it++) {//iterate through backfill queue
...
StatefulScheduler::EvenLessManager::EvenLessManager(StatefulScheduler *inscheduler,
                                                    JobComparator *comp, int fillTimes,
                                                    const Machine &inmach) : Manager(inmach) {
    SCComparator *sccomp = new SCComparator();
    scheduler = inscheduler;
    backfill = new set<Job *, JobComparator>(*comp);
...
StatefulScheduler::EvenLessManager::EvenLessManager(EvenLessManager *inmanager,
                                                    set<Job *, JobComparator> *inbackfill,
                                                    const Machine &inmach) : Manager(inmach) {
    //    set<Job*, JobComparator>* newbackfill = new set<Job*, JobComparator>(*origcomp);
    //   int notfound = 0;
    //   for (set<Job*, JobComparator>::iterator it = backfill -> begin(); it != backfill -> end(); it++) {
...
StatefulScheduler::EvenLessManager *StatefulScheduler::EvenLessManager::copy(
    std::vector<Job *> *running, std::vector<Job *> *intoRun) {
    set < Job * , JobComparator > *newbackfill = new set<Job *, JobComparator>(*origcomp);
    int notfound = 0;
    for (set<Job *, JobComparator>::iterator it = backfill->begin(); it != backfill->end(); it++) {
    ...
    if (notfound > 1)
        schedout.fatal(CALL_INFO, 1,
                       "Even Less Conservative Manager could not find two jobs for its new backfill in copy()");
    ...
    for (map<Job *, SchedChange *, JobComparator>::iterator it = guarJobToEvents->begin();
         it != guarJobToEvents->end(); it++) {
        bool found = false;
        //find the job in newbackfill
        for (set<Job *, JobComparator>::iterator it2 = newbackfill->begin();
             !found && it2 != newbackfill->end(); it2++) {
            if ((*it2)->getJobNum() == it->first->getJobNum()) {
                found = true;
        ...
        for (vector<Job *>::iterator it2 = running->begin();
             !found && it2 != running->end(); it2++) {
            if ((*it2)->getJobNum() == it->first->getJobNum()) {
                found = true;
                SchedChange *tempsched = new SchedChange(it->second, mach);
                tempsched->j = *it2;
        ...
        if (!found)
            schedout.fatal(CALL_INFO, 1, "Could not find %s in new backfill\n",
                           it->first->toString().c_str());
    }
    printPlan();
    ret->printPlan();
...
void StatefulScheduler::EvenLessManager::deepCopy(set<SchedChange *, SCComparator> *from,
                                                  set<SchedChange *, SCComparator> *to,
                                                  map<Job *, SchedChange *, JobComparator> *toJ) {
    ...
    for (set<SchedChange *, SCComparator>::iterator sc = from->begin(); sc != from->end(); sc++) {
        if (!((*sc)->isEnd)) {
            ...
            if (toJ->find((*sc)->j) ==
                toJ->end()) { //there is no entry for j in toJ; the job is running
                SchedChange *je = new SchedChange((*sc)->getTime(), (*sc)->j, true, mach, nullptr);
                to->insert(je);
...
void StatefulScheduler::EvenLessManager::backfillfunc(unsigned long time) {
    for (int i = 0; i < bftimes; i++) {
        for (set<Job *, JobComparator>::iterator it = backfill->begin();
            ...
            set<SchedChange *, SCComparator>::iterator upper = scheduler->estSched->upper_bound(
                js->getPartner());
            --upper;
            ...
            SchedChange *je = new SchedChange(start + (*it)->getEstimatedRunningTime(), (*it), true,
                                              mach, nullptr);
            ...
            if (start == time && old != time) {
                ...
                //String reason = "";
                for (set<SchedChange *, SCComparator>::iterator sc = guarantee->begin();
                     sc != guarantee->end(); sc++) {
                    ...
                    if (last != (*sc)->getTime() && freeprocs < 0) {
                        ...
                        schedout.debug(CALL_INFO, 4, 0, "\nguarantee:\n");
                        for (set<SchedChange *, SCComparator>::iterator sc3 = guarantee->begin();
                             sc3 != guarantee->end(); sc3++) {
                        ...
                        schedout.debug(CALL_INFO, 4, 0, "\nestSched:\n");
                        for (set<SchedChange *, SCComparator>::iterator sc3 = scheduler->estSched->begin();
                             sc3 != scheduler->estSched->end(); sc3++) {
                            tempfreeprocs += (*sc3)->freeProcChange();
                            schedout.debug(CALL_INFO, 4, 0, "%d |", tempfreeprocs);
                            (*sc3)->print();
                        }
            ...
            if (start < old) {
                ...
                schedout.debug(CALL_INFO, 7, 0, "%lu: backfilled %s to %lu from %lu\n", time,
                               (*it)->toString().c_str(), start, old);
                break;
            } else if (start > old) {
                schedout.output("%lu: Backfilling error, plan:\n", time);
                scheduler->printPlan();
                schedout.fatal(CALL_INFO, 1,
                               "ELC gave a worse start time to %s. Old: %lu, New: %lu\n",
...
void StatefulScheduler::EvenLessManager::arrival(Job *j, unsigned long time) {
    ...
    SchedChange *je = new SchedChange(gtime + j->getEstimatedRunningTime(), j, true, mach, nullptr);
...
void StatefulScheduler::EvenLessManager::onTimeFinish(Job *j, unsigned long time) {
    ...
    SchedChange *sc = guarJobToEvents->find(j)->second;
    if (sc == nullptr)
        schedout.fatal(CALL_INFO, 1, "SchedChange for %s is %p", j->toString().c_str(), sc);
    guarantee->erase(sc);
    guarJobToEvents->erase(j);
}
...
void StatefulScheduler::EvenLessManager::start(Job *j, unsigned long time) {
    ...
    for (set<SchedChange *, SCComparator>::iterator sc = guarantee->begin();
         sc != guarantee->end();) {
        //If this is a new time from the last time
        if (last != (*sc)->getTime() && freeprocs <
                                        0) { //the last test is because a negative (temporarily) is OK if they're all at the same time; and any needed processors are immediately released.
            destroyed = true;
    ...
    if (destroyed || freeprocs < 0 || freeprocs != scheduler->numProcs) {
        ...
        if (freeprocs != scheduler->numProcs)
            schedout.debug(CALL_INFO, 4, 0, "All procs not freed\n");
        scheduler->printPlan();

        schedout.debug(CALL_INFO, 4, 0,
                       "the schedule is impossible, using estimated schedule instead\n");
