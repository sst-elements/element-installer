...
EASYScheduler::EASYScheduler(JobComparator *comp) {
    ...
    running = new multiset<RunningInfo *, RunningInfo>(
        *RIComp); //don't need to pass comp because compare longs
    delete RIComp;
...
EASYScheduler::EASYScheduler(EASYScheduler *insched, std::set<Job *, JobComparator> *newtoRun,
                             std::multiset<RunningInfo *, RunningInfo> *newrunning) {
...
void EASYScheduler::jobFinishes(Job *j, unsigned long time, const Machine &mach) {
    ...
    if (!success)
        schedout.fatal(CALL_INFO, 1, "Could not find finishing job in running list\n%s\n",
                       j->toString().c_str());
    giveGuarantee(time, mach);
}
...
Job *EASYScheduler::tryToStart(unsigned long time, const Machine &mach) {
    ...
    if (!running->empty() && (*(running->begin()))->estComp == time) {
        return nullptr;  //don't backfill if another job is about to finish
    }
    if (toRun->empty()) return nullptr;

    set<Job *, JobComparator>::iterator job = toRun->begin();
    if (time > guaranteedStart) {
        schedout.fatal(CALL_INFO, 1,
                       "Failed to start job #%ld at guaranteed time \nTime: %lu Guarantee: %lu\n",
    ...
    while (!succeeded && job != toRun->end()) {
        ...
        if (availNodes >= nodesNeeded) {
            ...
            if (time + (*job)->getEstimatedRunningTime() <= guaranteedStart) {
                ...
                if (avail - nodesNeeded >=
                    ceil(((float) (*tempit)->getProcsNeeded()) / mach.coresPerNode)) {
                    succeeded = true;
                }
            }
        }
...
void EASYScheduler::giveGuarantee(unsigned long time, const Machine &mach) {
    ...
    if (succeeded) {
        schedout.debug(CALL_INFO, 7, 0, "%ld: Giving %s guarantee of time %ld\n", time,
                       (*firstJob)->toString().c_str(), guaranteedStart);
        if ((*firstJob)->getJobNum() == prevFirstJobNum && lastGuarantee + 1 < guaranteedStart &&
            lastGuarantee > 0) {
            schedout.output("EASY scheduler gave new guarantee worse than previous\n");
            schedout.output("Time: %lu\n", time);
            schedout.output("Running: ");
            for (multiset<RunningInfo *, RunningInfo>::iterator it3 = running->begin();
                 it3 != running->end(); it3++) {
                schedout.output("%ld %d %ld \n", (*it3)->jobNum, (*it3)->numNodes, (*it3)->estComp);
            }
            schedout.output("toRun: ");
            for (set<Job *, JobComparator>::iterator it3 = toRun->begin();
                 it3 != toRun->end(); it3++) {
                schedout.output("%ld ", (*it3)->getJobNum());
            }
            schedout.output("\n");
            schedout.fatal(CALL_INFO, 1,
                           "last guarantee: %ld new guarantee %ld\n EASY scheduler gave new guarantee worse than previous\nFor %s",
                           lastGuarantee, guaranteedStart, (*firstJob)->toString().c_str());
...
EASYScheduler *EASYScheduler::copy(std::vector<Job *> *inrunning, std::vector<Job *> *intoRun) {
    ...
    set < Job * , JobComparator,
        std::allocator < Job * > > *newtoRun = new set < Job *, JobComparator, std::allocator <
                                                                               Job * > > (*comp);

    RunningInfo *RIComp = new RunningInfo();
    multiset < RunningInfo * ,
        RunningInfo > *newrunning = new multiset<RunningInfo *, RunningInfo>(*RIComp);
    delete RIComp;

    //copy running
    for (multiset<RunningInfo *, RunningInfo>::iterator it = running->begin();
         it != running->end(); it++) {
        newrunning->insert(new RunningInfo(*it));
    }

    //replace pointers in toRun
    for (set < Job * , JobComparator, std::allocator < Job * > > ::iterator it = toRun->begin();
        it != toRun->end();
    it++) {
        bool found = false;
...
string EASYScheduler::JobComparator::toString() {
    switch (type) {
        ...
        default:
            schedout.fatal(CALL_INFO, 1, "toString() called on JobComparator w/ invalid type %d",
                           (int) type);
    }
