...
void RCBTaskMapper::mapTaskHelper(Grouper <MeshLocation> *inLocs, Grouper<int> *inTasks,
                                  TaskMapInfo *tmi) {
...
void RCBTaskMapper::Grouper<T>::divideBy(int dim, Grouper <T> **first, Grouper <T> **second) {
    ...
    *first = new Grouper<T>(new vector<T>(elements->begin(), elements->begin() + halfSize),
                            rotator);
...
RCBTaskMapper::Dims RCBTaskMapper::Rotator::getDims(MeshLocation loc) const {
    Dims outDims;
    if (indMap == nullptr) { //support for Grouper initialization
