                     ...
                     bool *doDetailedNetworkSim) //NetworkSim: added bool parameter
{
    ...
    if (*(this->doDetailedNetworkSim)) {
        ...
        if (nullptr == dir) {
            ...
            completedJobTrace =
                outputDirectory + params.find<std::string>("completedJobsTrace").c_str();
            runningJobTrace =
                outputDirectory + params.find<std::string>("runningJobsTrace").c_str();
        }
    }
    //end->NetworkSim
        ...
        while (!input.eof()) {
            ...
            if (jobNum == -1 || runningTime == 0) {
                schedout.fatal(CALL_INFO, 1,
                               "Invalid values are assigned to jobNum or runningTime:\njobNum:%d runningTime:%lu\n",
                               jobNum, runningTime);
            }
        ...
        while (!input.eof()) {
            ...
            if (!isTimeLine) {
                if (jobNum == -1 || soFarRunningTime == 0 || currentMotifCount == -1) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Invalid values are assigned to jobNum or soFarRunningTime or currentMotifCount:\njobNum:%d soFarRunningTime:%lu currentMotifCount:%d\n",
...
bool JobParser::newYumYumJobLine(std::string line, SimTime_t currSimTime) {
    ...
    sscanf(tokens.at(1).c_str(), "%"
    PRIu64, &duration );
    sscanf(tokens.at(2).c_str(), "%"
    PRIu64, &procs );
    ...
    if (tokens.size() != 3) {
        ...
        if (useYumYumTraceFormat) {
            jobs.push_back(
                new Job(currSimTime + 1, procs, duration, duration + 1, std::string(ID)));
        } else {
...
bool JobParser::newJobLine(std::string line) {
        ...
        while (!nextStr.empty()) {
            if (nextStr.compare("mesh") == 0) {
                ...
                if (x * y * z != procsNeeded) {
                    schedout.fatal(CALL_INFO, 1,
                                   "The communication mesh structure does not match the number of processors in line:\n\"%s\"\n",
                                   line.c_str());
                } else {
    ...
    if (*doDetailedNetworkSim == true && phaseInfo.phaseFile.empty()) {
        schedout.fatal(CALL_INFO, 1,
                       "Phase file must be defined when using DETAILED NETWORK SIM mode:\n\"%s\"\n",
                       line.c_str());
    }
    //end->NetworkSim
    ...
    jobs.push_back(new Job(arrivalTime, procsNeeded, runningTime, estRunningTime, commInfo,
                           phaseInfo)); //NetworkSim: added phase info
...
bool JobParser::validateJob(Job *j, vector<Job *> *jobs, long runningTime) {
    ...
    if (ok && runningTime < 0) {  //time 0 also strange, but perhaps rounded down     
        schedout.verbose(CALL_INFO, 0, 0,
                         "Warning: Job %ld  has running time of %ld; ignoring it\n",
    ...
    if (ok && j->getProcsNeeded() > (machine->numNodes * machine->coresPerNode)) {
        schedout.fatal(CALL_INFO, 1,
                       ...
                       j->getJobNum(), j->getProcsNeeded(),
                       (machine->numNodes * machine->coresPerNode));
        ok = false;
    }
...
std::vector <std::map<int, int>> *CommParser::readCommFile(std::string fileName, int procsNeeded) {
    ...
    if (reader.xdim != reader.ydim) {
        ...
        schedout.fatal(CALL_INFO, 1,
                       "The size of the matrix in file %s does not match with the job size\n",
                       fileName.c_str());
    }
...
double **CommParser::readCoordFile(std::string fileName, int procsNeeded) {
    ...
    if (reader.ydim != 2 && reader.ydim != 3) {
        schedout.fatal(CALL_INFO, 1,
                       "Coordinates matrix in file %s has a dimension other than 2 or 3.\n",
                       fileName.c_str());
    }
        ...
        for (int j = 0; j < ydim; j++) {
            for (int i = 0; i < xdim; i++) {
                ...
                if (!(inputFile >> tempData[j])) {
                    schedout.fatal(CALL_INFO, 1,
                                   "Number of lines in file %s does not match with the matrix.\n",
                                   fileName);
                }
            }
        }
