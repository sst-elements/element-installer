...
TaskCommInfo::TaskCommInfo(Job *job, std::vector <std::map<int, int>> *inCommInfo,
                           double **inCoords, int inCenterTask)
    : xdim(0), ydim(0), zdim(0), centerTask(inCenterTask) {
    init(job);
...
TaskCommInfo::TaskCommInfo(const TaskCommInfo &tci)
    ...
    if (taskCommType == CUSTOM || taskCommType == COORDINATE) {
        ...
        for (unsigned int i = 0; i < size; i++) {
            for (std::map<int, int>::iterator it = tci.commInfo->at(i).begin();
                 it != tci.commInfo->at(i).end(); it++) {
...
int **TaskCommInfo::buildCustomMatrix() const {
    ...
    for (unsigned int taskIt = 0; taskIt < commInfo->at(0).size(); ++taskIt) {
        for (std::map<int, int>::iterator it = commInfo->at(taskIt).begin();
             it != commInfo->at(taskIt).end(); it++) {
