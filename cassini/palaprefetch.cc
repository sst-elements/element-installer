...
void PalaPrefetcher::DispatchRequest(Addr targetAddress) {
    ...
    if (overrunPageBoundary) {
        ...
        ev = new MemEvent(getName(), targetAddress + (strideReach * stride),
                          targetAddress + (strideReach * stride), Command::GetS,
...
PalaPrefetcher::PalaPrefetcher(ComponentId_t id, Params &params) : CacheListener(id, params) {
    ...
    output->verbose(CALL_INFO, 1, 0, "PalaPrefetcher created, cache line: %"
    PRIu64
    ", page size: %"
    PRIu64
    "\n",
