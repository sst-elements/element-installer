...
libcacheTracer.so: $(OBJ)
	$(CXX) $(CXXFLAGS) -I$(memHierarchy) $(LDFLAGS) -o $@ $^ -L$(memHierarchy) -lmemHierarchy

install: libcacheTracer.so
	sst-register cacheTracer cacheTracer_LIBDIR=$(CURDIR)

uninstall:
	sst-register -u cacheTracer

clean: uninstall
	rm -rf .build libcacheTracer.so
