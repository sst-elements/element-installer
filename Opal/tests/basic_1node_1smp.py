...
for next_core in range(cores):
    ...
    if next_core < cores / 2:
        ...
        ArielOpalLink.connect((memmgr, "opal_link_%d" % next_core, "300ps"),
                              (opal, "requestLink%d" % (2 * next_core), "300ps"))
        MMUCacheLink.connect((mmu, "mmu_to_cache%d" % next_core, "300ps"), (l1, "high_network_0", "300ps"))
        PTWOpalLink.connect((mmu, "ptw_to_opal%d" % next_core, "300ps"),
                            (opal, "requestLink%d" % (2 * next_core + 1), "300ps"))
    else:
    ...
    l2_ring_link.connect((l2, "cache", "300ps"),
                         (internal_network.rtr, "port%d" % (internal_network.getNextPort()), "300ps"))
...
l3_ring_link.connect((l3cache, "directory", "300ps"),
                     (internal_network.rtr, "port%d" % (internal_network.getNextPort()), "300ps"))
