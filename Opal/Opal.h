...
namespace SST {
    namespace OpalComponent {
        ...
        public:
            ...
            void processShootdownEvent(int node, int coreId, uint64_t vaddress, uint64_t paddress,
                                       int fault_level);
            ...
            {
                "requestLink%(num_ports)d", "Link to receive allocation requests", {
                    "OpalComponent.OpalEvent", ""}
            },
            {
                "shootdownLink%(num_nodes*num_cores)d", "Link to send shootdown requests to Samba units", {
                    "OpalComponent.OpalEvent", ""}
            },
            )
        ...
        private:
            ...
            std::map<int, std::pair < std::list < int>*, std::list <uint64_t>* > >
            mmapFileIdHints;
        ...
        public:
            ...
            std::list <std::pair<uint64_t, std::pair < uint64_t, int>> >
            invalidAddrs;

            void addInvalidAddress(uint64_t pAddress, uint64_t vAddress, int fault_level) {
                invalidAddrs.push_back(
                    std::make_pair(pAddress, std::make_pair(vAddress, fault_level)));
            }

            void setInvalidAddresses(std::list <std::pair<uint64_t, std::pair < uint64_t, int>>

            > ia) { invalidAddrs = ia; }
            std::list <std::pair<uint64_t, std::pair < uint64_t, int>> > *

            getInvalidAddresses() { return &invalidAddrs; }
        ...
        public:
            NodePrivateInfo(Opal *_owner, uint32_t node, Params params) {
                ...
                page_migration_policy = (uint32_t) params.find<uint32_t>("page_migration_policy",
                                                                         0);
                num_pages_to_migrate = (uint32_t) params.find<uint32_t>("num_pages_to_migrate", 0);
                nextallocmem = 0;
                allocatedmempool = 0;
                pool = new Pool(owner, (Params) params.find_prefix_params("memory."),
                ...
                std::cerr << "Node: " << node_num << " Allocation policy: "
                          << memoryAllocationPolicy << std::endl;
            ...
            void insertFrame(int coreId, uint64_t pAddress, uint64_t vAddress, int fault_level,
