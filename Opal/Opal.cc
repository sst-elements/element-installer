...
//    primaryComponentDoNotEndSim();
    ...
    for (uint32_t i = 0; i < num_nodes; i++) {
        ...
        for (uint32_t j = 0; j < nodeInfo[i]->cores; j++) {
            memset(buffer, 0, 256);
            sprintf(buffer, "requestLink%"
            PRIu32, linksCount + j * 2);
            nodeInfo[i]->coreInfo[j].coreLink = configureLink(buffer, "1ns",
                                                              new Event::Handler<CorePrivateInfo>(
                                                                  (&nodeInfo[i]->coreInfo[j]),
                                                                  &CorePrivateInfo::handleRequest));
            memset(buffer, 0, 256);
            sprintf(buffer, "requestLink%"
            PRIu32, linksCount + (j * 2) + 1);
            nodeInfo[i]->coreInfo[j].mmuLink = configureLink(buffer, "1ns",
                                                             new Event::Handler<CorePrivateInfo>(
                                                                 (&nodeInfo[i]->coreInfo[j]),
                                                                 &CorePrivateInfo::handleRequest));
...
void Opal::setNextMemPool(int node) {
    switch (nodeInfo[node]->memoryAllocationPolicy) {
        ...
        case 3:
            ...
            nodeInfo[node]->nextallocmem = (nodeInfo[node]->nextallocmem + 1) %
...
void Opal::processHint(int node, int fileId, uint64_t vAddress, int size) {
    ...
    std::map < int, std::pair < std::list < int > *, std::list < uint64_t > * > > ::iterator
    fileIdHint = mmapFileIdHints.find(fileId);
    ...
    if (fileIdHint != mmapFileIdHints.end()) {
        ...
        if (it1 != it->end()) {
            std::cerr << "Memory is already allocated for fileId: " << fileIdHint->first
                      << " in the same node" << std::endl;
        } else {
            int owner_node = it->back();
            if (nodeInfo[owner_node]->page_size != nodeInfo[node]->page_size)
                output->fatal(CALL_INFO, -1,
                              "Opal: Page sizes of the owner node which reserved space with fileId: %d and the requesting node are not same\n",
                              fileId);

            it->push_back(node);
            //(fileIdHint->second).first = it;
            nodeInfo[node]->reservedSpace.insert(std::make_pair(vAddress / 4096,
                                                                std::make_pair(fileId,
                                                                               std::make_pair(ceil(
                                                                                   size /
                                                                                   (nodeInfo[node]->page_size *
                                                                                    1024)), 0))));
        }
    } else {
        ...
        nodeInfo[node]->reservedSpace.insert(std::make_pair(vAddress / 4096, std::make_pair(fileId,
                                                                                            std::make_pair(
                                                                                                ceil(
                                                                                                    size /
                                                                                                    (nodeInfo[node]->page_size *
...
REQRESPONSE Opal::isAddressReserved(int node, uint64_t vAddress) {
    ...
    for (std::map < uint64_t, std::pair < int, std::pair < int, int > > >
                                                                ::iterator it = (nodeInfo[node]->reservedSpace).begin();
        it != (nodeInfo[node]->reservedSpace).end();
    ++it)
    {
        uint64_t reservedVAddress = it->first;
        int pages_reserved = (it->second).second.first;
        if (reservedVAddress <= vAddress &&
            vAddress < reservedVAddress + pages_reserved * nodeInfo[node]->page_size * 1024) {
            response.status = 1;
...
REQRESPONSE Opal::allocateSharedMemory(int node, int coreId, uint64_t vAddress, int fault_level,
                                       int pages) {
    REQRESPONSE response;
    response.status = 0;
    ...
    if (nodeInfo[node]->memoryAllocationPolicy) {
        ...
        if (sharedMemoryInfo[sharedMemPoolId]->available_frames >= pages) {
            ...
            for (int j = 0; j < pages; j++) {
                ...
                nodeInfo[node]->insertFrame(coreId, response.address, vAddress, fault_level,
                                            SST::OpalComponent::MemType::SHARED);
...
REQRESPONSE Opal::allocateLocalMemory(int node, int coreId, uint64_t vAddress, int fault_level,
                                      int pages) {
    ...
    if (nodeInfo[node]->pool->available_frames >= pages) {
        ...
        for (int i = 0; i < pages; i++) {
            response = pool->allocate_frame(1);
            nodeInfo[node]->insertFrame(coreId, response.address, vAddress, fault_level,
                                        SST::OpalComponent::MemType::LOCAL);
            if (!response.status)
                output->fatal(CALL_INFO, -1,
        ...
        OPAL_VERBOSE(8, output->verbose(CALL_INFO, 8, 0, "Node%"
            PRIu32
            " Local Memory is drained out\n", node));

        if ((nodeInfo[node]->page_migration && !nodeInfo[node]->memoryAllocationPolicy) ||
            4 == fault_level) {
            if (4 == fault_level)
                std::cerr << getName().c_str() << " migrating a page to allocate memory for node "
                          << node << " core " << coreId << " CR3" << std::endl;
...
REQRESPONSE Opal::allocateFromReservedMemory(int node, uint64_t reserved_vAddress,
    ...
    if (pages_used + pages <= pages_reserved) {
        ...
        output->fatal(CALL_INFO, -1, "Opal: address :%"
        PRIu64
        "llu requested with fileId:%d has no space left\n", vAddress, fileID);
    }
    ...
    if (4 == fault_level) {
        ...
        if (response.status)
            ...
            if (!nodeInfo[node]->allocatedmempool) {
                response = allocateLocalMemory(node, coreId, vAddress, fault_level, pages);
                if (response.page_migration) {
                    if (nodeInfo[node]->page_migration &&
                        0 == nodeInfo[node]->page_migration_policy &&
                        !nodeInfo[node]->memoryAllocationPolicy) {
                        // Bulk page migrations
                        migratePages(node, coreId, nodeInfo[node]->num_pages_to_migrate);
                    } else if (nodeInfo[node]->page_migration &&
                               1 == nodeInfo[node]->page_migration_policy &&
                               !nodeInfo[node]->memoryAllocationPolicy) {
                        // Only one page migration
                        migratePages(node, coreId, 1);
                    } else
                        output->fatal(CALL_INFO, -1,
                                      "Opal: Page migration policy not detected. This should not happen\n");
...
void Opal::migratePages(int node, int coreId, int pages) {
    ...
    std::list < std::pair < uint64_t, std::pair < uint64_t,
        int > > > lm_pages = nodeInfo[node]->getPagesToMigrate(pages);
    ...
    // error checking
    if ((uint32_t) lm_pages.size() != (uint32_t) pages &&
        (uint32_t) sm_pages.size() != (uint32_t) pages)
    ...
    while (!lm_pages.empty()) {
        ...
        nodeInfo[node]->insertFrame(coreId, sm_address, it.second.first, it.second.second,
                                    SST::OpalComponent::MemType::SHARED);
        ...
        nodeInfo[node]->coreInfo[coreId].addInvalidAddress(sm_address, it.second.first,
                                                           it.second.second);
...
void Opal::tlbShootdown(int node, int coreId, int shootdownId) {
    ...
    std::list < std::pair < uint64_t, std::pair < uint64_t,
        int > > > *invalidAddrs = nodeInfo[node]->coreInfo[coreId].getInvalidAddresses();
    ...
    int shootdown_latency =
        (nodeInfo[node]->cores / 2) * (15000 / nodeInfo[node]->clock) * 1000; // in ns
...
void Opal::processTLBShootdownAck(int node, int coreId, int shootdownId) {
    ...
    it = tlbShootdownInfo.find(shootdownId);
...
void Opal::processShootdownEvent(int node, int coreId, uint64_t vaddress, uint64_t paddress,
                                 int fault_level) {
    int shootdownId = nodeInfo[node]->coreInfo[coreId].id;
    ...
    std::list < std::pair < uint64_t, std::pair < uint64_t,
        int > > > lm_pages = nodeInfo[node]->getPagesToMigrate(1);

    if (lm_pages.size() > 1)
        output->fatal(CALL_INFO, -1,
                      "%s, Error - shootdown request local memory migration pages more than 1\n",
                      getName().c_str());
    ...
    nodeInfo[node]->insertFrame(coreId, paddress, it.second.first, it.second.second,
                                SST::OpalComponent::MemType::SHARED);

    nodeInfo[node]->insertFrame(coreId, it.first, vaddress, fault_level,
                                SST::OpalComponent::MemType::LOCAL);
        ...
        if (inst_served < max_inst) {
            ...
            switch (ev->getType()) {
                ...
                case SST::OpalComponent::EventType::MMAP: {
                    ...
                    std::cerr << "MLM mmap(" << ev->getFileId() << ") : level " << ev->getHint()
                              << " Starting address is " << std::hex << ev->getAddress();
                    std::cerr << std::dec << " Size: " << ev->getSize();
                ...
                case SST::OpalComponent::EventType::INVALIDADDR: {
                    OPAL_VERBOSE(8, output->verbose(CALL_INFO, 8, 0, "Node%"
                        PRIu32
                        " Opal has received an INVALIDADDR CALL\n", ev->getNodeId()));
                ...
                case SST::OpalComponent::EventType::SHOOTDOWN: {
                    OPAL_VERBOSE(8, output->verbose(CALL_INFO, 8, 0, "Node%"
                        PRIu32
                        " Opal has received an SHOOTDOWN CALL\n", ev->getNodeId()));
                    processShootdownEvent(ev->getNodeId(), ev->getCoreId(), ev->getAddress(),
                                          ev->getPaddress(), ev->getFaultLevel());
                }
                    break;
                ...
                case SST::OpalComponent::EventType::SDACK: {
                    OPAL_VERBOSE(8, output->verbose(CALL_INFO, 8, 0, "Node%"
                        PRIu32
                        " Opal has received an SDACK CALL\n", ev->getNodeId()));
                ...
                case SST::OpalComponent::EventType::REQUEST: {
                    ...
                    removeEvent = processRequest(ev->getNodeId(), ev->getCoreId(), ev->getAddress(),
