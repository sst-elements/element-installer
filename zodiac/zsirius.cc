...
ZodiacSiriusTraceReader::ZodiacSiriusTraceReader(ComponentId_t id, Params &params) :
    ...
    selfLink = configureSelfLink("Self", "1ns",
                                 new Event::Handler<ZodiacSiriusTraceReader>(this,
                                                                             &ZodiacSiriusTraceReader::handleSelfEvent));
...
void ZodiacSiriusTraceReader::finish() {
    msgapi->finish();
    zOut.verbose(CALL_INFO, 1, 0, "Completed simulation at: %"
    PRIu64
    "ns\n",
        getCurrentSimTimeNano());
    zOut.verbose(CALL_INFO, 1, 0, "Statistics for run are:\n");
    zOut.verbose(CALL_INFO, 1, 0, "- Total Send Count:           %"
    PRIu64
    "\n", zSendCount);
    zOut.verbose(CALL_INFO, 1, 0, "- Total Recv Count:           %"
    PRIu64
    "\n", zRecvCount);
    zOut.verbose(CALL_INFO, 1, 0, "- Total IRecv Count:          %"
    PRIu64
    "\n", zIRecvCount);
    zOut.verbose(CALL_INFO, 1, 0, "- Total Wait Count:           %"
    PRIu64
    "\n", zWaitCount);
    zOut.verbose(CALL_INFO, 1, 0, "- Total Allreduce Count:      %"
    PRIu64
    "\n", zAllreduceCount);
    zOut.verbose(CALL_INFO, 1, 0, "- Total Send Bytes:           %"
    PRIu64
    "\n", zSendBytes);
    zOut.verbose(CALL_INFO, 1, 0, "- Total Recv Bytes:           %"
    PRIu64
    "\n", zRecvBytes);
    zOut.verbose(CALL_INFO, 1, 0, "- Total Posted-IRecv Bytes:   %"
    PRIu64
    "\n", zIRecvBytes);
    zOut.verbose(CALL_INFO, 1, 0, "- Total Allreduce Bytes       %"
    PRIu64
    "\n", zAllreduceBytes);
    zOut.verbose(CALL_INFO, 1, 0, "- Time spend in compute:      %"
    PRIu64
    " ns\n", nanoCompute);
    zOut.verbose(CALL_INFO, 1, 0, "- Time spend in send:         %"
    PRIu64
    " ns (mean: %f)\n", nanoSend,
        zSendCount > 0 ? ((double) nanoSend) / ((double) zSendCount) : 0.0);
    zOut.verbose(CALL_INFO, 1, 0, "- Time spend in recv:         %"
    PRIu64
    " ns (mean: %f)\n", nanoRecv,
        zRecvCount > 0 ? ((double) nanoRecv) / ((double) zRecvCount) : 0.0);
    zOut.verbose(CALL_INFO, 1, 0, "- Time spend in irecv issue:  %"
    PRIu64
    " ns\n", nanoIRecv);
    zOut.verbose(CALL_INFO, 1, 0, "- Time spend in wait:         %"
    PRIu64
    " ns\n", nanoWait);
    zOut.verbose(CALL_INFO, 1, 0, "- Time spend in all-reduce:   %"
    PRIu64
    " ns\n", nanoAllreduce);
    zOut.verbose(CALL_INFO, 1, 0, "- Time spend in barrier:      %"
    PRIu64
    " ns\n", nanoBarrier);
    zOut.verbose(CALL_INFO, 1, 0, "- Time spend in init:         %"
    PRIu64
    " ns\n", nanoInit);
    zOut.verbose(CALL_INFO, 1, 0, "- Time spend in finalize:     %"
    PRIu64
    " ns\n", nanoFinalize);

    zOut.output("Completed at %"
    PRIu64
    " ns\n", getCurrentSimTimeNano());
}
...
ZodiacSiriusTraceReader::~ZodiacSiriusTraceReader() {
    if (trace) {
        if (!trace->hasReachedFinalize()) {
            zOut.output(
                "WARNING: Component did not reach a finalize event, yet the component destructor has been called.\n");
        }
    ...
    if (zEv) {
        switch (zEv->getEventType()) {
            ...
            default:
                zOut.verbose(__LINE__, __FILE__, "handleSelfEvent",
                             0, 1,
                             "Attempted to process an ZodiacEvent which is not included in the event decoding step.\n");
                exit(-1);
                break;
        }
    } else {
        zOut.verbose(__LINE__, __FILE__, "handleSelfEvent",
                     0, 1,
                     "Attempted to process an event which is not supported by the Zodiac engine.\n");
        exit(-1);
    }
...
void ZodiacSiriusTraceReader::handleSendEvent(ZodiacEvent *zEv) {
    ...
    zOut.verbose(__LINE__, __FILE__, "handleSendEvent",
                 2, 1, "Processing a Send event (length=%"
    PRIu32
    ", tag=%d, dest=%"
    PRIu32
    ")\n",
...
void ZodiacSiriusTraceReader::handleRecvEvent(ZodiacEvent *zEv) {
    ...
    zOut.verbose(__LINE__, __FILE__, "handleRecvEvent",
                 2, 1, "Processing a Recv event (length=%"
    PRIu32
    ", tag=%d, source=%"
    PRIu32
    ")\n",
...
void ZodiacSiriusTraceReader::handleIRecvEvent(ZodiacEvent *zEv) {
    ...
    zOut.verbose(__LINE__, __FILE__, "handleIrecvEvent",
                 2, 1, "Processing a Irecv event (length=%"
    PRIu32
    ", tag=%d, source=%"
    PRIu32
    ")\n",
...
void ZodiacSiriusTraceReader::handleComputeEvent(ZodiacEvent *zEv) {
    ...
    if (eventQ->size() > 0) {
        ...
        zOut.verbose(__LINE__, __FILE__, "handleComputeEvent",
                     2, 1,
                     "Enqueuing next event at a delay of %f seconds, scaled by %f = %f seconds)\n",
                     zCEv->getComputeDuration(), scaleCompute,
                     scaleCompute * zCEv->getComputeDuration());
        eventQ->pop();
...
bool ZodiacSiriusTraceReader::completedWaitFunction(int retVal) {
    ...
    std::map<uint64_t, MessageRequest *>::iterator req_map_itr = reqMap.find(
        currentlyProcessingWaitEvent);
    if (req_map_itr == reqMap.end()) {
        zOut.fatal(CALL_INFO, -1,
                   "Error: unable to find a wait request in the ID to MessageRequest map.\n");
    } else {
...
void ZodiacSiriusTraceReader::enqueueNextEvent() {
    ...
    if (eventQ->size() > 0) {
        ...
        zOut.verbose(CALL_INFO, 2, 0,
                     "No more events to process, Zodiac will mark component as complete.\n");
