...
SiriusReader::SiriusReader(char *file, uint32_t focusOnRank, uint32_t maxQLen,
                           std::queue<ZodiacEvent *> *evQ, int verbose) {
...
void SiriusReader::close() {
    if (nullptr == trace) {
        output->fatal(CALL_INFO, -1,
                      "Error: trace file is nullptr when being closed, has an error occured in SIRIUS?\n");
    } else {
...
void SiriusReader::generateNextEvent() {
    ...
    if (evTimeDiff > 0) {
        output->verbose(__LINE__, __FILE__, "generateNextEvent", 8, 0,
                        "Generated a compute event (length=%f)\n", evTimeDiff);
