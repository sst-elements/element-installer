...
int handleOTFDefineProcess(void *userData, uint32_t stream, uint32_t process, const char *name,
                           uint32_t parent) {
...
int handleOTFSendMsg(void *userData, uint64_t time, uint32_t sender, uint32_t receiver,
                     uint32_t group, uint32_t type,
                     uint32_t length, uint32_t source, OTF_KeyValueList *list) {

    OTFReader *reader = (OTFReader *) userData;
    std::cout << "OTF: Send message from " << sender << " to " << receiver << " qSize="
              << reader->getCurrentQueueSize() << std::endl;
...
int handleOTFRecvMsg(void *userData, uint64_t time, uint32_t recvProc, uint32_t sendProc,
                     uint32_t group, uint32_t type,
                     uint32_t length, uint32_t source, OTF_KeyValueList *list) {

    OTFReader *reader = (OTFReader *) userData;
    std::cout << "OTF: Recv message from " << sendProc << " to " << recvProc << " qSize="
              << reader->getCurrentQueueSize() << std::endl;
...
int handleOTFEnter(void *data, uint64_t time, uint32_t func, uint32_t proc, uint32_t src) {
    std::cout << "OTF: Entered function: " << func << " on process: " << proc << " at time: "
              << time << std::endl;
...
int handleOTFExit(void *data, uint64_t time, uint32_t func, uint32_t proc, uint32_t src) {
    std::cout << "OTF: Exited function: " << func << " on process: " << proc << " at time: " << time
              << std::endl;

    return OTF_RETURN_OK;
}

int handleOTFCollectiveOperation(void *userData, uint64_t time, uint32_t process,
                                 uint32_t collective, uint32_t procGroup,
...
int handleOTFBeginCollective(void *userData, uint64_t time, uint32_t process, uint32_t collOp,
...
int handleOTFEndCollective(void *userData, uint64_t time, uint32_t process, uint64_t matchingId,
                           OTF_KeyValueList *list) {
...
OTFReader::OTFReader(string file, uint32_t focusRank, uint32_t maxQLength,
    ...
    OTF_HandlerArray_setHandler(handlers, (OTF_FunctionPointer *) handleOTFDefineProcess,
                                OTF_DEFPROCESS_RECORD);
    OTF_HandlerArray_setFirstHandlerArg(handlers, this, OTF_DEFPROCESS_RECORD);
    OTF_HandlerArray_setHandler(handlers, (OTF_FunctionPointer *) handleOTFRecvMsg,
                                OTF_RECEIVE_RECORD);
    OTF_HandlerArray_setFirstHandlerArg(handlers, this, OTF_RECEIVE_RECORD);
    OTF_HandlerArray_setHandler(handlers, (OTF_FunctionPointer *) handleOTFSendMsg,
                                OTF_SEND_RECORD);
    OTF_HandlerArray_setFirstHandlerArg(handlers, this, OTF_SEND_RECORD);
    OTF_HandlerArray_setHandler(handlers, (OTF_FunctionPointer *) handleOTFCollectiveOperation,
                                OTF_COLLOP_RECORD);
    OTF_HandlerArray_setFirstHandlerArg(handlers, this, OTF_COLLOP_RECORD);
    OTF_HandlerArray_setHandler(handlers, (OTF_FunctionPointer *) handleOTFBeginCollective,
                                OTF_BEGINCOLLOP_RECORD);
    OTF_HandlerArray_setFirstHandlerArg(handlers, this, OTF_BEGINCOLLOP_RECORD);
    OTF_HandlerArray_setHandler(handlers, (OTF_FunctionPointer *) handleOTFEndCollective,
