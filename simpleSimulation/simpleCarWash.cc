...
namespace SST {
    namespace SimpleCarWash {
        ...
        simpleCarWash::simpleCarWash(ComponentId_t id, Params &params) :
            ...
            srand(
                5);                    // We need a bit of randomness for our customer arrival simulation

            // Initialize all of the simulation variables and parameters (this code is reasonably self-explanatory)

            m_ptrCarRecordList = nullptr;    // Clear the car queue linked list
...
//		printf("Queueing a car type=%d\n", CarType);
            ...
            if ((0 < m_CarWashTick) && (0 == (m_CarWashTick % 60))) {
                std::cout << "------------------------------------------------------------------"
                          << std::endl;
                std::cout << "Time= " << m_CarWashTick
                          << "(minutes) another simulated hour has passed! Summary:" << std::endl;
            ...
            if ((m_SimulationTimeLength) <= m_CarWash.currentTime) {
                ...
                std::cout << "Simulation duration was:" << m_SimulationTimeLength
                          << "(sec.) (" << (m_SimulationTimeLength / 60) << " hours) of operation."
                          << std::endl;
                ...
                std::cout << "Total Cars washed: "
                          << (m_CarWash.smallCarsWashed + m_CarWash.largeCarsWashed) << std::endl;
            ...
            std::cout
                << "Help! For some reason the simulator has reached code that shouldn't have been reached!"
                << std::endl;
            std::cout << "This would imply there is a bug that needs to be addressed??"
                      << std::endl;
...
//	printf("Customer Code:%d\n", rndNumber);
        ...
        int simpleCarWash::QueueACar(int carSize, int clockTick) {
            ...
            if (nullptr == ptr) {                // First entry is a special case
                if (nullptr == (m_ptrCarRecordList = new CAR_RECORD)) {
                    printf("Error allocating memory for the first new car wash record\n");
                    return (-1);
                }
                ptr = m_ptrCarRecordList;            // Anchor the new list
            } else {
                if (nullptr == (ptr->ptrNext = new CAR_RECORD)) {
...
// but this will do the trick for the moment...
        ...
        int *simpleCarWash::DeQueueCar() {
            ...
            car->ptrNext = nullptr;                        // This isn't necessary, but it's tidy
...
// This code is unnecessary and can be deleted
        int simpleCarWash::DumpCarRecordList() {
            ...
            while (1) {
                if (nullptr != ptr) {
                    std::cout << "Car record[" << count << "]: " << ptr->EntryTime << " "
                              << ptr->CarSize << std::endl;
                    count++;                // Increment
...
// one of our neighbors.
        bool simpleCarWash::CarWashClockTick(int clockTick) {
            ...
            if (nullptr != m_ptrCarRecordList) {        // Are there any cars waiting?
                // See if we can fill the small bay first
                if ((m_SmallBay.Occupied == WASH_BAY_EMPTY) &&
                    (m_ptrCarRecordList->CarSize == SMALL_CAR)) { // SB - Open SM - Next
//			printf("%d Filling Small Bay\n", __LINE__);
            ...
            if (nullptr !=
                m_ptrCarRecordList) {        // Examine the linked list to see if there are any cars waiting to be washed?
...
//			printf("%d, Filling Large Bay\n", __LINE__);
            ...
            if (nullptr != m_ptrCarRecordList) {        // Are there any cars waiting?
                if ((m_SmallBay.Occupied == WASH_BAY_EMPTY) &&
                    (m_ptrCarRecordList->CarSize == SMALL_CAR)) {
