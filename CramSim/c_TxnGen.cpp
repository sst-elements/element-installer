...
c_TxnGenBase::c_TxnGenBase(ComponentId_t x_id, Params &x_params) :
    ...
    uint32_t k_numBytesPerTransaction = x_params.find<std::uint32_t>("numBytesPerTransaction", 32,
                                                                     l_found);
    if (!l_found) {
    ...
    std::string l_controllerClockFreqStr = (std::string) x_params.find<std::string>(
...
void c_TxnGenBase::finish() {
    ...
    double l_txnsPerCycle =
        static_cast<double>(m_resReadCount + m_resWriteCount) / static_cast<double>(m_simCycle);
...
c_TxnGen::c_TxnGen(ComponentId_t x_id, Params &x_params) :
    ...
    if (l_randSeedStr.compare("-") == 0) { // use a random seed
        ...
        k_randSeed = (SimTime_t) std::strtoul(l_randSeedStr.c_str(), nullptr, 0);
    }
