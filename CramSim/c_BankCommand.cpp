...
c_BankCommand::c_BankCommand(unsigned x_cmdSeqNum,
    ...
    assert(x_cmdMnemonic == e_BankCommandType::REF ||
           x_cmdMnemonic == e_BankCommandType::PRE); // This constructor only for REF cmds!
...
void c_BankCommand::print(SST::Output *x_debugOutput, const std::string x_prefix,
                          SimTime_t x_cycle) const {
    x_debugOutput->verbosePrefix(x_prefix.c_str(), CALL_INFO, 1, 0,
                                 "Cycle:%lld Cmd:%s seqNum: %llu CH:%d PCH:%d Rank:%d BG:%d B:%d Row:%d Col:%d BankId:%d\n",
                                 x_cycle,
