...
c_TxnScheduler::c_TxnScheduler(SST::Component *owner, SST::Params &x_params) : SubComponent(owner) {
    ...
    string l_txnSchedulingPolicy = (string) x_params.find<std::string>("txnSchedulingPolicy",
                                                                       "FCFS", l_found);
    if (!l_found) {
        std::cout << "txnSchedulingPolicy value is missing... FCFS policy will be used"
                  << std::endl;
    }
    ...
    if (l_txnSchedulingPolicy == "FCFS") {
        ...
        std::cout << "unsupported txnSchedulingPolicy (" << l_txnSchedulingPolicy << "),, exit"
                  << std::endl;
        exit(1);
    }
    ...
    k_isReadFirstScheduling = (unsigned) x_params.find<unsigned>("boolReadFirstTxnScheduling", 0,
                                                                 l_found);
    if (!l_found) {
    ...
    if (!k_isReadFirstScheduling)
        ...
        k_maxPendingWriteThreshold = (float) x_params.find<float>("maxPendingWriteThreshold", 1,
                                                                  l_found);
        if (!l_found) {
            std::cout << "maxPendingWriteThreshold value is missing... it will be 1.0 (default)"
                      << std::endl;
        } else {
            if (k_maxPendingWriteThreshold > 1) {
                std::cout
                    << "maxPendingWriteThreshold value should be greater than 0 and less than (or equal to) one"
                    << std::endl;
                exit(1);
            }
        }

        k_minPendingWriteThreshold = (float) x_params.find<float>("minPendingWriteThreshold", 0.2,
                                                                  l_found);
        if (!l_found) {
            std::cout << "minPendingWriteThreshold value is missing... it will be 1.0 (default)"
                      << std::endl;
        } else {
            if (k_minPendingWriteThreshold > k_maxPendingWriteThreshold) {
                std::cout
                    << "minPendingWriteThreshold value should be smaller than maxPendingWriteThreshold"
                    << std::endl;
                exit(1);
            }
        }
    ...
    if (k_txnSchedulingPolicy == e_txnSchedulingPolicy::FCFS) {
        ...
        for (auto &l_txn: x_queue) {
            if (m_cmdScheduler->getToken(l_txn->getHashedAddress()) >= 3) {
                ...
                c_BankInfo *l_bankInfo = m_txnConverter->getBankInfo(
                    l_txn->getHashedAddress().getBankId());
...
bool c_TxnScheduler::isHit(c_Transaction *x_txn) {
    ...
    if (l_isRead) {
        ...
        for (TxnQueue::reverse_iterator l_txnItr = l_queue->rbegin();
             l_txnItr != l_queue->rend(); ++l_txnItr) {
