...
c_TxnConverter::c_TxnConverter(SST::Component *owner, SST::Params &x_params) : SubComponent(owner) {
    ...
    std::string l_bankPolicy = (std::string) x_params.find<std::string>("bankPolicy", "CLOSE",
                                                                        l_found);
    if (!l_found) {
        std::cout << "bankPolicy value is missing... it will be \"close-page policy\" (Default)"
                  << std::endl;
    }
    if (l_bankPolicy == "CLOSE")
        ...
        if (!l_found) {
            std::cout
                << "bank policy is set to pseudo open policy, but bankCloseTime value is missing... exiting"
                << std::endl;
            exit(-1);
        }
        if (k_bankPolicy == 2) {
    ...
    if (k_bankPolicy == 0) {
        ...
        if (l_bankinfo->isRowOpen()) {
            ...
            if (l_bankinfo->getOpenRowNum() != l_row) {
                ...
                x_commandVec.push_back(
                    new c_BankCommand(m_cmdSeqNum++, e_BankCommandType::PRE, x_nAddr,
                                      l_hashedAddr));
                //open row
                x_commandVec.push_back(
                    new c_BankCommand(m_cmdSeqNum++, e_BankCommandType::ACT, x_nAddr,
                                      l_hashedAddr));
