...
c_Controller::c_Controller(ComponentId_t id, Params &params) :
    ...
    std::string l_subCompName = params.find<std::string>("DeviceDriver", "CramSim.c_DeviceDriver",
                                                         l_found);
    if (!l_found) {
        output->output(
            "Device Controller is not specified... c_DeviceDriver (default) will be used\n");
    }
    m_deviceDriver = dynamic_cast<c_DeviceDriver *>(loadSubComponent(l_subCompName.c_str(), this,
                                                                     params));
    ...
    if (!l_found) {
        output->output(
            "Command Scheduler is not specified... c_CmdScheduler (default) will be used\n");
    }

    m_cmdScheduler = dynamic_cast<c_CmdScheduler *>(loadSubComponent(l_subCompName.c_str(), this,
                                                                     params));
    ...
    if (!l_found) {
        output->output(
            "Transaction Converter is not specified... c_TxnConverter (default) will be used\n");
    }
    m_txnConverter = dynamic_cast<c_TxnConverter *>(loadSubComponent(l_subCompName.c_str(), this,
                                                                     params));
    ...
    m_txnScheduler = dynamic_cast<c_TxnScheduler *>(loadSubComponent(l_subCompName.c_str(), this,
                                                                     params));
    ...
    m_addrHasher = dynamic_cast<c_AddressHasher *>(loadSubComponent(l_subCompName.c_str(), this,
                                                                    params));
    ...
    k_controllerClockFreqStr = (std::string) params.find<std::string>("strControllerClockFrequency",
                                                                      "1GHz", l_found);
    ...
    for (std::deque<c_Transaction *>::iterator l_it = m_ReqQ.begin(); l_it != m_ReqQ.end();) {
        ...
        if (m_txnScheduler->push(newTxn)) {
            ...
            if (k_enableQuickResponse && newTxn->isWrite()) {
                ...
                c_Transaction *l_txnRes = new c_Transaction(newTxn->getSeqNum(),
...
void c_Controller::handleInDeviceResPtrEvent(SST::Event *ev) {
    ...
    if (l_cmdResEventPtr) {
        ...
        if (l_txnRes == nullptr) {
            std::cout << "Error! Couldn't find transaction to match cmdSeqnum " << l_resSeqNum
                      << std::endl;
            exit(-1);
        }
