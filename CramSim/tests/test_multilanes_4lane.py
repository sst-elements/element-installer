...
txnGenLink.connect((comp_txnGen, "memLink", g_params["clockCycle"]),
                   (comp_txnDispatcher, "txnGen", g_params["clockCycle"]))
...
for chid in range(numLanes):
    ...
    txnReqLink_0.connect((comp_txnDispatcher, "lane_" + (str(chid)), g_params["clockCycle"]),
                         (comp_controller, "txngenLink", g_params["clockCycle"]))

    # Controller -> Dimm (Req)
    cmdReqLink_1 = sst.Link("cmdReqLink_1_" + (str(chid)))
    cmdReqLink_1.connect((comp_controller, "memLink", g_params["clockCycle"]),
                         (comp_dimm, "ctrlLink", g_params["clockCycle"]))
