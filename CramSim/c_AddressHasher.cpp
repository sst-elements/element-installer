...
c_AddressHasher::c_AddressHasher(Component *comp, Params &params) : SubComponent(comp) {
    ...
    if (l_allSimple) { // if simple address detected, reset the bitPositions structureSizes
        ...
        l_cfgBits["c"] = (uint) log2(
            k_pNumPseudoChannels);   // set the number of address bits assigned to pseudo channel.
    ...
    if (l_it == m_structureSizes.end()) { // if not found
        ...
        if (l_aNumChannels > k_pNumChannels) {
            cerr << "Warning!: Number of address map channels is larger than numChannelsPerDimm."
                 << endl;
            cerr << "Some channels will be unused" << endl << endl;
        }
        if (l_aNumChannels < k_pNumChannels) { // some addresses have nowhere to go
            cerr
                << "Error!: Number of address map channels is smaller than numChannelsPerDimm. Aborting!"
                << endl;
            exit(-1);
        }
    } // else found in map
    ...
    if (l_it == m_structureSizes.end()) { // if not found
        if (k_pNumPseudoChannels > 1) {
            cerr << "Number of Pseudo Channels (" << k_pNumPseudoChannels
                 << ") is greater than 1, but no "
                 << "Channels were specified (c) in the address map! Aborting!" << endl;
            exit(-1);
        }
    } else { // found in map
        ...
        if (l_aNumPChannels > k_pNumPseudoChannels) {
            cerr << "Warning!: Number of address map channels is larger than numPseudoChannels."
                 << endl;
            cerr << "Some channels will be unused" << endl << endl;
        }
        if (l_aNumPChannels < k_pNumPseudoChannels) { // some addresses have nowhere to go
            cerr
                << "Error!: Number of address map channels is smaller than numPseudoChannels. Aborting!"
                << endl;
            exit(-1);
        }
    } // else found in map
    ...
    if (l_it == m_structureSizes.end()) { // if not found
        ...
        if (l_aNumRanks > k_pNumRanks) {
            cerr << "Warning!: Number of address map Ranks is larger than numRanksPerChannel."
                 << endl;
            cerr << "Some Ranks will be unused" << endl << endl;
        }
        if (l_aNumRanks < k_pNumRanks) { // some addresses have nowhere to go
            cerr
                << "Error!: Number of address map Ranks is smaller than numRanksPerChannel. Aborting!"
                << endl;
            exit(-1);
        }
    } // else found in map
    ...
    if (l_it == m_structureSizes.end()) { // if not found
        ...
        if (l_aNumBankGroups > k_pNumBankGroups) {
            cerr
                << "Warning!: Number of address map bankGroups is larger than numBankGroupsPerRank."
                << endl;
            cerr << "Some BankGroups will be unused" << endl << endl;
        }
        if (l_aNumBankGroups < k_pNumBankGroups) { // some addresses have nowhere to go
            cerr
                << "Error!: Number of address map bankGroups is smaller than numBankGroupsPerRank. Aborting!"
                << endl;
            exit(-1);
        }
    } // else found in map
    ...
    if (l_it == m_structureSizes.end()) { // if not found
        ...
        if (l_aNumBanks > k_pNumBanks) {
            cerr << "Warning!: Number of address map Banks is larger than numBanksPerBankGroup."
                 << endl;
            cerr << "Some Banks will be unused" << endl << endl;
        }
        if (l_aNumBanks < k_pNumBanks) { // some addresses have nowhere to go
            cerr
                << "Error!: Number of address map Banks is smaller than numBanksPerBankGroup. Aborting!"
                << endl;
            exit(-1);
        }
    } // else found in map
    ...
    if (l_it == m_structureSizes.end()) { // if not found
        ...
        if (l_aNumRows < k_pNumRows) { // some addresses have nowhere to go
            cerr << "Error!: Number of address map Rows is smaller than numRowsPerBank. Aborting!"
                 << endl;
            exit(-1);
        }
    } // else found in map
    ...
    if (l_it == m_structureSizes.end()) { // if not found
        ...
        if (l_aNumCols < k_pNumCols) { // some addresses have nowhere to go
            cerr << "Error!: Number of address map Cols is smaller than numColsPerBank. Aborting!"
                 << endl;
            exit(-1);
        }
    } // else found in map
    ...
    if (l_it == m_structureSizes.end()) { // if not found
        ...
        if (l_aNumCachelines != k_pBurstSize) {
            cerr
                << "Error!: Number of address map Cachelines is not equal to numBytesPerTransaction."
                << endl;
            cerr
                << "Make sure that the address map cachelines (h) are equal to numBytesPerTransaction (i.e. 2**h == numByutesPerTransaction!"
                << endl;
            exit(-1);
        }
    } // else found in map
...
void c_AddressHasher::fillHashedAddress(c_HashedAddress *x_hashAddr, const ulong x_address) {
    ...
    unsigned l_bankId =
        ...
        + x_hashAddr->getChannel() * k_pNumPseudoChannels * k_pNumBanks * k_pNumBankGroups *
          k_pNumRanks;
...
ulong c_AddressHasher::getAddressForBankId(const unsigned x_bankId) {
    ...
    cout << "Final " << l_chan << " " << l_pchan << " " << l_rank << " " << l_bankgroup << " "
         << l_bank << endl;
    ...
    while (!l_matched) {
        if (isdigit(*l_sIter)) {
            if (l_sizeMatched) {
                ...
                cerr << "Parsing error at " << *l_sIter << " in address map string " << *x_inStr
                     << endl;
                exit(-1);
            }
