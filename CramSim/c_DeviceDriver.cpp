...
c_DeviceDriver::c_DeviceDriver(Component *owner, Params &params) : SubComponent(owner) {
    ...
    k_numPChannelsPerChannel = (uint32_t) params.find<uint32_t>("numPChannelsPerChannel", 1,
                                                                l_found);
    if (!l_found) {
    ...
    if (!l_found) {
        std::cout << "boolUseSBRefresh (single bank refresh) param value is missing... disabled"
                  << std::endl;
    }
    ...
    if (k_printCmdTrace) {
        if (k_cmdTraceFileName.compare("-") == 0) {// set output to std::cout
            ...
            if (m_cmdTraceOFStream) {
                ...
                std::cerr << "Failed to open cmd trace output file " << k_cmdTraceFileName
...
bool c_DeviceDriver::isCmdAllowed(c_BankCommand *x_bankCommandPtr) {
    ...
    if (x_bankCommandPtr->isColCommand() &&
        (m_banks.at(l_bankId)->getCurrentState() == e_BankState::PRE ||
         m_banks.at(l_bankId)->getCurrentState() == e_BankState::IDLE)) {
        //check if ACT command is already inserted to inputQ
        for (auto &l_cmd: m_inputQ) {
            if ((l_cmd->getBankId() == x_bankCommandPtr->getBankId()) &&
                (l_cmd->getCommandMnemonic() == e_BankCommandType::ACT))
                return false;
        }
        ...
        c_BankCommand *l_newCmd = new c_BankCommand(0, e_BankCommandType::ACT, l_addr,
                                                    *l_hashedAddr);
...
void c_DeviceDriver::sendRequest() {
    ...
    for (auto l_cmdPtrItr = m_inputQ.begin(); l_cmdPtrItr != m_inputQ.end();) {
        ...
        if ((e_BankCommandType::ACT == ((l_cmdPtr))->getCommandMnemonic()) &&
            (l_numACTIssuedInFAW[l_rankNum] >= 4)) {
            l_proceed = false;
        }
        ...
        if (l_proceed) {
            ...
            if (isCommandBusAvailable(l_cmdPtr)) {   //check if the command bus is available
                ...
                if (l_cmdPtr->getCommandMnemonic() == e_BankCommandType::PRE)
                    if (l_bank->getCurrentState() == e_BankState::PRE ||
                        l_bank->getCurrentState() == e_BankState::IDLE) {
        ...
        if (l_cmdPtr->getBankIdVec().size() > 0) {
            for (auto &l_bankid:l_cmdPtr->getBankIdVec()) {
                ...
                c_BankCommand *l_cmd = new c_BankCommand(l_bankid,
                                                         l_cmdPtr->getCommandMnemonic(),
                                                         l_cmdPtr->getAddress(),
                                                         *(l_cmdPtr->getHashedAddress()),
                                                         l_cmdPtr->getBankIdVec());
        ...
        if (k_printCmdTrace) {
            ...
            if (x_bankCommandPtr->isRefreshType()) {
                ...
                (*m_cmdTraceStream) << "@" << std::dec
                                    ...
                                    << " " << std::dec
                                    << x_bankCommandPtr->getHashedAddress()->getCacheline()
