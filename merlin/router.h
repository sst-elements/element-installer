...
#define MERLIN_ENABLE_TRACE
        ...
        public:
            enum RtrEventType {
                CREDIT, PACKET, INTERNAL, TOPOLOGY, INITIALIZATION
            };
        ...
        public:
            ...
            virtual void print(const std::string &header, Output &out) const override {
                out.output("%s TopologyEvent to be delivered at %"
                PRIu64
                " with priority %d\n",
        ...
        public:
            ...
            enum Commands {
                REQUEST_VNS, SET_VCS, REPORT_ID, REPORT_BW, REPORT_FLIT_SIZE, REPORT_PORT
            };
            ...
            virtual void print(const std::string &header, Output &out) const override {
                out.output("%s RtrInitEvent to be delivered at %"
                PRIu64
                " with priority %d\n",
                    header.c_str(), getDeliveryTime(), getPriority());
        ...
        public:
            ...
            internal_router_event(RtrEvent *ev) :
                BaseRtrEvent(BaseRtrEvent::INTERNAL) { encap_ev = ev; }

            virtual ~internal_router_event() {
                if (encap_ev != nullptr) delete encap_ev;
            }
            ...
            inline void setCreditReturnVC(int vc) {
                credit_return_vc = vc;
                return;
            }
            ...
            inline void setNextPort(int np) {
                next_port = np;
                return;
            }
            ...
            inline void setVN(int vn) {
                encap_ev->request->vn = vn;
                return;
            }
            ...
            virtual void print(const std::string &header, Output &out) const override {
                out.output("%s internal_router_event to be delivered at %"
                PRIu64
                " with priority %d.  src = %d, dest = %d\n",
                    header.c_str(), getDeliveryTime(), getPriority(), getSrc(), getDest());
                if (encap_ev != nullptr) encap_ev->print(header + std::string("-> "), out);
            }
        ...
        public:
            ...
            Topology(Component *comp) : SubComponent(comp), output(
