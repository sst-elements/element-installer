...
namespace SST {
    namespace Merlin {
        ...
        public:
            ...
            { "message_rate", "", "1GHz" },
            { "PacketDest:pattern", "Address pattern to be used (NearestNeighbor, Uniform, HotSpot, Normal, Binomial)", nullptr },
            ...
            { "PacketSize:pattern", "Address pattern to be used (Uniform, HotSpot, Normal, Binomial)", nullptr },
            ...
            { "PacketDelay:pattern", "Address pattern to be used (Uniform, HotSpot, Normal, Binomial)", nullptr },
        ...
        private:
            ...
            class NearestNeighbor : public Generator {
                ...
                int *neighbors;
            public:
                NearestNeighbor(Generator *dist, int id, int maxX, int maxY, int maxZ,
                                int numNeighbors) :
                    dist(dist) {
                    int myX = id % maxX;
                    ...
                    switch (numNeighbors) {
                        case 6: {
                            neighbors[0] =
                                (((myX - 1) + maxX) % maxX) + myY * maxX + myZ * (maxX * maxY);
                            neighbors[1] = ((myX + 1) % maxX) + myY * maxX + myZ * (maxX * maxY);
                            neighbors[2] =
                                myX + (((myY - 1) + maxY) % maxY) * maxX + myZ * (maxX * maxY);
                            neighbors[3] = myX + (((myY + 1)) % maxY) * maxX + myZ * (maxX * maxY);
                            neighbors[4] =
                                myX + myY * maxX + (((myZ - 1) + maxZ) % maxZ) * (maxX * maxY);
            ...
            class NormalDist : public Generator {
                ...
                int maxValue;
            public:
                NormalDist(int min, int max, double mean, double stddev) : minValue(min),
                                                                           maxValue(max) {
                    gen = new MersenneRNG();
