...
namespace SST {
    ...
    namespace Merlin {
        ...
        LinkControl::LinkControl(Component *parent, Params &params) :
            ...
            rr(0), input_buf(nullptr), output_buf(nullptr),
            rtr_credits(nullptr), in_ret_credits(nullptr),
            ...
            std::string checkerboard_alg = params.find<std::string>("checkerboard_alg",
                                                                    "deterministic");
        ...
        LinkControl::LinkControl(ComponentId_t cid, Params &params, int vns) :
            ...
            if (!link_bw.hasUnits("B/s") && !link_bw.hasUnits("b/s")) {
                merlin_abort.fatal(CALL_INFO, 1,
                                   "Error: link_bw must be specified in either B/s or b/s (SI prefix also allowed)\n");
            }
        ...
        void LinkControl::init(unsigned int phase) {
            ...
            switch (phase) {
                ...
                case 1: {
                    ...
                    if (static_cast<BaseRtrEvent *>(ev)->getType() !=
                        BaseRtrEvent::INITIALIZATION) {
                        // fail
                    }
                    ...
                    while ((ev = rtr_link->recvInitData()) != nullptr) {
                        ...
                        switch (bev->getType()) {
                            case BaseRtrEvent::CREDIT: {
                                ...
                                if (ce->vc <
                                    total_vns) {  // Ignore credit events for VNs I don't have
                            ...
                            default:
                                ...
                                merlin_abort_full.fatal(CALL_INFO, 1,
                                                        "Reached state where a non-RtrEvent was not handled.");
                                break;
                        }
                    }
                    break;
...
// the next event.
        SST::Interfaces::SimpleNetwork::Request *LinkControl::recv(int vn) {
            if (input_buf[vn].size() == 0) return nullptr;
            ...
            // into account.
            rtr_link->send(1, new credit_event(event->request->vn,
                                               in_ret_credits[event->request->vn]));
            ...
            if (event->getTraceType() != SimpleNetwork::Request::NONE) {
                output.output("TRACE(%d): %"
                PRIu64
                " ns: recv called on LinkControl in NIC: %s\n", event->getTraceID(),
                    getCurrentSimTimeNano(), getName().c_str());
            }
        ...
        void LinkControl::handle_input(Event *ev) {
            ...
            if (base_event->getType() == BaseRtrEvent::CREDIT) {
                ...
                if (is_idle) {
                    idle_time->addData(
                        Simulation::getSimulation()->getCurrentSimCycle() - idle_start);
                    is_idle = false;
                }
                if (event->request->getTraceType() == SimpleNetwork::Request::FULL) {
                    output.output("TRACE(%d): %"
                    PRIu64
                    " ns: Received and event on LinkControl in NIC: %s"
                    " on VN %d from src %"
                    PRIu64
                    "\n",
