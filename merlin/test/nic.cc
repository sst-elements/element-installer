...
namespace SST {
    ...
    namespace Merlin {
        ...
        nic::nic(ComponentId_t cid, Params &params) :
            ...
            std::string linkcontrol_type = params.find<std::string>("linkcontrol_type",
                                                                    "merlin.linkcontrol");
        ...
        void nic::finish() {
            if (init_count != num_peers) {
                output.output(
                    "NIC %d didn't receive all complete point-to-point messages.  Only recieved %d\n",
                    net_id, init_count);
            }

            if (init_broadcast_count != (num_peers - 1)) {
                output.output(
                    "NIC %d didn't receive all complete broadcast messages.  Only recieved %d\n",
        ...
        void nic::setup() {
            ...
            if (link_control->getEndpointID() != net_id) {
                output.output("NIC ids don't match: param = %"
                PRIi64
                ", LinkControl = %"
                PRIi64
                "\n",
            ...
            if (init_count != num_peers) {
                output.output(
                    "NIC %d didn't receive all init point-to-point messages.  Only recieved %d\n",
                    net_id, init_count);
            }

            if (init_broadcast_count != (num_peers - 1)) {
                output.output(
                    "NIC %d didn't receive all init broadcast messages.  Only recieved %d\n",
                    net_id, init_broadcast_count);
            }
        }
...
*/
        ...
        nic::init_complete(unsigned int phase) {
            ...
            switch (init_state) {
                ...
                case 1: {
                    ...
                    SimpleNetwork::Request *req;
                    while ((req = link_control->recvInitData()) != nullptr) {
                ...
                case 2: {
                    ...
                    while ((req = link_control->recvInitData()) != nullptr) {
                        ...
                        if (req->dest == SimpleNetwork::INIT_BROADCAST_ADDR) {
                            ...
                            if (req->dest != net_id)
                                output.output("%d: received event with dest %lld and src %lld\n",
        ...
        public:
            ...
            virtual void print(const std::string &header, Output &out) const override {
                out.output("%s MyRtrEvent to be delivered at %"
                PRIu64
                " with priority %d.  seq = %d\n",
...
// }
        ...
        nic::clock_handler(Cycle_t cycle) {
            ...
            if (!done && (packets_recd >= expected_recv_count)) {
                output.output("%"
                PRIu64
                ": NIC %d received all packets (total of %d)!\n", cycle, id, expected_recv_count);
                primaryComponentOKToEndSim();
                done = true;
            }
            ...
            if (packets_sent < expected_recv_count) {
                if (link_control->spaceToSend(send_vc, msg_size)) {
                    ...
                    if (packets_sent == expected_recv_count) {
                        output.output("%"
                        PRIu64
                        ":  %d Finished sending packets (total of %d)\n",
                            cycle, id, num_msg);
                    }
                } else {
                    stalled_cycles++;
