...
namespace SST {
    ...
    namespace Merlin {
        ...
        shift_nic::shift_nic(ComponentId_t cid, Params &params) :
            ...
            link_control = (SimpleNetwork *) loadSubComponent("merlin.reorderlinkcontrol", this,
                                                              params);
            ...
            // Register a clock
            registerClock("1GHz", new Clock::Handler<shift_nic>(this, &shift_nic::clock_handler),
                          false);

            link_control->setNotifyOnReceive(
                new SST::Interfaces::SimpleNetwork::Handler<shift_nic>(this,
                                                                       &shift_nic::handle_event));
        ...
        void shift_nic::setup() {
            ...
            if (link_control->getEndpointID() != net_id) {
                output.output("NIC ids don't match: parem = %"
                PRIi32
                ", LinkControl = %"
                PRIi64
