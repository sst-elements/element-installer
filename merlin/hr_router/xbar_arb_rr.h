...
namespace SST {
    namespace Merlin {
        ...
        public:
            ...
            ~xbar_arb_rr() {
                if (rr_vcs != nullptr) delete[] rr_vcs;
            }
            ...
            void arbitrate(
                ...
                for (int port = rr_port, pcount = 0;
                     pcount < num_ports; port = ((port != num_ports - 1) ? port + 1
                                                                         : 0), pcount++) {
                    ...
                    for (int vc = rr_vcs[port], vcount = 0;
                         vcount < num_vcs; vc = ((vc != num_vcs - 1) ? (vc + 1) : 0), vcount++) {

                        // If there is no event, move to next VC
                        internal_router_event *src_event = vc_heads[vc];
                        if (src_event == nullptr) continue;
                        ...
                        if (!ports[next_port]->spaceToSend(next_vc,
