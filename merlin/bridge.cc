...
void Bridge::configureNIC(uint8_t id, SST::Params &params) {
    ...
    nic.nic->initialize("network" + std::to_string(id),
                        ...
                        1, /* should be num VN */
                        params.find<SST::UnitAlgebra>("network_input_buffer_size",
                                                      SST::UnitAlgebra("1KiB")),
                        params.find<SST::UnitAlgebra>("network_output_buffer_size",
                                                      SST::UnitAlgebra("1KiB")));

    nic.nic->setNotifyOnReceive(
        new SimpleNetwork::Handler<Bridge, uint8_t>(this, &Bridge::handleIncoming, id));
    nic.nic->setNotifyOnSend(
        new SimpleNetwork::Handler<Bridge, uint8_t>(this, &Bridge::spaceAvailable, id));
