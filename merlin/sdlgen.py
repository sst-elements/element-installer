    ...
    def formatSDL(self, out, endPoint):
        ...
        for i in xrange(num_core):
            ...
            for l in xrange(params["router_radix"]):
                out.write(
                    "    <link name=link:pod%d_core%d port=port%d latency=%s />\n" % (l, i, l, params["link_lat"]))
            out.write("  </component>\n")
            out.write("\n")
        ...
        for pod in xrange(params["router_radix"]):
            ...
            for r in xrange(params["router_radix"] / 2):
                ...
                for n in range(params["fattree:hosts_per_edge_rtr"]):
                    ...
                    endPoint.formatComp(out, "nic:%s" % formatAddr(myip), node_id,
                                        "link:pod%d_edge%d_node%d" % (pod, r, node_id),
                                        dict([("fattree:addr", addrToNum(myip))]))
...
class topoDragonFly(Topo):
    ...
    def formatParams(self, out):
        ...
        params["peers"] = params["dragonfly:hosts_per_router"] * params["dragonfly:routers_per_group"] * params[
            "dragonfly:num_groups"]

        if (params["dragonfly:routers_per_group"] - 1 + params["dragonfly:hosts_per_router"] + params[
            "dragonfly:intergroup_per_router"]) > params["router_radix"]:
