...
topo_hyperx::topo_hyperx(Component *comp, Params &params) :
    ...
    if (n_ports < needed_ports) {
        output.fatal(CALL_INFO, -1,
                     "Number of ports should be at least %d for this configuration\n",
                     needed_ports);
    }
    ...
    if (dest_router == router_id) {
        ...
        for (int dim = 0; dim < dimensions; dim++) {
            ...
            if ((vc & 0x1) == 1) {
                ...
                for (int p = port_start[dim] + offset;
                     p < port_start[dim] + offset + dim_width[dim]; ++p) {
                    int weight = output_queue_lengths[p * num_vcs + vc];
                    if (weight < min) {
                        min = weight;
                        min_port = p;
                ...
                int min_vc = vc;
                for (int curr_port = port_start[dim]; curr_port < port_start[dim] +
                                                                  ((dim_size[dim] - 1) *
                                                                   dim_width[dim]); ++curr_port) {
