...
API::API(Component *owner, Params &params) :
    ProtocolAPI(owner), m_memHeapLink(nullptr) {
...
void API::send(const Hermes::MemAddr &addr, size_t len, nid_t dest, uint64_t tag) {
    ...
    sendv_common(ioVec, MP::CHAR, dest, tag, MP::GroupWorld, nullptr);
}
...
void API::send(const Hermes::MemAddr &addr, size_t len, MP::RankID dest, uint64_t tag,
    ...
    sendv_common(ioVec, MP::CHAR, dest, tag, grp, nullptr);
}
...
void API::recv(const Hermes::MemAddr &addr, size_t len, nid_t src, uint64_t tag) {
    ...
    ioVec[0].addr = addr;
    ioVec[0].len = len;
    recvv_common(ioVec, MP::CHAR, src, tag, MP::GroupWorld, nullptr);
}
...
void API::recv(const Hermes::MemAddr &addr, size_t len, nid_t src, uint64_t tag,
    ...
    ioVec[0].addr = addr;
    ioVec[0].len = len;
    recvv_common(ioVec, MP::CHAR, src, tag, group, nullptr);
}
