...
struct WidgetEntry {
    WidgetEntry(int cacheLineSize, MemReq *req, SimTime_t issueTime, Callback *callback = nullptr)
        : curAccess(0),
...
public:
    BusLoadWidget(SimpleMemoryModel &model, Output &dbg, int id, Unit *cache, int width, int qSize,
        ...
        m_blocked(false), m_scheduled(false), m_blockedSrc(nullptr), m_numPending(0),
    ...
    void resume(UnitBase *src = nullptr) {
        m_dbg.verbosePrefix(prefix(), CALL_INFO, 1, BUS_WIDGET_MASK, "\n");
...
public:
    BusStoreWidget(SimpleMemoryModel &model, Output &dbg, int id, Unit *cache, int width, int qSize,
                   int latency) :
        Unit(model, dbg), m_cache(cache), m_width(width), m_qSize(qSize), m_latency(latency),
        m_blocked(false), m_blockedSrc(nullptr), m_scheduled(false), m_numPending(0),
    ...
    bool storeCB(UnitBase *src, MemReq *req, Callback *callback = nullptr) {
        assert(nullptr == m_blockedSrc);
