...
class SendMachine {
    ...
    class OutQ {
        ...
        struct Entry {
            Entry(std::pair<FireflyNetworkEvent *, int> data, Callback callback = nullptr) : data(
    ...
    public:
        ...
        OutQ(Nic &nic, Output &output, int myId, int maxQsize) :
            ...
            m_wakeUpCallback(nullptr), m_lastEnq(0), m_enqCnt(0), m_qCnt(0) {
...
public:
    ...
    SendMachine(Nic &nic, int nodeId, int verboseLevel, int verboseMask, int myId,
        ...
        m_unit(unit), m_pktOverhead(pktOverhead), m_activeEntry(nullptr), m_I_manage(flag),
        m_numSent(0) {
        char buffer[100];
