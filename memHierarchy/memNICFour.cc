...
void MemNICFour::build(Params &params) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        if (linkName == "")
            dbg.fatal(CALL_INFO, -1,
                      "Param not specified(%s): %s.port - the name of the port that the MemNIC's %s network is attached to. This should be set internally by components creating the memNIC.\n",
        ...
        std::string linkInbufSize = params.find<std::string>(pref[i] + ".network_input_buffer_size",
                                                             "1KiB");
        std::string linkOutbufSize = params.find<std::string>(
            pref[i] + ".network_output_buffer_size", "1KiB");

        link_control[i] = (SimpleNetwork *) loadSubComponent(
            params.find<std::string>(pref[i] + ".linkcontrol", "kingsley.linkcontrol"), params);
        // But link control doesn't use params so manually initialize
        link_control[i]->initialize(linkName, UnitAlgebra(linkBandwidth), num_vcs,
                                    UnitAlgebra(linkInbufSize), UnitAlgebra(linkOutbufSize));

        // Packet size
        UnitAlgebra packetSize = UnitAlgebra(
            params.find<std::string>(pref[i] + ".min_packet_size", "8B"));
        if (!packetSize.hasUnits("B"))
            dbg.fatal(CALL_INFO, -1,
                      "Invalid param(%s): %s.min_packet_size - must have units of bytes (B). SI units OK. You specified '%s'\n.",
    ...
    link_control[DATA]->setNotifyOnReceive(
        new SimpleNetwork::Handler<MemNICFour>(this, &MemNICFour::recvNotifyData));
    link_control[REQ]->setNotifyOnReceive(
        new SimpleNetwork::Handler<MemNICFour>(this, &MemNICFour::recvNotifyReq));
    link_control[ACK]->setNotifyOnReceive(
        new SimpleNetwork::Handler<MemNICFour>(this, &MemNICFour::recvNotifyAck));
    link_control[FWD]->setNotifyOnReceive(
        new SimpleNetwork::Handler<MemNICFour>(this, &MemNICFour::recvNotifyFwd));
...
bool MemNICFour::clock() {
    if (sendQueue[REQ].empty() && sendQueue[ACK].empty() && sendQueue[FWD].empty() &&
        sendQueue[DATA].empty() && recvQueue.empty())
        return true;
...
#endif
            if (link_control[i]->spaceToSend(0, head->size_in_bits) &&
                link_control[i]->send(head, 0)) {
#ifdef __SST_DEBUG_OUTPUT__
...
void MemNICFour::send(MemEventBase *ev) {
    ...
    if (is_debug_event(ev)) {
        ...
        dbg.debug(_L9_, "%s, memNIC adding to %s send queue: dst: %"
        PRIu64
        ", bits: %zu, cmd: %s\n",
...
void MemNICFour::doRecv(SimpleNetwork::Request *req, NetType net) {
    ...
    if (mre != nullptr) {
        dbg.debug(_L3_, "%s, memNIC received a message: <%"
        PRIu64
        ", %u>\n",
        ...
        if (mre->tag == recvTags[src]) { // Got the tag we were expecting
            ...
            while (orderBuffer[src].find(recvTags[src]) != orderBuffer[src].end()) {
                ...
                stat_orderLatency->addData(
                    getCurrentSimTime() - orderBuffer[src][recvTags[src]].second);
    ...
    if (req != nullptr) {
        ...
        if (mre->hasClientData()) {
            ...
            if (smre->event != nullptr) {
                ...
                ev->setDeliveryLink(smre->getLinkId(), nullptr);
            }
            return smre;
        } else {
            ...
            if (networkAddressMap.find(imre->info.name) == networkAddressMap.end()) {
                dbg.fatal(CALL_INFO, -1,
                          "%s (MemNIC), received information about previously unknown endpoint. This case is not handled. Endpoint name: %s\n",
...
void MemNICFour::printStatus(Output &out) {
    ...
    out.output("      Ordering buffer size: %"
    PRIu64
    "\n", totalOOO);
