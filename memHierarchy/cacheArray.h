...
namespace SST {
    namespace MemHierarchy {
        ...
        public:
            ...
            public:
                DataLine(unsigned int size, int index, Output *dbg) : size_(size), index_(index),
                                                                      dbg_(dbg), dirLine_(nullptr) {
                ...
                void setData(vector <uint8_t> data, uint32_t offset) {
                    ...
                    if (data.size() + offset > size_) { // TODO can we remove this check somehow?
                        dbg_->fatal(CALL_INFO, -1,
                                    "Error: Cacheline write exceeds line size. Size: %"
                        PRIu32
                        ", Offset: %"
                        PRIu32
                        ", Write size: %zu\n",
            ...
            public:
                CacheLine(unsigned int size, int index, Output *dbg, bool cache) : size_(size),
                                                                                   index_(index),
                                                                                   dbg_(dbg),
                                                                                   baseAddr_(0),
                                                                                   state_(I) {
                ...
                std::string getString() {
                    ...
                    str << " Sharers: [";
                    for (std::set<std::string>::iterator it = sharers_.begin();
                         it != sharers_.end(); it++) {
                ...
                bool inTransition() {
                    return (state_ != I) && (state_ != S) && (state_ != E) && (state_ != M) &&
                           (state_ != NP);
                }
                ...
                void removeSharer(std::string name) {
                    ...
                    if (sharers_.find(name) == sharers_.end())
                        dbg_->fatal(CALL_INFO, -1,
                                    "Error: cannot remove sharer '%s', not a current sharer. Addr = 0x%"
                    PRIx64
                ...
                void clearAtomics() {
                    if (userLock_ != 0)
                        dbg_->fatal(CALL_INFO, -1,
                                    "Error: clearing cacheline but userLock is not 0. Address = 0x%"
                    PRIx64
                    "\n", baseAddr_);
                    if (eventsWaitingForLock_)
                        dbg_->fatal(CALL_INFO, -1,
                                    "Error: clearing cacheline but eventsWaitingForLock is true. Address = 0x%"
                    PRIx64
                    "\n", baseAddr_);
                    atomicEnd();
                }
                ...
                void setEventsWaitingForLock(
                    bool eventsWaiting) { eventsWaitingForLock_ = eventsWaiting; }
            ...
            /** Replace cache line */
            virtual void replace(Addr baseAddr, CacheArray::CacheLine *candidate,
                                 CacheArray::DataLine *dataCandidate = nullptr) = 0;
        ...
        protected:
            ...
            CacheArray(Output *dbg, unsigned int numLines, unsigned int associativity,
                       unsigned int lineSize,
                       ReplacementMgr *replacementMgr, HashFunction *hash, bool sharersAware,
                       bool cache) : dbg_(dbg),
                                     numLines_(numLines), associativity_(associativity),
                                     lineSize_(lineSize),
                                     replacementMgr_(replacementMgr), hash_(hash) {
                dbg_->debug(_INFO_,
                            "--------------------------- Initializing [Set Associative Cache Array]... \n");
 ...
 */
        ...
        public:
            ...
            SetAssociativeArray(Output *dbg, unsigned int numLines, unsigned int lineSize,
 ...
 */
        ...
        public:
            DualSetAssociativeArray(Output *dbg, unsigned int lineSize, HashFunction *hf,
                                    bool sharersAware, unsigned int dirNumLines,
                                    unsigned int dirAssociativity, ReplacementMgr *dirRp,
                                    unsigned int cacheNumLines, unsigned int cacheAssociativity,
