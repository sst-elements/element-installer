...
/* Element Library Info */
            SST_ELI_REGISTER_COMPONENT(Scratchpad,
            "memHierarchy", "Scratchpad", SST_ELI_ELEMENT_VERSION(1,0,0),
            "Scratchpad memory", COMPONENT_CATEGORY_MEMORY)

            SST_ELI_DOCUMENT_PARAMS(
            { "clock", "(string) Clock frequency or period with units (Hz or s; SI units OK).", nullptr },
            { "size", "(string) Size of the scratchpad in bytes (B), SI units ok", nullptr },
...
/* Begin class defintion */
        ...
        private:
            ...
            public:
                ...
                OutstandingEvent(MemEventBase *request, MemEventBase *response) : request(request),
                                                                                  response(
                                                                                      response),
                                                                                  remoteWrite(
                                                                                      nullptr),
                ...
                OutstandingEvent(MemEventBase *request, MemEventBase *response, MemEvent *write)
                    : request(request), response(response), remoteWrite(write), count(0) {}
            ...
            public:
                ...
                MSHREntry(SST::Event::id_type id, Command cmd, MemEvent *scratch = nullptr) : id(
                    id), scratch(scratch), cmd(cmd), needData(false), needAck(false) {}

                // For Get events which have not yet started but remote read has been sent
                MSHREntry(SST::Event::id_type id, Command cmd, bool needData) : id(id),
                                                                                scratch(nullptr),
                                                                                cmd(cmd),
                                                                                needData(needData),
                                                                                needAck(false) {}
                ...
                MSHREntry(SST::Event::id_type id, Command cmd, bool needData, bool needAck) : id(
                    id), scratch(nullptr), cmd(cmd), needData(needData), needAck(needAck) {}
