...
/* Element Library Info */
            SST_ELI_REGISTER_SUBCOMPONENT_DERIVED(TimingDRAM,
            "memHierarchy", "timingDRAM", SST_ELI_ELEMENT_VERSION(1,0,0),
            "Moderately-detailed timing model for DRAM", SST::MemHierarchy::SimpleMemBackend)

            SST_ELI_DOCUMENT_PARAMS( MEMBACKEND_ELI_PARAMS,
            /* Own parameters */
            { "id", "ID number for this TimingDRAM instance", nullptr },
...
/* Begin class definition */
        private:
            ...
            public:
                ...
                Bank(ComponentId_t, Params &, unsigned mc, unsigned chan, unsigned rank,
                     unsigned bank, Output *);
...
//        bool                m_busy;
            ...
            public:
                ...
                Cmd(Bank *bank, Op op, unsigned cycles, unsigned row = -1, unsigned dataCycles = 0,
                    Transaction *trans = nullptr) :
                    m_bank(bank), m_op(op), m_cycles(cycles), m_row(row), m_dataCycles(dataCycles),
                    m_trans(trans) {
                    switch (m_op) {
                        case PRE:
                            m_name = "PRE";
            ...
            public:
                ...
                Rank(ComponentId_t, Params &, unsigned mc, unsigned chan, unsigned rank, Output *,
                     AddrMapper *);
                ...
                void pushTrans(Transaction *trans) {
                    ...
                    m_output->verbosePrefix(prefix(), CALL_INFO, 2, DBG_MASK, "bank=%d addr=%#"
                    PRIx64
                    "\n",
                        bank, trans->addr);
            ...
            public:
                ...
                Channel(ComponentId_t, std::function<void(ReqId)>, Params &, unsigned mc,
                        unsigned chan, Output *, AddrMapper *);

                bool issue(SimTime_t createTime, ReqId id, Addr addr, bool isWrite,
                           unsigned numBytes) {
                    ...
                    m_output->verbosePrefix(prefix(), CALL_INFO, 3, DBG_MASK, "reqId=%"
                    PRIu64
                    " rank=%d addr=%#"
                    PRIx64
                    ", createTime=%"
                    ...
                    Transaction *trans = new Transaction(createTime, id, addr, isWrite, numBytes,
                                                         m_mapper->getBank(addr),
        ...
        public:
            ...
            void handleResponse(ReqId id) {
                output->verbose(CALL_INFO, 2, DBG_MASK, "req=%"
                PRIu64
                "\n", id );
