...
namespace SST {
    namespace MemHierarchy {
        namespace Backend {
            ...
            public:
                BackingMMAP(std::string memoryFile, size_t size, size_t offset = 0)
                    : Backing(), m_fd(-1), m_size(size), m_offset(offset) {
                ...
                BackingMalloc(size_t size) {
                    ...
                    if (!isPowerOfTwo(m_allocUnit)) {
                        ...
                        out.fatal(CALL_INFO, -1,
                                  "BackingMalloc: Error - size must be a power of two. Got: %zu\n",
                                  size);
                    }
