...
GOBLINHMCSimBackend::GOBLINHMCSimBackend(Component *comp, Params &params) : ExtMemBackend(comp,
                                                                                          params) {
    build(params);
}

GOBLINHMCSimBackend::GOBLINHMCSimBackend(ComponentId_t id, Params &params) : ExtMemBackend(id,
                                                                                           params) {
    build(params);
}
...
void GOBLINHMCSimBackend::build(Params &params) {
    ...
    for (uint32_t i = 0; i < hmc_link_count; ++i) {
        output->verbose(CALL_INFO, 1, 0, "Configuring link: %"
        PRIu32
        "...\n", i);
    ...
    output->verbose(CALL_INFO, 1, 0,
                    "Populating tag entries allowed at the controller, max tag count is: %"
    PRIu32
    "\n", hmc_tag_count);
    ...
    if (nullptr == hmc_trace_file_handle) {
    ...
    output->verbose(CALL_INFO, 1, 0, "Setting the trace level of the HMC to %"
    PRIu32
    "\n", hmc_trace_level);
    rc = hmcsim_trace_level(&the_hmc, hmc_trace_level);

    if (rc > 0) {
        output->fatal(CALL_INFO, -1, "Unable to set the HMC trace level to %"
        PRIu32
        "\n", hmc_trace_level);
    } else {
    ...
    output->verbose(CALL_INFO, 1, 0, "Setting the maximum HMC request size to: %"
    PRIu32
    "\n", hmc_max_req_size);
    rc = hmcsim_util_set_all_max_blocksize(&the_hmc, hmc_max_req_size);

    if (rc > 0) {
        output->fatal(CALL_INFO, -1, "Unable to set maximum HMC request size to %"
        PRIu32
        "\n", hmc_max_req_size);
    } else {
    ...
    if (rc > 0) {
        output->fatal(CALL_INFO, -1,
                      "Unable to write trace header information into the HMC trace file.\n");
    } else {
...
bool GOBLINHMCSimBackend::issueMappedRequest(ReqId reqId, Addr addr, bool isWrite,
    ...
    // if our MapCmd is nullptr, then nothing was found
    if (MapCmd == nullptr) {
        return false;
    }
    ...
    output->verbose(CALL_INFO, 8, 0, "Issuing mapped request: %"
    PRIu64
    " %s tag: %"
    PRIu16
    " dev: %"
    ...
    if (rc > 0) {
        output->fatal(CALL_INFO, -1, "Unable to build a mapped request for address: %"
        PRIu64
        "\n", addr);
    }
    ...
    if (HMC_STALL == rc) {
        ...
        *Success = false;
        ...
        output->verbose(CALL_INFO, 4, 0, "Issue of mapped request for address 0X%"
        PRIx64
        " successfully accepted by HMC.\n", addr);
        ...
        *Success = false;
        output->fatal(CALL_INFO, -1, "Error issue request for address %"
        PRIu64
        " into HMC on link %"
        PRIu8
        "\n", addr, req_link);
...
bool GOBLINHMCSimBackend::issueCustomRequest(ReqId reqId, Addr addr, uint32_t cmd,
    ...
    output->verbose(CALL_INFO, 8, 0,
                    "Received custom request of opcode %"
    PRIu32
    " of size: %"
    PRIu32
    "\n",
    ...
    output->verbose(CALL_INFO, 4, 0, "Queue front is: %"
    PRIu16
    "\n", tag_queue.front());
    ...
    if (rc > 0) {
        output->fatal(CALL_INFO, -1, "Unable to build a custom request for address: %"
        PRIu64
        "\n", addr);
    }
    ...
    if (HMC_STALL == rc) {
        ...
        output->verbose(CALL_INFO, 4, 0,
                        "Issue of custom request for %s at address 0x%"
        PRIx64
        " successfully accepted by HMC.\n",
            name.c_str(), addr);
        ...
        output->fatal(CALL_INFO, -1,
                      "Error issue request for %s at address %"
        PRIu64
        " into HMC on link %"
        PRIu8
        "\n",
...
bool GOBLINHMCSimBackend::issueRequest(ReqId reqId, Addr addr, bool isWrite,
    ...
    output->verbose(CALL_INFO, 8, 0, "Received request of size: %"
    PRIu32
    " of type %s\n",
    ...
    output->verbose(CALL_INFO, 8, 0, "Issuing request: %"
    PRIu64
    " %s tag: %"
    PRIu16
    " dev: %"
    ...
    if (rc > 0) {
        output->fatal(CALL_INFO, -1, "Unable to build a request for address: %"
        PRIu64
        "\n", addr);
    }
    ...
    if (HMC_STALL == rc) {
        ...
        if (isPosted) {
            output->verbose(CALL_INFO, 4, 0, "Issue of posted request %s for address 0X%"
            PRIx64
            " successfully accepted by HMC.\n", RqstStr.c_str(), addr);
        } else {
            output->verbose(CALL_INFO, 4, 0, "Issue of request %s for address 0X%"
            PRIx64
            " successfully accepted by HMC.\n", RqstStr.c_str(), addr);
        }
...
void GOBLINHMCSimBackend::processResponses() {
    ...
    for (uint32_t i = 0; i < (uint32_t) the_hmc.num_links; ++i) {
        ...
        while (rc != HMC_STALL) {
            ...
            if (HMC_OK == rc) {
                ...
                if (HMC_OK == decode_rc) {
                    output->verbose(CALL_INFO, 4, 0,
                                    "Successfully decoded an HMC memory response for tag: %"
                    PRIu16
                    "\n", resp_tag);

                    std::map<uint16_t, HMCSimBackEndReq *>::iterator locate_tag = tag_req_map.find(
                        resp_tag);

                    if (locate_tag == tag_req_map.end()) {
                        output->fatal(CALL_INFO, -1, "Unable to find tag: %"
                        PRIu16
                        " in the tag/request lookup table.\n", resp_tag);
                    } else {
                        HMCSimBackEndReq *matchedReq = locate_tag->second;

                        output->verbose(CALL_INFO, 4, 0, "Matched tag %"
                        PRIu16
                        " to request for address: 0x%"
                        PRIx64
                        ", processing time: %"
                        PRIu64
                        "ns\n",
                    ...
                    output->verbose(CALL_INFO, 8, 0,
                                    "Attempted to decode an HMC memory request but HMC returns stall.\n");
                } else {
                    fflush(hmc_trace_file_handle);
                    output->fatal(CALL_INFO, -1,
                                  "Error: call to decode an HMC sim memory response returned an error code (code=%d, possible tag=%"
                    PRIu16
                    ")\n", decode_rc, resp_tag);
                }
            } else if (HMC_STALL == rc) {
                output->verbose(CALL_INFO, 8, 0,
                                "Call to HMC simulator recv returns a stall, no requests in flight.\n");
            } else {
                output->fatal(CALL_INFO, -1,
                              "Error attempting to call a recv pool on the HMC simulator.\n");
            }
        }
    }
    ...
    for (it = tag_req_map.begin(); it != tag_req_map.end();) {
        ...
        if (!matchedReq->hasResponse()) {
            ...
            output->verbose(CALL_INFO, 4, 0, "Handling posted memory response for tag: %"
            PRIu16
            "\n", resp_tag);
            handleMemResponse(matchedReq->getRequest(), flags);
...
void GOBLINHMCSimBackend::printPendingRequests() {
    ...
    for (pending_itr = tag_req_map.begin(); pending_itr != tag_req_map.end(); pending_itr++) {
        output->verbose(CALL_INFO, 8, 0, "Tag: %8"
        PRIu16
        " for address 0X%"
        PRIx64
        "\n",
