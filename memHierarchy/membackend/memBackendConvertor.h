...
#define MEMBACKENDCONVERTOR_ELI_SLOTS {"backend", "Backend memory model", "SST::MemHierarchy::MemBackend"}
            ...
            public:
                ...
                std::string getString() {
                    std::ostringstream str;
                    str << "addr: " << addr() << " baseAddr: " << baseAddr() << " processed: "
                        << processed();
                    str << " isWrite: " << isWrite();
        ...
        public:
            ...
            virtual void setCallbackHandlers(
                std::function<void(Event::id_type, uint32_t)> responseCB,
                std::function<Cycle_t()> clockenableCB);
        ...
        private:
            ...
            bool setupMemReq(MemEvent *ev) {
                if (Command::FlushLine == ev->getCmd() || Command::FlushLineInv == ev->getCmd()) {
                    ...
                    for (std::deque<BaseReq *>::iterator it = m_requestQueue.begin();
                        ...
                        if (mr->baseAddr() == ev->getBaseAddr()) {
                            ...
                            if (m_dependentRequests.find(req->getID()) ==
                                m_dependentRequests.end()) {
            ...
            void(Event::id_type
            id, uint32_t)>
            m_notifyResponse; // notify parent of response
            ...
            std::map<MemEvent *, std::set < SST::Event::id_type> >
            m_waitingFlushes; // Set of request IDs for each flush
