...
HBMDRAMSimMemory::HBMDRAMSimMemory(Component *comp, Params &params) : SimpleMemBackend(comp,
                                                                                       params) {
    build(params);
}

HBMDRAMSimMemory::HBMDRAMSimMemory(ComponentId_t id, Params &params) : SimpleMemBackend(id,
                                                                                        params) {
    build(params);
}
...
void HBMDRAMSimMemory::build(Params &params) {
    ...
    if (ramSize.getRoundedValue() % (1024 * 1024) != 0) {
        output->fatal(CALL_INFO, -1,
                      "For HBMDRAMSim, backend.mem_size must be a multiple of 1MiB. Note: for units in base-10 use 'MB', for base-2 use 'MiB'. You specified '%s'\n",
...
#endif
    if (reqs.size() == 0)
        output->fatal(CALL_INFO, -1, "Error: reqs.size() is 0 at HBMDRAMSimMemory done\n");
