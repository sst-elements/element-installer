...
const MemEvent::id_type DirectoryController::DirEntry::NO_LAST_REQUEST = std::make_pair(
    (uint64_t) - 1, -1);
...
DirectoryController::DirectoryController(ComponentId_t id, Params &params) :
    ...
    if (found) {
        out.output(
            "%s, ** Found deprecated parameter: network_num_vc ** MemHierarchy does not use multiple virtual channels. Remove this parameter from your input deck to eliminate this message.\n",
            getName().c_str());
    }
    ...
    if (!found)
        addrRangeEnd = params.find<uint64_t>("memNIC.addr_range_end", (uint64_t) - 1, found);
    if (!found)
        addrRangeEnd = params.find<uint64_t>("memlink.addr_range_end", (uint64_t) - 1, found);
    ...
    if (!packetSize.hasUnits("B"))
        dbg.fatal(CALL_INFO, -1,
                  "%s, Invalid param: min_packet_size - must have units of bytes (B). SI units are ok. You specified '%s'\n",
    ...
    if (mshrSize < 1)
        dbg.fatal(CALL_INFO, -1,
                  "Invalid param(%s): mshr_num_entries - must be at least 1 or else -1 to indicate a very large MSHR\n",
                  getName().c_str());
    ...
    if (protstr == "mesi" || protstr == "MESI") protocol = CoherenceProtocol::MESI;
        ...
        dbg.fatal(CALL_INFO, -1,
                  "Invalid param(%s): coherence_protocol - must be 'MESI' or 'MSI'. You specified: %s\n",
    ...
    if (!UnitAlgebra(ilSize).hasUnits("B") || interleaveSize % cacheLineSize != 0) {
        dbg.fatal(CALL_INFO, -1,
                  "Invalid param(%s): interleave_size - must be specified in bytes with units (SI units OK) and must also be a multiple of cache_line_size. This definition has CHANGED. Example: If you used to set this to '1', change it to '1KiB'. You specified %s\n",
                  getName().c_str(), ilSize.c_str());
    }
    if (!UnitAlgebra(ilStep).hasUnits("B") || interleaveStep % cacheLineSize != 0) {
        dbg.fatal(CALL_INFO, -1,
                  "Invalid param(%s): interleave_step - must be specified in bytes with units (SI units OK) and must also be a multiple of cache_line_size. This definition has CHANGED. Example: If you used to set this to '4', change it to '4KiB'. You specified %s\n",
    ...
    if (fixupParam(params, "network_bw", "memNIC.network_bw"))
        out.output(CALL_INFO,
                   "Note (%s): Changed 'network_bw' to 'memNIC.network_bw' in params. Change your input file to remove this notice.\n",
                   getName().c_str());
    if (fixupParam(params, "network_input_buffer_size", "memNIC.network_input_buffer_size"))
        out.output(CALL_INFO,
                   "Note (%s): Changed 'network_input_buffer_size' to 'memNIC.network_input_buffer_size' in params. Change your input file to remove this notice.\n",
                   getName().c_str());
    if (fixupParam(params, "network_output_buffer_size", "memNIC.network_output_buffer_size"))
        out.output(CALL_INFO,
                   "Note (%s): Changed 'network_output_buffer_size' to 'memNIC.network_output_buffer_size' in params. Change your input file to remove this notice.\n",
                   getName().c_str());
    if (fixupParam(params, "addr_range_start", "memNIC.addr_range_start"))
        out.output(CALL_INFO,
                   "Note (%s): Changed 'addr_range_start' to 'memNIC.addr_range_start' in params. Change your input file to remove this notice.\n",
                   getName().c_str());
    if (fixupParam(params, "addr_range_end", "memNIC.addr_range_end"))
        out.output(CALL_INFO,
                   "Note (%s): Changed 'addr_range_end' to 'memNIC.addr_range_end' in params. Change your input file to remove this notice.\n",
                   getName().c_str());
    if (fixupParam(params, "interleave_size", "memNIC.interleave_size"))
        out.output(CALL_INFO,
                   "Note (%s): Changed 'interleave_size' to 'memNIC.interleave_size' in params. Change your input file to remove this notice.\n",
                   getName().c_str());
    if (fixupParam(params, "interleave_step", "memNIC.interleave_step"))
        out.output(CALL_INFO,
                   "Note (%s): Changed 'interleave_step' to 'memNIC.interleave_step' in params. Change your input file to remove this notice.\n",
                   getName().c_str());
    if (fixupParam(params, "min_packet_size", "memNIC.min_packet_size"))
        out.output(CALL_INFO,
                   "Note (%s): Changed 'min_packet_size' to 'memNIC.min_packet_size' in params. Change your input file to remove this notice.\n",
                   getName().c_str());
    ...
    if (portCount == 4) {
        ...
        network = loadAnonymousSubComponent<MemLinkBase>("memHierarchy.MemNICFour", "cpulink", 0,
                                                         ComponentInfo::SHARE_PORTS |
                                                         ComponentInfo::INSERT_STATS, nicParams);
    } else {
        nicParams.insert("port", "network");
        network = loadAnonymousSubComponent<MemLinkBase>("memHierarchy.MemNIC", "cpulink", 0,
                                                         ComponentInfo::SHARE_PORTS |
                                                         ComponentInfo::INSERT_STATS, nicParams);
    }

    network->setRecvHandler(
        new Event::Handler<DirectoryController>(this, &DirectoryController::handlePacket));
    ...
    if (isPortConnected("memory")) {
        ...
        memLink = loadAnonymousSubComponent<MemLinkBase>("memHierarchy.MemLink", "memlink", 0,
                                                         ComponentInfo::SHARE_PORTS |
                                                         ComponentInfo::INSERT_STATS, memParams);
        memLink->setRecvHandler(
            new Event::Handler<DirectoryController>(this, &DirectoryController::handlePacket));
        if (!memLink) {
            dbg.fatal(CALL_INFO, -1,
                      "%s, Error creating link to memory from directory controller\n",
                      getName().c_str());
        }
        memoryName = "";
    } else {
        ...
        if (memoryName == "")
            dbg.fatal(CALL_INFO, -1,
                      "Param not specified(%s): net_memory_name - name of the memory owned by this directory controller. If you did not intend to connect to memory over the network, please connect memory to the 'memory' port and ignore this parameter.\n",
                      getName().c_str());
        memLink = nullptr;
...
DirectoryController::~DirectoryController() {
    for (std::unordered_map<Addr, DirEntry *>::iterator i = directory.begin();
         i != directory.end(); ++i) {
        delete i->second;
    }
    directory.clear();
...
void DirectoryController::handlePacket(SST::Event *event) {
    ...
    if (MemEventTypeArr[(int) evb->getCmd()] != MemEventType::Cache ||
        evb->queryFlag(MemEvent::F_NONCACHEABLE)) {
    ...
    if (ev->getCmd() == Command::GetSResp || ev->getCmd() == Command::GetXResp ||
        ev->getCmd() == Command::FlushLineResp
        || ev->getCmd() == Command::ForceInv || ev->getCmd() == Command::FetchInv ||
...
void DirectoryController::processPacket(MemEvent *ev, bool replay) {
    ...
    if (!isRequestAddressValid(ev->getAddr())) {
        dbg.fatal(CALL_INFO, -1, "%s, Error: Request address is not valid. Event: %s. Time = %"
        PRIu64
        "ns\n",
    ...
    switch (cmd) {
        ...
        case Command::ForceInv:
            ...
            break;
        default:
            dbg.fatal(CALL_INFO, -1, "%s, Error: Received unrecognized request: %s. Time = %"
            PRIu64
            "ns\n",
                getName().c_str(), ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
...
void DirectoryController::handleNoncacheableResponse(MemEventBase *ev) {
    if (noncacheMemReqs.find(ev->getID()) == noncacheMemReqs.end()) {
        dbg.fatal(CALL_INFO, -1,
                  "%s, Error: Received a noncacheable response that does not match a pending request. Event: %s\n. Time: %"
        PRIu64
        "ns\n",
            getName().c_str(), ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
...
void DirectoryController::handleGetS(MemEvent *ev, bool replay) {
    ...
    if (!entry->isCached()) {
        ...
        if (is_debug_addr(entry->getBaseAddr())) dbg.debug(_L6_, "Entry %"
        PRIx64
        " not in cache.  Requesting from memory.\n", entry->getBaseAddr());
...
void DirectoryController::handleGetX(MemEvent *ev, bool replay) {
    ...
    switch (state) {
        case I:
            ...
            break;
        case S:
            if (entry->getSharerCount() == 1 &&
                entry->isSharer(node_id(ev->getSrc()))) {   // Special case: upgrade
                mshr->removeFront(ev->getBaseAddr());

                if (is_debug_event(ev)) dbg.debug(_L10_, "\t%s\tMSHR remove event <%s, %"
                PRIx64
                ...
                if (is_debug_event(ev)) {
                    dbg.debug(_L4_, "Sending response for 0x%"
                    PRIx64
                    " to %s, send time: %"
                    PRIu64
                    "\n", entry->getBaseAddr(), respEv->getDst().c_str(), timestamp +
                                                                          accessLatency);
                }
...
void DirectoryController::handlePutS(MemEvent *ev) {
    ...
    switch (state) {
        ...
        default:
            dbg.fatal(CALL_INFO, -1,
                      "%s, Error: Directory received PutS but state is %s. Event = %s. Time = %"
            PRIu64
            "ns\n",
...
void DirectoryController::handlePutE(MemEvent *ev) {
    ...
    if (!((uint32_t) entry->getOwner() == node_name_to_id(ev->getSrc()))) {
        dbg.fatal(CALL_INFO, -1,
                  "%s, Error: received PutE from a node who does not own the block. Event = %s. Time = %"
        PRIu64
        "ns\n",
            getName().c_str(), ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
    ...
    if (!(entry->isCached())) {
        ...
        if (is_debug_event(ev)) dbg.debug(_L6_, "Entry %"
        PRIx64
        " not in cache.  Requesting from memory.\n", entry->getBaseAddr());
    ...
    switch (state) {
        ...
        default:
            dbg.fatal(CALL_INFO, -1,
                      "%s, Error: Directory received PutE but state is %s. Event: %s. Time = %"
            PRIu64
            "ns\n",
                getName().c_str(), StateString[state], ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
...
void DirectoryController::handlePutM(MemEvent *ev) {
    ...
    if (!((uint32_t) entry->getOwner() == node_name_to_id(ev->getSrc()))) {
        dbg.fatal(CALL_INFO, -1,
                  "%s, Error: received PutM from a node who does not own the block. Event: %s. Time = %"
        PRIu64
        "ns\n",
            getName().c_str(), ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
    ...
    if (!(entry->isCached())) {
        if (!(mshr->elementIsHit(ev->getBaseAddr(), ev))) {
            ...
            if (is_debug_event(ev)) {
                dbg.debug(_L8_, "Inserting request in mshr. Cmd = %s, BaseAddr = 0x%"
                PRIx64
                ", Addr = 0x%"
                PRIx64
    ...
    switch (state) {
        ...
        default:
            dbg.fatal(CALL_INFO, -1,
                      "%s, Error: Directory received PutM but state is %s. Event: %s. Time = %"
            PRIu64
            "ns, %"
            PRIu64
            " cycles.\n",
...
void DirectoryController::handleFlushLine(MemEvent *ev) {
    ...
    switch (state) {
        ...
        default:
            dbg.fatal(CALL_INFO, -1,
                      "%s, Error: Directory received FlushLine but state is %s. Event: %s. Time = %"
            PRIu64
            "ns\n",
                getName().c_str(), StateString[state], ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
...
void DirectoryController::handleFlushLineInv(MemEvent *ev) {
    ...
    switch (state) {
        ...
        default:
            dbg.fatal(CALL_INFO, -1,
                      "%s, Error: Directory received FlushLineInv but state is %s. Event: %s. Time = %"
            PRIu64
            "ns\n",
                getName().c_str(), StateString[state], ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
...
void DirectoryController::handleFetchResp(MemEvent *ev, bool keepEvent) {
    ...
    if (is_debug_event(ev))
        dbg.debug(_L4_, "Finishing Fetch for reqEv = %s.\n", reqEv->getBriefString().c_str());

    /* Error checking */
    if (!((uint32_t) entry->getOwner() == node_name_to_id(ev->getSrc()))) {
        dbg.fatal(CALL_INFO, -1,
                  "%s, Error: received FetchResp from a node who does not own the block. Event: %s. Time = %"
        PRIu64
        "ns, %"
        PRIu64
        " cycles\n",
    ...
    switch (state) {
        case M_Inv:
            if (reqEv->getCmd() != Command::FetchInv &&
                reqEv->getCmd() != Command::ForceInv) { // GetX request, not back invalidation
                writebackData(ev, Command::PutM);
        ...
        default:
            dbg.fatal(CALL_INFO, -1,
                      "%s, Error: Directory received %s but state is %s. Event: %s. Time = %"
            PRIu64
            "ns, %"
            PRIu64
            " cycles\n",
...
void DirectoryController::handleAckInv(MemEvent *ev) {
    ...
    switch (state) {
        ...
        default:
            dbg.fatal(CALL_INFO, -1,
                      "%s, Error: Directory received AckInv but state is %s. Event = %s. Time = %"
            PRIu64
            "ns\n",
                getName().c_str(), StateString[state], ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
    ...
    if (!(mshr->elementIsHit(ev->getBaseAddr(), ev))) {
        ...
        if (conflict) {
            if (mshr->exists(ev->getBaseAddr())) { // front entry is an event
                ...
                if (entry->getState() == IS || entry->getState() == IM ||
                    entry->getState() == S_D) {
...
void DirectoryController::handleMemoryResponse(SST::Event *event) {
    ...
    if (ev->queryFlag(MemEvent::F_NONCACHEABLE)) {
        ...
        dbg.fatal(CALL_INFO, -1,
                  "%s, Error: Received unexpected noncacheable response from memory. %s. Time = %"
        PRIu64
        "ns\n",
            getName().c_str(), ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
    ...
    if (ev->getCmd() == Command::ForceInv || ev->getCmd() == Command::FetchInv) {
        ...
        replayWaitingEvents(ev->getBaseAddr());
    } else if (ev->getBaseAddr() == 0 && dirEntryMiss.find(ev->getResponseToID()) !=
                                         dirEntryMiss.end()) {    // directory entry miss
        ...
        dbg.fatal(CALL_INFO, -1,
                  "%s, Error: Received unexpected response from memory - matching request not found. %s. Time = %"
        PRIu64
        "ns\n",
            getName().c_str(), ev->getVerboseString().c_str(), getCurrentSimTimeNano());
    }
...
void DirectoryController::handleDataResponse(MemEvent *ev) {
    ...
    switch (state) {
        ...
        case SM:
            ...
            break;
        default:
            dbg.fatal(CALL_INFO, 1, "Directory %s received Get Response for addr 0x%"
            PRIx64
            " but state is %s. Event: %s\n",
    ...
    if (is_debug_event(ev)) {
        dbg.debug(_L4_, "\tSending requested data for 0x%"
        PRIx64
...
void DirectoryController::handleDirEntryMemoryResponse(MemEvent *ev) {
    ...
    switch (st) {
        ...
        default:
            dbg.fatal(CALL_INFO, -1,
                      "Directory Controller %s: DirEntry response received for addr 0x%"
            PRIx64
            " but state is %s. Event: %s\n",
...
void DirectoryController::issueInvalidates(MemEvent *ev, DirEntry *entry, Command cmd) {
    ...
    if (is_debug_addr(entry->getBaseAddr()))
        dbg.debug(_L4_, "Sending Invalidates to fulfill request for exclusive, BsAddr = %"
    PRIx64
    ".\n", entry->getBaseAddr());
}
...
void DirectoryController::issueMemoryRequest(MemEvent *ev, DirEntry *entry) {
    ...
    if (is_debug_addr(entry->getBaseAddr())) {
        dbg.debug(_L5_, "\tRequesting data from memory.  Cmd = %s, BaseAddr = x%"
        PRIx64
        ", Size = %u, noncacheable = %s\n",
            CommandString[(int) reqEv->getCmd()], reqEv->getBaseAddr(), reqEv->getSize(), reqEv->queryFlag(
            MemEvent::F_NONCACHEABLE) ? "true" : "false");
    }
}
...
void DirectoryController::getDirEntryFromMemory(DirEntry *entry) {
    ...
    switch (st) {
        ...
        default:
            dbg.fatal(CALL_INFO, -1, "Direcctory Controller %s: cache miss for addr 0x%"
            PRIx64
    ...
    if (is_debug_addr(entry->getBaseAddr())) dbg.debug(_L10_, "Requesting Entry from memory for 0x%"
    PRIx64
    "(%"
    PRIu64
...
void DirectoryController::printStatus(Output &statusOut) {
    ...
    statusOut.output("  Cached entries: %"
    PRIu64
    "\n", entryCacheSize);
    statusOut.output("  Requests waiting to be handled:  %zu\n", workQueue.size());
    for (std::list < std::pair < MemEvent * , bool > > ::iterator i = workQueue.begin(); i !=
                                                                                         workQueue.end();
    ++i){
        statusOut.output("    %s, %s\n", i->first->getVerboseString().c_str(),
                         i->second ? "replay" : "new");
    }
    ...
    for (std::unordered_map<Addr, DirEntry *>::iterator it = directory.begin();
         it != directory.end(); it++) {
        statusOut.output("    0x%"
        PRIx64
...
DirectoryController::DirEntry *DirectoryController::getDirEntry(Addr baseAddr) {
    ...
    if (directory.end() == i) {
        ...
        entry->setCached(
            true);   // TODO fix this so new entries go to memory if we're caching, little bit o cheatin here
    } else {
        entry = i->second;
    }
    return entry;
...
void DirectoryController::sendInvalidate(int target, MemEvent *reqEv, DirEntry *entry,
                                         Command cmd) {
    MemEvent *me = new MemEvent(getName(), entry->getBaseAddr(), entry->getBaseAddr(), cmd,
                                cacheLineSize, getCurrentSimTimeNano());
    me->setDst(nodeid_to_name[target]);
    me->setRqstr(reqEv->getRqstr());

    if (is_debug_event(reqEv))
...
void DirectoryController::forwardFlushRequest(MemEvent *event) {
    MemEvent *reqEv = new MemEvent(getName(), event->getAddr(), event->getBaseAddr(),
                                   Command::FlushLine, cacheLineSize, getCurrentSimTimeNano());
    ...
    if (is_debug_event(event)) {
        dbg.debug(_L5_, "Forwarding FlushLine to memory. BaseAddr = x%"
        PRIx64
        ", Size = %u, noncacheable = %s\n",
            reqEv->getBaseAddr(), reqEv->getSize(), reqEv->queryFlag(MemEvent::F_NONCACHEABLE)
                                                    ? "true" : "false");
    }
}
...
uint32_t DirectoryController::node_name_to_id(const std::string &name) {
    ...
    if (node_lookup.end() == i) {
        dbg.fatal(CALL_INFO, -1,
                  "%s, Error: Attempt to lookup node ID but name not found: %s. Time = %"
        PRIu64
        "ns\n",
...
void DirectoryController::updateCache(DirEntry *entry) {
    if (0 == entryCacheMaxSize) {
        ...
        if (entry->getState() == I) {
            if (is_debug_addr(entry->getBaseAddr())) dbg.debug(_L10_, "Entry for 0x%"
            PRIx64
            " has no references - purging\n", entry->getBaseAddr());
            ...
            while (entryCacheSize > entryCacheMaxSize) {
                ...
                if (is_debug_addr(entry->getBaseAddr()))
                    dbg.debug(_L10_, "entryCache too large.  Evicting entry for 0x%"
                PRIx64
...
void DirectoryController::sendEntryToMemory(DirEntry *entry) {
    ...
    MemEvent *me = new MemEvent(getName(), entryAddr, entryAddr, Command::PutE, cacheLineSize,
                                getCurrentSimTimeNano()); // MemController discards PutE's without writeback so this is safe
    me->setSize(entrySize);
    profileRequestSent(me);
...
MemEvent::id_type DirectoryController::writebackData(MemEvent *data_event, Command wbCmd) {
    MemEvent *ev = new MemEvent(getName(), data_event->getBaseAddr(), data_event->getBaseAddr(),
                                wbCmd, cacheLineSize, getCurrentSimTimeNano());

    if (data_event->getPayload().size() != cacheLineSize) {
        dbg.fatal(CALL_INFO, -1,
                  "%s, Error: Writing back data request but payload does not match cache line size of %uB. Event: %s. Time = %"
        PRIu64
        "ns\n",
    ...
    if (mshr->isHit(addr)) {
        ...
        if (replayEntries->begin()->elem.isEvent()) {
            ...
            if (is_debug_addr(addr))
                dbg.debug(_L5_, "\tReactivating event. %s\n", ev->getBriefString().c_str());
...
void DirectoryController::init(unsigned int phase) {
    ...
    // InitData: Name, nullptrCMD, Endpoint type, inclusive of all upper levels, will send writeback acks, line size
    if (!phase) {
        ...
        if (memLink) {
            memLink->sendInitData(
                new MemEventInitCoherence(getName(), Endpoint::Directory, true, true, cacheLineSize,
                                          true));
        }
        // Announce to network we're here
        network->sendInitData(
            new MemEventInitCoherence(getName(), Endpoint::Directory, true, true, cacheLineSize,
                                      true));
    }
    ...
    while (MemEventInit *ev = network->recvInitData()) {
        if (ev->getCmd() == Command::nullptrCMD) {
            ...
            if (!memLink && !network->isDest(memoryName)) {
                dbg.fatal(CALL_INFO, -1,
                          "%s, Invalid param: net_memory_name - must name a valid memory component in the system. You specified: %s\n",
                          getName().c_str(), memoryName.c_str());
            }

            dbg.debug(_L10_, "Found Init Info for address 0x%"
            PRIx64
            "\n", ev->getAddr());
            if (isRequestAddressValid(ev->getAddr())) {
                dbg.debug(_L10_, "Sending Init Data for address 0x%"
                PRIx64
                " to memory\n", ev->getAddr());
                ev->setDst(memoryName);
                if (memLink) {
    ...
    if (memLink) {
        while ((ev = memLink->recvInitData())) {
            ...
            if (initEv && initEv->getCmd() == Command::nullptrCMD) {
                dbg.debug(_L10_, "%s received init event: %s\n",
...
void DirectoryController::setup(void) {
    ...
    if (0 == numTargets)
        dbg.fatal(CALL_INFO, -1, "%s, Error: Did not find any caches during init\n",
                  getName().c_str());
