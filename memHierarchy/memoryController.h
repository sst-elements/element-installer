...
/* Element Library Info */
            SST_ELI_REGISTER_COMPONENT(MemController,
            "memHierarchy", "MemController", SST_ELI_ELEMENT_VERSION(1,0,0),
            "Memory controller, interfaces to a main memory model for timing", COMPONENT_CATEGORY_MEMORY)

#define MEMCONTROLLER_ELI_PARAMS {"backend.mem_size",    "(string) Size of physical memory. NEW REQUIREMENT: must include units in 'B' (SI ok). Simple fix: add 'MiB' to old value.", nullptr},\
            {"clock",               "(string) Clock frequency of controller", nullptr},\
            ...
            {"network_bw",          "MOVED. Now a member of the MemNIC subcomponent.", nullptr}, /* Remove 9.0 */\
        ...
        protected:
            ...
            void notifyListeners(MemEvent *ev) {
                if (!listeners_.empty()) {
                    ...
                    CacheListenerNotification notify(ev->getAddr(), ev->getAddr(),
                                                     ev->getVirtualAddress(),
                                                     ev->getInstructionPointer(), ev->getSize(),
                                                     READ, HIT);
