...
void Cache::profileEvent(MemEvent *event, Command cmd, bool replay, bool canStall) {
    ...
    if (mshr_->isHit(event->getBaseAddr()) && canStall)
        return;   // will block this event, profile it later
...
bool Cache::processEvent(MemEventBase *ev, bool replay) {
    ...
    if (is_debug_event(ev)) {
        if (replay) {
            d_->debug(_L3_, "Replay. Name: %s. Cycles: %"
            PRIu64
            ". Event: (%s)\n", this->getName().c_str(), timestamp_, ev->getVerboseString().c_str());
        } else {
            d2_->debug(_L3_,
                       "\n\n-------------------------------------------------------------------------------------------------------------------------------------------------------------------\n");
            d_->debug(_L3_, "New Event. Name: %s. Cycles: %"
            PRIu64
            ", Event: (%s)\n", this->getName().c_str(), timestamp_, ev->getVerboseString().c_str());
        }
        cout << flush;
    ...
    if (baseAddr % cacheArray_->getLineSize() != 0) {
        out_->fatal(CALL_INFO, -1, "%s, Base address is not a multiple of line size! Line size: %"
        PRIu64
    ...
    if (bankStatus_.size() > 0) {
        ...
        if (bankStatus_[bank]) { // bank conflict
            if (is_debug_event(event))
                d_->debug(_L3_, "Bank conflict on bank %"
            PRIu64
            "\n", bank);
            ...
            d_->debug(_L3_, "No bank conflict, setting bank %"
            PRIu64
            " to busy\n", bank);
    ...
    switch (cmd) {
        ...
        case Command::GetSX:
            ...
            if (mshr_->isHit(baseAddr) && canStall) {
                ...
                if (processRequestInMSHR(baseAddr, event)) {
                    if (is_debug_addr(baseAddr))
                        d_->debug(_L9_,
                                  "Added event to MSHR queue.  Wait till blocking event completes to proceed with this event.\n");
        ...
        case Command::FlushLineInv:
            ...
            break;
        default:
            out_->fatal(CALL_INFO, -1, "%s, Command not supported. Time = %"
            PRIu64
            "ns, Event = %s", getName().c_str(), getCurrentSimTimeNano(), event->getVerboseString().c_str());
    }
    return true;
}
...
void Cache::processNoncacheable(MemEventBase *event) {
    if (CommandCPUSide[(int) event->getCmd()]) {
        ...
        std::map<SST::Event::id_type, std::string>::iterator it = responseDst_.find(
            event->getResponseToID());
        if (it == responseDst_.end()) {
            out_->fatal(CALL_INFO, 01,
                        "%s, Error: noncacheable response received does not match a request. Event: (%s). Time: %"
            PRIu64
            "\n",
...
void Cache::processPrefetchEvent(SST::Event *ev) {
    ...
    if (requestsThisCycle_ != maxRequestsPerCycle_) {
        if (event->getCmd() != Command::nullptrCMD && mshr_->getSize() < dropPrefetchLevel_ &&
            mshr_->getPrefetchCount() < maxOutstandingPrefetch_) {
            requestsThisCycle_++;
...
void Cache::init(unsigned int phase) {
    if (linkUp_ == linkDown_) {
        ...
        if (!phase)
            linkDown_->sendInitData(
                new MemEventInitCoherence(getName(), Endpoint::Cache, type_ == "inclusive",
                                          type_ != "inclusive", cacheArray_->getLineSize(), true));

        /*  */
        while (MemEventInit *event = linkDown_->recvInitData()) {
            if (event->getCmd() == Command::nullptrCMD) {
    ...
    if (!phase) {
        // MemEventInit: Name, nullptrCMD, Endpoint type, inclusive of all upper levels, will send writeback acks, line size
    ...
    while (MemEventInit *memEvent = linkDown_->recvInitData()) {
        if (memEvent->getCmd() == Command::nullptrCMD) {
            ...
            if (linkDown_->isDest(memEvent->getSrc()) &&
                memEvent->getInitCmd() == MemEventInit::InitCommand::Coherence) {
...
void Cache::setup() {
    ...
    if (names->empty()) {
        ...
        if (upperLevelCacheNames_.empty())
            upperLevelCacheNames_.push_back(
                ""); // TODO is this a carry over from the old init or is it needed to avoid segfaults still?
...
void Cache::processIncomingEvent(SST::Event *ev) {
    ...
    if (requestsThisCycle_ == maxRequestsPerCycle_) {
        ...
        if ((event->getCmd() == Command::GetS || event->getCmd() == Command::GetX) &&
            !requestBuffer_.empty()) {
        ...
        while (!bankConflictBuffer_[bank].empty() && !bankStatus_[bank]) {
            ...
            if (is_debug_event(bankConflictBuffer_[bank].front()))
                d_->debug(_L9_, "%s, Retrying event from bank conflict. Bank: %u, Event: %s\n",
                          getName().c_str(), bank,
                          bankConflictBuffer_[bank].front()->getBriefString().c_str());
...
void Cache::turnClockOn() {
    ...
    for (int64_t i = 0; i <
                        cyclesOff; i++) {           // TODO more efficient way to do this? Don't want to add in one-shot or we get weird averages/sum sq.
        statMSHROccupancy->addData(mshr_->getSize());
    }
