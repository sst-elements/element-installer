...
CoherenceController::CoherenceController(Component *comp, Params &params) : SubComponent(comp) {
    ...
    output->fatal(CALL_INFO, -1,
                  "%s, Error: CohrenceController subcomponents do not support loading via legacy API\n",
                  getName().c_str());
}

CoherenceController::CoherenceController(ComponentId_t id, Params &params, Params &ownerParams)
    : SubComponent(id) {
    ...
    /* Debug stream */
    debug = new Output("--->  ", params.find<int>("debug_level", 1), 0,
    ...
    if (!found) {
        output->fatal(CALL_INFO, -1,
                      "%s, Param not specified: access_latency_cycles - this is the access time in cycles for the cache; if tag_latency is also specified, this is the data array access time\n",
                      getName().c_str());
    }

    tagLatency_ = params.find<uint64_t>("tag_access_latency_cycles", accessLatency_);
    mshrLatency_ = params.find<uint64_t>("mshr_latency_cycles",
                                         1); /* cacheFactory is currently checking/setting this for us */
    ...
    if (!packetSize.hasUnits("B"))
        output->fatal(CALL_INFO, -1,
                      "%s, Invalid param: min_packet_size - must have units of bytes (B), SI units OK. Ex: '8B'. You specified '%s'\n",
                      ownerName_.c_str(), packetSize.toString().c_str());
    if (!downLinkBW.hasUnits("B"))
        output->fatal(CALL_INFO, -1,
                      "%s, Invalid param: request_link_width - must have units of bytes (B), SI units OK. Ex: '64B'. You specified '%s'\n",
                      ownerName_.c_str(), downLinkBW.toString().c_str());
    if (!upLinkBW.hasUnits("B"))
        output->fatal(CALL_INFO, -1,
                      "%s, Invalid param: response_link_width - must have units of bytes (B), SI units OK. Ex: '64B'. You specified '%s'\n",
...
void CoherenceController::sendNACK(MemEvent *event, bool up, SimTime_t timeInNano) {
    ...
    if (is_debug_event(event)) debug->debug(_L3_, "Sending NACK at cycle = %"
    PRIu64
    "\n", deliveryTime);
}
...
uint64_t CoherenceController::sendResponseUp(MemEvent *event, vector <uint8_t> *data, bool replay,
                                             uint64_t baseTime, bool atomic) {
    return sendResponseUp(event, CommandResponse[(int) event->getCmd()], data, false, replay,
                          baseTime, atomic);
}


/* Send response towards the CPU. L1s need to implement their own to split out the requested block */
uint64_t CoherenceController::sendResponseUp(MemEvent *event, Command cmd, vector <uint8_t> *data,
                                             bool dirty, bool replay, uint64_t baseTime,
                                             bool atomic) {
...
/* Send response towards the CPU. L1s need to implement their own to split out the requested block */
uint64_t CoherenceController::sendResponseUp(MemEvent *event, Command cmd, vector <uint8_t> *data,
                                             bool replay, uint64_t baseTime, bool atomic) {
...
void CoherenceController::resendEvent(MemEvent *event, bool up) {
    ...
    if (is_debug_event(event))
        debug->debug(_L3_, "Sending request: %s\n", event->getBriefString().c_str());
}


/* Forward a message to a lower level (towards memory) in the hierarchy */
uint64_t CoherenceController::forwardMessage(MemEvent *event, Addr baseAddr,
                                             unsigned int requestSize, uint64_t baseTime,
                                             vector <uint8_t> *data) {
    ...
    if (is_debug_event(event)) debug->debug(_L3_, "Forwarding request at cycle = %"
    PRIu64
    "\n", deliveryTime);
...
bool CoherenceController::sendOutgoingCommands(SimTime_t curTime) {
    ...
    while (!outgoingEventQueueUp_.empty() &&
           outgoingEventQueueUp_.front().deliveryTime <= timestamp_) {
...
void CoherenceController::notifyListenerOfAccess(MemEvent *event, NotifyAccessType accessT,
                                                 NotifyResultType resultT) {
    if (!event->isPrefetch()) {
        CacheListenerNotification notify(event->getAddr(), event->getBaseAddr(),
                                         event->getVirtualAddress(),
                                         event->getInstructionPointer(), event->getSize(), accessT,
                                         resultT);
...
void CoherenceController::setupLowerStatus(bool silentEvict, bool isLastCoherenceLevel,
                                           bool expectWritebackAck, bool lowerIsNoninclusive) {
...
void CoherenceController::printStatus(Output &out) {
    ...
    for (list<Response>::iterator it = outgoingEventQueue_.begin();
         it != outgoingEventQueue_.end(); it++) {
        out.output("      Time: %"
        PRIu64
    ...
    for (list<Response>::iterator it = outgoingEventQueueUp_.begin();
         it != outgoingEventQueueUp_.end(); it++) {
        out.output("      Time: %"
        PRIu64
        ", Event: %s\n", (*it).deliveryTime, (*it).event->getVerboseString().c_str());
    }
