...
/* Element Library Info */
            SST_ELI_REGISTER_COMPONENT(Cache,
            "memHierarchy", "Cache", SST_ELI_ELEMENT_VERSION(1,0,0), "Cache controller", COMPONENT_CATEGORY_MEMORY)

            SST_ELI_DOCUMENT_PARAMS(
            /* Required */
            { "cache_frequency", "(string) Clock frequency or period with units (Hz or s; SI units OK). For L1s, this is usually the same as the CPU's frequency.", nullptr },
            ...
            {
                "high_network_0", "Port connected to higher level caches (closer to CPU)", {
                    "memHierarchy.MemEventBase"}
            },
            {
                "directory", "Network link port to directory; doubles as request network port for split networks", {
                    "memHierarchy.MemRtrEvent"}
            },
            {
                "directory_ack", "For split networks, response/ack network port to directory", {
                    "memHierarchy.MemRtrEvent"}
            },
            {
                "directory_fwd", "For split networks, forward request network port to directory", {
                    "memHierarchy.MemRtrEvent"}
            },
            {
                "directory_data", "For split networks, data network port to directory", {
                    "memHierarchy.MemRtrEvent"}
            },
            {
                "cache", "Network link port to cache; doubles as request network port for split networks", {
                    "memHierarchy.MemRtrEvent"}
            },
            {
                "cache_ack", "For split networks, response/ack network port to cache", {
                    "memHierarchy.MemRtrEvent"}
            },
            {
                "cache_fwd", "For split networks, forward request network port to cache", {
                    "memHierarchy.MemRtrEvent"}
            },
            {
                "cache_data", "For split networks, data network port to cache", {
                    "memHierarchy.MemRtrEvent"}
            })
...
/* Class definition */
        ...
        private:
            ...
            ReplacementMgr *constructReplacementManager(std::string policy, uint64_t lines,
            ...
            inline bool allocateDirCacheLine(MemEvent *event, Addr baseAddr, CacheLine *dirLine,
                                             bool noStall);
            ...
            inline bool activatePrevEvent(MemEvent *event, vector <mshrType> &entries, Addr addr,
                                          vector<mshrType>::iterator it, int i);

            inline void postRequestProcessing(MemEvent *event, CacheLine *cacheLine, bool mshrHit);

            inline void postReplacementProcessing(MemEvent *event, CacheAction action,
                                                  bool mshrHit);
