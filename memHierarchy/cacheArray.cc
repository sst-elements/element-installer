...
/* Set Associative Array Class */
        SetAssociativeArray::SetAssociativeArray(Output *dbg, unsigned int numLines,
                                                 unsigned int lineSize, unsigned int associativity,
                                                 ReplacementMgr *rm, HashFunction *hf,
                                                 bool sharersAware) :
        ...
        CacheArray::CacheLine *SetAssociativeArray::findReplacementCandidate(const Addr baseAddr,
                                                                             bool cache) {
        ...
        unsigned int SetAssociativeArray::preReplace(const Addr baseAddr) {
            ...
            return replacementMgr_->findBestCandidate(setBegin, setStates, setSharers, setOwned,
                                                      sharersAware_ ? true : false);
        }

        void SetAssociativeArray::replace(const Addr baseAddr, CacheArray::CacheLine *candidate,
                                          CacheArray::DataLine *dataCandidate) {
...
/* Dual Set Associative Array Class */
        DualSetAssociativeArray::DualSetAssociativeArray(Output *dbg, unsigned int lineSize,
                                                         HashFunction *hf, bool sharersAware,
                                                         unsigned int dirNumLines,
                                                         unsigned int dirAssociativity,
                                                         ReplacementMgr *dirRp,
                                                         unsigned int cacheNumLines,
                                                         unsigned int cacheAssociativity,
                                                         ReplacementMgr *cacheRp) :
            CacheArray(dbg, dirNumLines, dirAssociativity, lineSize, dirRp, hf, sharersAware,
                       false) {
            // Set up data cache
            ...
            for (int i = setBegin; i < setEnd; i++) {
                if (lines_[i]->getBaseAddr() == baseAddr) {
                    if (update) {
                        ...
                        if (lines_[i]->getDataLine() != nullptr) {
        ...
        CacheArray::CacheLine *DualSetAssociativeArray::findReplacementCandidate(
            const Addr baseAddr, bool cache) {
            ...
            if (!cache && dataLines_[index]->getDirLine() == nullptr) {
                ...
                if (currentDirEntry->getDataLine() != nullptr) {
                    dbg_->fatal(CALL_INFO, -1,
                                "Error: Attempting to allocate a data line for a directory entry that already has an associated data line. Addr = 0x%"
                    PRIx64
                    "\n", baseAddr);
                }
                currentDirEntry->setDataLine(
                    dataLines_[index]);    // Don't need to set dirLine, "replace" will do that
        ...
        unsigned int DualSetAssociativeArray::preReplaceDir(const Addr baseAddr) {
            ...
            return replacementMgr_->findBestCandidate(setBegin, dirSetStates, dirSetSharers,
                                                      dirSetOwned, sharersAware_);
        }

        void DualSetAssociativeArray::replace(const Addr baseAddr, CacheArray::CacheLine *candidate,
                                              CacheArray::DataLine *dataCandidate) {
            unsigned int index = candidate->getIndex();
            if (dataCandidate == nullptr) {
                replacementMgr_->replaced(index);
                if (lines_[index]->getDataLine() != nullptr) {
        ...
        unsigned int DualSetAssociativeArray::preReplaceCache(const Addr baseAddr) {
            ...
            for (unsigned int id = 0; id < cacheAssociativity_; id++) {
                int dirIndex = dataLines_[id + setBegin]->getDirLine() ? dataLines_[id +
                                                                                    setBegin]->getDirLine()->getIndex()
                                                                       : -1;
                if (dirIndex == -1) {
                    cacheSetStates[id] = I;
            ...
            return cacheReplacementMgr_->findBestCandidate(setBegin, cacheSetStates,
                                                           cacheSetSharers, cacheSetOwned,
                                                           sharersAware_);
        }
...
/* Cache Array Class */
        ...
        void CacheArray::errorChecking() {
            if (0 == numLines_ || 0 == numSets_)
                dbg_->fatal(CALL_INFO, -1,
                            "Cache size and/or number of sets not greater than zero. Number of lines = %d, Number of sets = %d.\n",
                            numLines_, numSets_);
            // TODO relax this, use mod instead of setmask_
            if ((numSets_ * associativity_) != numLines_)
                dbg_->fatal(CALL_INFO, -1,
                            "Wrong configuration.  Make sure numSets * associativity = Size/cacheLineSize. Number of sets = %d, Associtaivity = %d, Number of lines = %d.\n",
                            numSets_, associativity_, numLines_);
            if (associativity_ < 1)
                dbg_->fatal(CALL_INFO, -1,
                            "Associativity has to be greater than zero. Associativity = %d\n",
                            associativity_);
        }
