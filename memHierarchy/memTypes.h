 ...
 *****************************************************************************************/
#define X_CMDS \
    X(nullptrCMD,          nullptrCMD,        Request,    Request,        1, 0,   Cache)   /* Dummy command */\
    /* Requests */ \
    ...
    /* Request Responses */\
    X(GetSResp,         nullptrCMD,        Response,   Data,           0, 0,   Cache)   /* Response to a GetS request */\
    X(GetXResp,         nullptrCMD,        Response,   Data,           0, 0,   Cache)   /* Response to a GetX request */\
    X(FlushLineResp,    nullptrCMD,        Response,   Ack,            0, 0,   Cache)   /* Response to FlushLine request */\
    X(FlushAllResp,     nullptrCMD,        Response,   Ack,            0, 0,   Cache)   /* Response to FlushAll request */\
    ...
    X(FetchResp,        nullptrCMD,        Response,   Data,           1, 0,   Cache)   /* response to a Fetch, FetchInv or FetchInvX request */\
    X(FetchXResp,       nullptrCMD,        Response,   Data,           1, 0,   Cache)   /* response to a FetchInvX request - indicates a shared copy of the line was kept */\
    /* Others */\
    X(NACK,             nullptrCMD,        Response,   Ack,            1, 0,   Cache)   /* NACK response to a message */\
    X(AckInv,           nullptrCMD,        Response,   Ack,            1, 0,   Cache)   /* Acknowledgement response to an invalidation request */\
    X(AckPut,           nullptrCMD,        Response,   Ack,            0, 0,   Cache)   /* Acknowledgement response to a replacement (Put*) request */\
    X(Put,              AckMove,        Request,    Request,        1, 0,   Move) \
    X(Get,              AckMove,        Request,    Request,        1, 0,   Move) \
    X(AckMove,          nullptrCMD,        Response,   Ack,            0, 0,   Move) \
    ...
    X(CustomResp,       nullptrCMD,        Response,   Data,           0, 0,   Custom) \
    X(CustomAck,        nullptrCMD,        Response,   Ack,            0, 0,   Custom)
