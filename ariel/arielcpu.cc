...
/** End memory manager subcomponent parameter translation */
    ...
    if (nullptr != (memmgr = loadUserSubComponent<ArielMemoryManager>("memmgr"))) {
    ...
    // Remember that the list of arguments must be nullptr terminated for execution
    execute_args[(pin_arg_count - 1) + app_argc] = nullptr;
...
int ArielCPU::forkPINChild(const char *app, char **args,
    ...
    while (nullptr != args[next_arg_index]) {
    ...
    if (the_child != 0) {
        ...
        if (0 == app_env.size()) {
            ...
            execute_env_cp[app_env.size()] = nullptr;
