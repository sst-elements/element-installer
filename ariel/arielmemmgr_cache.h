...
#define ARIEL_ELI_MEMMGR_CACHE_STATS { "tlb_hits", "Hits in the simple Ariel TLB", "hits", 2 },\
        ...
        { "tlb_page_allocs",      "Number of pages allocated by the memory manager", "pages", 2 }
            ...
            /* Constructor
                ...
                */
            ArielMemoryManagerCache(ComponentId_t id, Params &params) : ArielMemoryManager(id,
                                                                                           params) {
                ...
                translationCacheEntries = (uint32_t) params.find<uint32_t>("translatecacheentries",
                                                                           4096);

                /* Statistics used by all memory managers; managers may also have their own */
            } // End constructor

            ArielMemoryManagerCache(Component *comp, Params &params) : ArielMemoryManager(comp,
                                                                                          params) {} // Legacy
        ...
        protected:
            ...
            void mapPagesRandom(uint64_t pageCount, uint64_t pageSize, uint64_t startAddr,
                ...
                for (uint64_t j = 0; j < pageCount; ++j) {
                    output->verbose(CALL_INFO, 64, 0, "Page[%"
                    PRIu64
                    "] Physical Start=%"
                    PRIu64
                    "\n",
            ...
            void populatePageTable(std::string popFilePath,
                                   ...
                                   std::deque <uint64_t> *freePagePool, uint64_t pageSize) {
                ...
                while (!feof(popFile)) {
                    ...
                    if (freePagePool->size() == 0) {
                        output->fatal(CALL_INFO, -1, "Attempted to pin address %"
                        PRIu64
                        " but no free pages.\n", pinAddr);
                    }
                    ...
                    if (pinAddr % pageSize > 0) {
                        output->fatal(CALL_INFO, -1, "Attempted to pin address %"
                        PRIu64
                        " but address is not page aligned to page size %"
                        PRIu64
                        "\n",
                            pinAddr, pageSize);
                    }
                    ...
                    output->verbose(CALL_INFO, 4, 0, "Pinning address %"
                    PRIu64
                    " (physical=%"
                    PRIu64
                    "\n",
